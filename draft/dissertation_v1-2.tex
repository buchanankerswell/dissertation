...
\caption[Example of marker classification for model cdi62]{Example of marker classification for model cdi62. (a) Guassian mixture modelling assigns 18,891 markers (colored squares) into 14 clusters according to their maximum \gls{pt}. Markers belonging to clusters with centroids positioned at \(\geq\) 3 \(^\circ C/km\) AND \(\leq\) 1300 \(^\circ C\) AND \(\leq\) 120 \(km\) are classified as recovered. In this experiment, (b) markers are generally recovered from thermal gradients between 5-10 \(^\circ C/km\) (thin black lines), but very few are recovered from the high-density region shared by \texttt{pd15} and \texttt{ag18} around 2 \(GPa\) and 550 \(^\circ C\) (light-pink contours; solid and filled). Contours in (b) are computed using Gaussian kernels with different binwidths for \texttt{pd15} (solid colored contours; n = 355 samples) and \texttt{ag18} (filled colored contours; n = 130 samples).}\label{fig:class}
\end{figure}
...
\caption[\gls{pt} distribution of recovered markers from all simulations]{Distribution of recovered markers from all 64 simulations. (a) Markers are generally recovered from near the continental Moho around 1 \(GPa\) and 350 \(^\circ C\), or near the antigorite dehydration reaction (onset of plate coupling), or broadly dispersed between 1-2.5 \(GPa\) up to approximately 500 \(^\circ C\). Note the gap in recovery around 2 \(GPa\) and 550 \(^\circ C\) perfectly coinciding with the highest-density of natural samples. (b) While the distribution of markers across P is very similar to \texttt{ag18}, (c-d) the distribution across T shows that markers are generally recovered from lower thermal gradients (85.5 \% from \(\leq\) 10 \(^\circ C/km\) and 98.5 \% from \(\leq\) 15 \(^\circ C/km\)) compared to natural samples (between 52-63\% from \(\leq\) 10 \(^\circ C/km\) and 70-94\% from \(\leq\) 15 \(^\circ C/km\)).}\label{fig:meta}
\end{figure}
