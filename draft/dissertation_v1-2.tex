...
Similarity and Kriging interpolations are easily distinguishable by eye at the regional scale when compared side-by-side (e.g.~compare Figures \ref{fig:centralAmericaDiff}, \ref{fig:kyushuRyukyuDiff}, \ref{fig:scotiaDiff}, \ref{fig:sumatraBandaSeaUpper}, \ref{fig:kyushuRyukyuUpper}, \ref{fig:newBritainSolomonUpper} with the remaining segments in Appendices \ref{interpDiffAppendix} \& \ref{lateralDiffAppendix}). The underlying mathematical frameworks that quickly discern Similarity from Kriging interpolations can be independently leveraged as strengths under certain circumstances.

For example, low and/or highly irregular observational density characterize surface heat flow observations globally (\protect\hyperlink{ref-lucazeau2019}{Lucazeau, 2019}) and is common across all 13 subduction zone segments (Tables @ref(tab:hfSummaryTable \& \ref{tab:sectorSummaryTable})). However, because Similarity is inherently agnostic to the spatial density and/or regularity of observations (\protect\hyperlink{ref-goutorbe2011}{Goutorbe et al., 2011}), accurate interpolations with well-defined plate boundaries are still possible for regions with relatively few observations (e.g.~Scotia and New Britain Solomon, Figures \ref{fig:scotiaDiff} \& \ref{fig:newBritainSolomonDiff}). Similarity is applied in the exact same manner across an area with 5 observations as an area with 5000 observations. Since surface heat flow near most subduction zone segments is sparsely surveyed, maintaining spatial-independence from observations during the interpolation process is a desirable property.

On the other hand, conserving ``ground-truth'' during the interpolation process is equally desirable. Local ordinary Kriging conserves ground-truth by remaining agnostic to all other factors but the spatial configuration of empirical data (see Appendix \ref{krigeOpt}). For example, Kriging resolves tectonic features near Tonga New Zealand and Vanuatu that are compatible with ThermoGlobe data but discordant with Similarity predictions (Figures \ref{fig:tongaNewZealandDiff} \& \ref{fig:vanuatuDiff}). Features highlighted by Kriging that contrast with Similarity interpolations imply incompatibility between ThermoGlobe data and one or more geologic proxy data sets used by Similarity. That is, Kriging will tend to highlight anomalies (compared to Similarity) if they exist and have been observed.

In principle, combining the desirable properties of Similarity (independent of observational density) and Kriging (conserving ground-truth) during the interpolation process is possible by layering Similarity and Kriging interpolations. A layered approach simultaneously respects the First (\protect\hyperlink{ref-krige1951}{Krige, 1951}) and Third Laws of Geography (\protect\hyperlink{ref-zhu2018}{Zhu et al., 2018}) by integrating geologic and spatial information. Any number of approaches may be applied to combine Kriging (layer 0) and Similarity (layer 1) predictions. As an example, comparing layers 0 \& 1 to detect anomalies then combine with a weighted average scheme. Chose weights proportional to the squared difference between layers 0 \& 1 such that anomalies are emphasized (or subdued).
