...
Surface heat flow profiles across subduction zone segments are computed in previous studies by sampling ThermoGlobe data from within a single sector (\protect\hyperlink{ref-currie2004}{Currie et al., 2004}; \protect\hyperlink{ref-currie2006}{Currie \& Hyndman, 2006}; \protect\hyperlink{ref-furukawa1993}{Furukawa, 1993}; \protect\hyperlink{ref-hyndman2005}{Hyndman et al., 2005}; \protect\hyperlink{ref-kerswell2020}{Kerswell et al., 2020}; \protect\hyperlink{ref-wada2009}{Wada \& Wang, 2009}). Although simple to implement, sampling ThermoGlobe data within adjacent sectors is ineffective for evaluating surface heat flow variability along-strike. The irregular spatial density and coverage of ThermoGlobe data make comparisons of summary statistics among sectors unreliable. For example, sample sizes within adjacent sectors can be drastically different (e.g.~Central America, Sumatra Banda Sea, or Tonga New Zealand, Table \ref{tab:sectorSummaryTable}).

Indeed, this study finds comparing ThermoGlobe data among upper-plate sectors problematic. For instance, ThermoGlobe data are often too few (n \textless{} 30 observations for 68/100 sectors, Table \ref{tab:sectorSummaryTable}) to compare with statistical confidence. Moreover, many (n = 11) sectors have a single observation with a singular distribution (\gls{iqr} = 0) or very few observations spanning a large range (very large \gls{iqr}). Many sectors are omitted in the analysis because they simply encompass zero ThermoGlobe data, although Similarity and Kriging predictions could still be compared.

Limitations arising from sampling irregularly-spaced data are easily overcome by a regular sampling grid. A regular grid allows for more consistent sample sizes and spatial coverage across sectors. For example, many sectors defined in this study have few ThermoGlobe data (n \textless{} 10 observations for 52/100 sectors, Table \ref{tab:sectorSummaryTable}), while the average number of interpolation grid points within upper-plate sectors in this study is 65. However, the advantages of regular grids (consistent sample sizes and regular spatial coverage) can be abstracted to ThermoGlobe data through interpolation (e.g.~Kriging, Similarity, or other). Surface heat flow variability among sectors is thus more confidently and consistently evaluated with Similarity and Kriging predictions compared to ThermoGlobe data alone.
