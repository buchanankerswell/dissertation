...
Surface heat flow profiles are computed for several adjacent upper-plate regions (sectors) to assess lateral (along-strike) surface heat flow variability. Profiles are defined by (1) splitting a subduction zone segment into 2-8 equidistant parts, (2) defining a 500 $km$-wide single-sided buffer around each part, and (3) finding the orthogonal great circle distance between each surface heat flow prediction, or observation, and the trench. Steps (1-3) above closely approximate collapsing the 2D distribution of surface heat flow within a sector onto a single trench-orthogonal line [e.g., @currie2004; @currie2006; @hyndman2005; @wada2009]. Surface heat flow variance is generally very high, so the profiles are smoothed with a five-point running average and fit with a local regression curve [LOESS, @cleveland1988].

### Similarity accuracy

Similarity accuracy is typically evaluated with a leave-one-out or k-fold cross-validation approach [e.g. @goutorbe2011].
...
footnote(
  general_title = '',
  general = c('\\\\uline{\\\\textit{note}}: Showing optimal Krige models from Table \\\\ref{tab:vgrmSummaryTable}, all units are $mWm^{-2}$', '\\\\uline{\\\\textit{key}}: $RMSE_S$: Similarity accuracy, $RMSE_K$: Kriging accuracy'),
  threeparttable = T,
  escape = F
)
```
...
Below are all fitted variogram models for each subduction zone segment. Please refer to Table \@ref(tab:vgrmSummaryTable) for variogram parameters.
