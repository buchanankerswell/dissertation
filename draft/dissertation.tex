...
These methods are called \emph{Similarity} methods (Figure \ref{fig:globalComp}) and follow assumptions embedded in the Third Law of Geography: \emph{the more similar the geographic configuration of two points, the more similar their values} (hereafter referred to as the Third Law, \protect\hyperlink{ref-zhu2018}{Zhu et al., 2018}). Global scale problems are particularly suited for this approach as it does not rely on sampling density. On the other hand, assumed interactions among many independent geologic proxies, each with its own assumptions and uncertainties, makes Similarity methods complex combinatorial problems vulnerable to decisions about the number of proxies, proxy dataset quality, and simplified geologic models used to compute observable classes (see \protect\hyperlink{ref-goutorbe2011}{Goutorbe et al., 2011} for method details).
...
\caption[Global heat flow and \gls{sz} boundaries]{Global \gls{shf} observations (top) and similarity interpolation (bottom) cropped within 1000 \(km\) buffers of 13 \gls{sz} segments. Note that \gls{shf} above 250 \(mWm^{-2}\) is colored yellow-white. ThermoGlobe data from Lucazeau (\protect\hyperlink{ref-lucazeau2019}{2019}) \& Jennings \& Hasterok (\protect\hyperlink{ref-jennings2021}{2021}) and \gls{sz} boundaries as defined by Syracuse \& Abers (\protect\hyperlink{ref-syracuse2006}{2006}).}\label{fig:globalComp}
\end{figure}

In contrast to the Third Law, there exists some degree of spatial dependence, or continuity, in the distribution of \gls{shf}. For example, a pair of \gls{shf} observations taken one meter apart will be strongly correlated, but the correlation will likely decrease with increasing distance between the points (\protect\hyperlink{ref-goovaerts1997}{Goovaerts, 1997}). This is encapsulated in the First Law of Geography (hereafter referred to as the First Law): \emph{everything is related, but nearer things are more related} (\protect\hyperlink{ref-krige1951}{Krige, 1951}; \protect\hyperlink{ref-matheron1963}{Matheron, 1963}). Generally speaking, the spatial (dis)continuity of \gls{shf} reflects the continuity of subsurface geodynamics plus near-surface perturbations. Broad low \gls{shf} regions on continents outline cratons (Figure \ref{fig:tglobeComp}), for example, and uniform \gls{shf} near \glspl{sz} are interpreted to reflect uniform lithospheric thickness (\protect\hyperlink{ref-currie2004}{Currie et al., 2004}; \protect\hyperlink{ref-currie2006}{Currie \& Hyndman, 2006}; \protect\hyperlink{ref-hyndman2005}{Hyndman et al., 2005}) and mechanical coupling depths (\protect\hyperlink{ref-furukawa1993}{Furukawa, 1993}; \protect\hyperlink{ref-kerswell2020}{Kerswell et al., 2020}; \protect\hyperlink{ref-wada2009}{Wada \& Wang, 2009}).
...
The \href{http://heatflow.org}{ThermoGlobe database} is available from the supplementary material of Lucazeau (\protect\hyperlink{ref-lucazeau2019}{2019}) and is accessible online at \url{http://heatflow.org} (\protect\hyperlink{ref-jennings2021}{Jennings \& Hasterok, 2021}). It currently contains 69,729 data points, their locations in latitude/longitude, and important metadata---including a data quality rank (\texttt{Code\ 6}) from A to D. The reader is referred to Lucazeau (\protect\hyperlink{ref-lucazeau2019}{2019}) and \url{http://heatflow.org} for details on compilation, references, and historical perspective on ThermoGlobe and previous compilations. We use ThermoGlobe because it is the most recent database available, has been carefully compiled, and is open-access despite containing much proprietary data.

Like Lucazeau (\protect\hyperlink{ref-lucazeau2019}{2019}), we exclude 4,790 poor quality observations (\texttt{Code\ 6} = D) from our analysis. We further remove 350 data points without heat flow observations and two without geographic information. Note that 9,315 observations have an undetermined quality (\texttt{Code\ 6} = Z). Multiple observations at the same location are parsed to avoid singular covariance matrices during Kriging:

\begin{equation}
  f(x_i,\ y_i)=
  \begin{cases}
    x_i & \text{if } Q_i^x > Q_i^y \\
    y_i & \text{if } Q_i^x < Q_i^y \\
    RAND(x_i,\ y_i) & \text{if } Q_i^x = Q_i^y
  \end{cases}
  \label{eq:parse}
\end{equation}

where \(x_i\) and \(y_i\) are duplicate observation pairs at location \(i\), \(Q_i^x\) and \(Q_i^y\) represent the quality of each duplicate observation pair, and \(RAND\) is a random function that selects either the observation \(x_i\) or \(y_i\). This procedure parses through each duplicate observation and selects the best quality measurement. If the measurements are equal quality, it randomly selects either observation. The final dataset used for Kriging has n = 55,245 observations after parsing n = 16,215 duplicate pairs.

\hypertarget{kriging}{%
\subsection{Kriging}\label{kriging}}

Kriging is a three-step process that involves first estimating an experimental variogram, \(\hat{\gamma}(h)\), fitting the experimental variogram with one of many variogram models, \(\gamma(h)\), and finally using the modelled variogram to predict random variables at unknown locations (\protect\hyperlink{ref-cressie2015}{Cressie, 2015}; \protect\hyperlink{ref-krige1951}{Krige, 1951}). We use the general-purpose functions defined in the ``R'' package \texttt{gstat} (\protect\hyperlink{ref-graler2016}{Gräler et al., 2016}; \protect\hyperlink{ref-pebesma2004}{Pebesma, 2004}) to perform all three steps. We begin by estimating an experimental variogram as defined by Bárdossy (\protect\hyperlink{ref-bardossy1997}{1997}):
...
where \(N(h)\) is the number of pairs of points, \(Z(u_i)\) and \(Z(u_j)\), separated by a lag distance, \(h\), \(\delta\) and \(n=15\) are used to define a binwidth for evaluating \(\hat{\gamma}(h)\) with irregularly spaced data. In simple terms, Equation \eqref{eq:variogram} represents the similarity, or dissimilarity, between pairs of observations in space. Equation \eqref{eq:variogram} adheres to the First Law of Geography and is derived from the theory of \emph{regionalized variables} (\protect\hyperlink{ref-matheron1963}{Matheron, 1963}, \protect\hyperlink{ref-matheron2019}{2019}), which formally defines a probabilistic framework for spatial interpolation of natural phenomena.
...
ThermoGlobe data and similarity interpolations from Lucazeau (\protect\hyperlink{ref-lucazeau2019}{2019}) are transformed onto a Pacific-centered Robinson \gls{crs} defined using the \texttt{proj} (\protect\hyperlink{ref-proj2021}{contributors, 2021}) string ``\texttt{+proj=robin\ +lon\_0=-155\ +lon\_wrap=-155\ +x\_0=0\ +y\_0=0\ +ellps=WGS84\ +datum=WGS84\ +units=m\ +no\_defs}''. All geographic operations, including Kriging and comparisons with similarity interpolations, are performed in the above \gls{crs} using the general-purpose functions in the \texttt{R} package \texttt{sf} (\protect\hyperlink{ref-pebesma2018}{Pebesma, 2018}). The Kriging domain is defined near individual arc segments in two steps: 1) 1000 \(km\) buffers are drawn around the arc segments as defined by Syracuse \& Abers (\protect\hyperlink{ref-syracuse2006}{2006}). 2) The bounding box of the 1000 \(km\) buffer is expanded by 10\% on all sides (Figure \ref{fig:segments}). Kriging interpolations are estimated on the same grid as Lucazeau (\protect\hyperlink{ref-lucazeau2019}{2019}) so differences can be taken point-by-point at the exact same locations.



\begin{figure}[htbp]

{\centering \includegraphics[width=1\linewidth,]{assets/figs/chpt3/segs_comp} 

}

\caption[]{Constructing interpolation domains around \gls{sz} segments. (a) Bounding boxes are drawn around thirteen \gls{sz} segments by (b) drawing a 1000 \(km\) buffer (lightest blue) around each segment and expanding the buffer's bounding box (medium blue) by 10\% on all sides (darkest blue). (c) ThermoGlobe data is cropped within the largest rectangle. Data from Syracuse \& Abers (\protect\hyperlink{ref-syracuse2006}{2006}) and Lucazeau (\protect\hyperlink{ref-lucazeau2019}{2019}).}\label{fig:segments}
\end{figure}
...
Summary statistics for surface heat flow observations by subduction zone segment are given in Table \ref{tab:hfSummaryTable} and Figure \ref{fig:hfSummaryPlot}. Surface heat flow is median-centered around 65 \(mWm^{-2}\) and narrowly distributed for most subduction zone segments, excluding outliers, with inter-quartile ranges (IQR) median-centered around 43 \(mWm^{-2}\). Alaska Aleutians is the exception with a higher median heat flow of 184 \(mWm^{-2}\) and broader range (IQR = 206 \(mW^{-2}\)). The whole distributions (including outliers) for all segments are strongly right-skewed with maximum heat flow values of several thousand of \(mWm^{-2}\) or more. Heat flow values above 250 \(mWm^{-2}\) are considered geothermal areas by Lucazeau (\protect\hyperlink{ref-lucazeau2019}{2019}), which we adopt as a relevant empirical limit for anomalously high heat flow.

\begin{table}

\caption{\label{tab:hfSummaryTable}ThermoGlobe summary by segment}
\centering
\begin{threeparttable}
...
\midrule
\cellcolor{gray!6}{Alaska Aleutians} & \cellcolor{gray!6}{2793} & \cellcolor{gray!6}{4} & \cellcolor{gray!6}{7765} & \cellcolor{gray!6}{184} & \cellcolor{gray!6}{206} & \cellcolor{gray!6}{233} & \cellcolor{gray!6}{344}\\
Andes & 5229 & 7 & 1710 & 45 & 50 & 87 & 91\\
\cellcolor{gray!6}{Central America} & \cellcolor{gray!6}{5043} & \cellcolor{gray!6}{8} & \cellcolor{gray!6}{911} & \cellcolor{gray!6}{45} & \cellcolor{gray!6}{48} & \cellcolor{gray!6}{88} & \cellcolor{gray!6}{90}\\
Kamchatka Marianas & 3951 & 1 & 25200 & 70 & 48 & 116 & 574\\
\cellcolor{gray!6}{Kyushu Ryukyu} & \cellcolor{gray!6}{3239} & \cellcolor{gray!6}{1} & \cellcolor{gray!6}{25200} & \cellcolor{gray!6}{72} & \cellcolor{gray!6}{45} & \cellcolor{gray!6}{125} & \cellcolor{gray!6}{633}\\
\addlinespace
Lesser Antilles & 3536 & 13 & 1710 & 41 & 13 & 53 & 61\\
\cellcolor{gray!6}{N Philippines} & \cellcolor{gray!6}{998} & \cellcolor{gray!6}{3} & \cellcolor{gray!6}{25200} & \cellcolor{gray!6}{71} & \cellcolor{gray!6}{35} & \cellcolor{gray!6}{191} & \cellcolor{gray!6}{1112}\\
New Britain Solomon & 180 & 3 & 174 & 64 & 42 & 69 & 30\\
\cellcolor{gray!6}{S Philippines} & \cellcolor{gray!6}{1441} & \cellcolor{gray!6}{1} & \cellcolor{gray!6}{25200} & \cellcolor{gray!6}{71} & \cellcolor{gray!6}{43} & \cellcolor{gray!6}{156} & \cellcolor{gray!6}{924}\\
Scotia & 72 & 13 & 145 & 76 & 23 & 74 & 28\\
\addlinespace
\cellcolor{gray!6}{Sumatra Banda Sea} & \cellcolor{gray!6}{3033} & \cellcolor{gray!6}{1} & \cellcolor{gray!6}{1350} & \cellcolor{gray!6}{65} & \cellcolor{gray!6}{47} & \cellcolor{gray!6}{76} & \cellcolor{gray!6}{66}\\
Tonga New Zealand & 508 & 2 & 414 & 54 & 40 & 64 & 45\\
\cellcolor{gray!6}{Vanuatu} & \cellcolor{gray!6}{349} & \cellcolor{gray!6}{2} & \cellcolor{gray!6}{283} & \cellcolor{gray!6}{54} & \cellcolor{gray!6}{40} & \cellcolor{gray!6}{64} & \cellcolor{gray!6}{44}\\
\bottomrule
\end{tabular}
\begin{tablenotes}
...
\caption[]{Distribution of ThermoGlobe observations cropped near \gls{sz} segments. Heat flow distributions near most segments are centered between 41 and 76 \(mWm^{-2}\) and highly skewed right. The skeweness likely represents sampling near geothermal systems, volcanic arcs, or spreading centers. Data from Lucazeau (\protect\hyperlink{ref-lucazeau2019}{2019}).}\label{fig:hfSummaryPlot}
\end{figure}

\hypertarget{variogram-models}{%
\subsection{Variogram Models}\label{variogram-models}}

The optimal variogram models and associated errors are given in Table \ref{tab:variogramSummaryTable}. Almost twice as many experimental variograms are fit with spherical models (10) compared to exponential models (3). Variogram model sills vary substantially among the subduction zone segments between 9 and 1181 \(mWm^{-2}\). Variogram model ranges also vary substantially among segments from 4 to 1397 \(km\).

No apparent correlation exists between variogram model range and subduction zone segment length, number of heat flow observations, nor domain area (Figure \ref{fig:variogramSummaryPlot}). Most subduction zone segments show spatial dependence of a few hundred kilometers or less, irrespective of the number of observations or segment size. The exceptions are Kyushu Ryukyu (range = 1397 \(km\)) and Vanuatu (range = 573.2 \(km\)), whose model variogram ranges are, perhaps, anomalously high.

\begin{table}

\caption{\label{tab:variogramSummaryTable}Kriging summary by segment}
\centering
\begin{threeparttable}
\begin{tabular}[t]{llrrrrrrr}
\toprule
\multicolumn{1}{c}{} & \multicolumn{3}{c}{Variogram} & \multicolumn{1}{c}{} & \multicolumn{4}{c}{Domain} \\
\cmidrule(l{3pt}r{3pt}){2-4} \cmidrule(l{3pt}r{3pt}){6-9}
Segment & Model & Sill & Range & RMSE & Length & Area & n & Density\\
\midrule
\cellcolor{gray!6}{Alaska Aleutians} & \cellcolor{gray!6}{Sph} & \cellcolor{gray!6}{285} & \cellcolor{gray!6}{222} & \cellcolor{gray!6}{213.2} & \cellcolor{gray!6}{5054} & \cellcolor{gray!6}{24.32} & \cellcolor{gray!6}{2793} & \cellcolor{gray!6}{114.9}\\
Andes & Sph & 91 & 369 & 50.3 & 7485 & 41.66 & 5229 & 125.5\\
\cellcolor{gray!6}{Central America} & \cellcolor{gray!6}{Sph} & \cellcolor{gray!6}{85} & \cellcolor{gray!6}{335} & \cellcolor{gray!6}{40.7} & \cellcolor{gray!6}{1939} & \cellcolor{gray!6}{15.19} & \cellcolor{gray!6}{5043} & \cellcolor{gray!6}{332.1}\\
Kamchatka Marianas & Exp & 602 & 270 & 530.9 & 6565 & 48.36 & 3951 & 81.7\\
\cellcolor{gray!6}{Kyushu Ryukyu} & \cellcolor{gray!6}{Sph} & \cellcolor{gray!6}{760} & \cellcolor{gray!6}{1397} & \cellcolor{gray!6}{683.2} & \cellcolor{gray!6}{1797} & \cellcolor{gray!6}{14.70} & \cellcolor{gray!6}{3239} & \cellcolor{gray!6}{220.3}\\
\addlinespace
Lesser Antilles & Sph & 60 & 379 & 50.2 & 1214 & 10.90 & 3536 & 324.3\\
\cellcolor{gray!6}{N Philippines} & \cellcolor{gray!6}{Sph} & \cellcolor{gray!6}{1181} & \cellcolor{gray!6}{238} & \cellcolor{gray!6}{1246.2} & \cellcolor{gray!6}{717} & \cellcolor{gray!6}{8.26} & \cellcolor{gray!6}{998} & \cellcolor{gray!6}{120.8}\\
New Britain Solomon & Exp & 29 & 90 & 24.8 & 995 & 9.29 & 180 & 19.4\\
\cellcolor{gray!6}{S Philippines} & \cellcolor{gray!6}{Sph} & \cellcolor{gray!6}{1011} & \cellcolor{gray!6}{415} & \cellcolor{gray!6}{1009.5} & \cellcolor{gray!6}{2543} & \cellcolor{gray!6}{16.92} & \cellcolor{gray!6}{1441} & \cellcolor{gray!6}{85.2}\\
Scotia & Exp & 9 & 4 & 20.8 & 1238 & 9.36 & 72 & 7.7\\
\addlinespace
\cellcolor{gray!6}{Sumatra Banda Sea} & \cellcolor{gray!6}{Sph} & \cellcolor{gray!6}{67} & \cellcolor{gray!6}{136} & \cellcolor{gray!6}{43.1} & \cellcolor{gray!6}{5921} & \cellcolor{gray!6}{40.56} & \cellcolor{gray!6}{3033} & \cellcolor{gray!6}{74.8}\\
Tonga New Zealand & Sph & 47 & 132 & 37.9 & 3333 & 21.99 & 508 & 23.1\\
\cellcolor{gray!6}{Vanuatu} & \cellcolor{gray!6}{Sph} & \cellcolor{gray!6}{48} & \cellcolor{gray!6}{573} & \cellcolor{gray!6}{38.6} & \cellcolor{gray!6}{1550} & \cellcolor{gray!6}{12.84} & \cellcolor{gray!6}{349} & \cellcolor{gray!6}{27.2}\\
\bottomrule
\end{tabular}
\begin{tablenotes}
\item \uline{\textit{key}}: Sill $[mWm^{-2}]$, Range $[km]$, RMSE: cross-validation root mean square error $[mWm^{-2}]$, Length $[km]$, Area $[Mkm^2]$, n $[\#~obs]$, Density $[n/Mkm^2]$
\end{tablenotes}
\end{threeparttable}
\end{table}
...
Segment & n & Min & Max & Median & IQR & Mean & $\sigma$\\
\midrule
\cellcolor{gray!6}{Alaska Aleutians} & \cellcolor{gray!6}{12563} & \cellcolor{gray!6}{-552} & \cellcolor{gray!6}{2653} & \cellcolor{gray!6}{-2} & \cellcolor{gray!6}{20} & \cellcolor{gray!6}{3} & \cellcolor{gray!6}{71}\\
Andes & 18591 & -910 & 2338 & 2 & 36 & 13 & 73\\
\cellcolor{gray!6}{Central America} & \cellcolor{gray!6}{6065} & \cellcolor{gray!6}{-190} & \cellcolor{gray!6}{2731} & \cellcolor{gray!6}{8} & \cellcolor{gray!6}{62} & \cellcolor{gray!6}{44} & \cellcolor{gray!6}{146}\\
Kamchatka Marianas & 22022 & -2777 & 297 & 4 & 15 & 4 & 45\\
\cellcolor{gray!6}{Kyushu Ryukyu} & \cellcolor{gray!6}{6072} & \cellcolor{gray!6}{-2780} & \cellcolor{gray!6}{140} & \cellcolor{gray!6}{5} & \cellcolor{gray!6}{23} & \cellcolor{gray!6}{1} & \cellcolor{gray!6}{85}\\
\addlinespace
Lesser Antilles & 4381 & -904 & 529 & 1 & 21 & 4 & 43\\
\cellcolor{gray!6}{N Philippines} & \cellcolor{gray!6}{3320} & \cellcolor{gray!6}{-2800} & \cellcolor{gray!6}{278} & \cellcolor{gray!6}{0} & \cellcolor{gray!6}{25} & \cellcolor{gray!6}{-5} & \cellcolor{gray!6}{94}\\
New Britain Solomon & 3665 & -81 & 236 & 2 & 22 & 5 & 20\\
\cellcolor{gray!6}{S Philippines} & \cellcolor{gray!6}{6726} & \cellcolor{gray!6}{-273} & \cellcolor{gray!6}{370} & \cellcolor{gray!6}{5} & \cellcolor{gray!6}{26} & \cellcolor{gray!6}{7} & \cellcolor{gray!6}{30}\\
Scotia & 5109 & -62 & 1001 & -4 & 24 & 5 & 33\\
\addlinespace
\cellcolor{gray!6}{Sumatra Banda Sea} & \cellcolor{gray!6}{16254} & \cellcolor{gray!6}{-308} & \cellcolor{gray!6}{386} & \cellcolor{gray!6}{0} & \cellcolor{gray!6}{20} & \cellcolor{gray!6}{2} & \cellcolor{gray!6}{20}\\
Tonga New Zealand & 9401 & -114 & 1695 & -9 & 23 & -2 & 30\\
\cellcolor{gray!6}{Vanuatu} & \cellcolor{gray!6}{5159} & \cellcolor{gray!6}{-166} & \cellcolor{gray!6}{1657} & \cellcolor{gray!6}{7} & \cellcolor{gray!6}{27} & \cellcolor{gray!6}{8} & \cellcolor{gray!6}{40}\\
\bottomrule
\end{tabular}
\begin{tablenotes}
\item \uline{\textit{note}}: Difference is calculated as Similarity - Krige
\item \uline{\textit{key}}: All units are in $mWm^{-2}$
\end{tablenotes}
\end{threeparttable}
\end{table}
...
Arguments in favour of First or Third Law interpolations, however, are not easily generalized. Third Law interpolations are justified in cases with inadequate heat flow observations (e.g.~Scotia and New Britain Solomon). First Law interpolations are arguably more favourable in all cases with adequate heat flow observations because 1) enough observations will resolve important features, 2) spatial dependency is respected, and 3) there are fewer sources of uncertainty. However, it is difficult to know what ``adequate'' observational density and spatial coverage are \emph{a priori}. In any case, it may not be feasible to achieve adequate observational density and spatial coverage due to time and budget constraints. Therefore, hypotheses and sampling strategies should be constructed with careful consideration of whether First or Third Law interpolations are more appropriate on a case-by-case basis.
...
\item
  Should First or Third Law interpolations be favoured when prioritizing data collection targets?
\end{enumerate}
...
\item
  Improving Kriging (First Law) interpolations through focused surveys within priority subduction zone segments is a favourable strategy for future data acquisition
\end{enumerate}
...
\hypertarget{thermoglobe-dataset}{%
\section{ThermoGlobe dataset}\label{thermoglobe-dataset}}
