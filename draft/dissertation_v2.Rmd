...
(ref:hfSummaryPlotCap) Distribution of ThermoGlobe observations from @lucazeau2019 cropped within 1000 $km$-radius circular buffers around 13 active subduction zone segments. Heat flow distributions are centered between `r min(hf.summary$median)` and `r max(hf.summary$median)` $mWm^{-2}$ and highly right-skewed. Skewness represents near-surface perturbations from geothermal systems and tectonic regions with high thermal activity. Diverse interactions between tectonic and near-surface processes are apparent from the broad, irregular surface heat flow distributions. Distributions are colored by quartiles (25%, 50%, 75%).
...
(ref:domainConstructCap) Example of an interpolation domain constructed around Kyushu Ryukyu. (a) ThermoGlobe data are cropped within a domain defined by drawing a 1000 $km$-radius circular buffer (thin black line) around the segment boundary (bold white line). Note that Kyushu Ryukyu is one of the most densely sampled regions, yet still has considerable observational gaps. (b) Similarity presents a textured interpolation by layering many geologic proxy datasets. Similarity interpolation from @lucazeau2019. Segment boundary defined by @syracuse2006. Plate boundaries (solid green lines) defined by @lawver2018.
...
Regional Kriging errors from `r round(min(v.opt$cv.rmse), 1)` to `r round(max(v.opt$cv.rmse), 1)` $mWm^{-2}$ all exceed the maximum regional Similarity errors from `r round(range(rmse.luca$rmse.sim)[2], 1)` $mWm^{-2}$ (Table \@ref(tab:diffSummaryTable)). Kriging errors can be relatively small compared to Similarity for domains with high observational density (`r v.opt$segment[which.min(v.opt$cv.rmse)]`; n = `r format(nrow(shp.hf.crop[[v.opt$segment[which.min(v.opt$cv.rmse)]]]), big.mark = ',')`, $\Delta$RMSE = `r round(v.opt$cv.rmse[which.min(v.opt$cv.rmse)] - rmse.luca$rmse.sim[which.min(v.opt$cv.rmse)], 1)`) but relatively large where observational density is comparatively low (`r v.opt$segment[which.max(v.opt$cv.rmse)]`; n = `r format(nrow(shp.hf.crop[[v.opt$segment[which.max(v.opt$cv.rmse)]]]), big.mark = ',')`, $\Delta$RMSE = `r round(v.opt$cv.rmse[which.max(v.opt$cv.rmse)] - rmse.luca$rmse.sim[which.max(v.opt$cv.rmse)], 1)`). On average, Kriging error rates are `r round(mean(v.opt$cv.rmse - mean(rmse.luca$rmse.sim))/mean(rmse.luca$rmse.sim), 1)` times higher than Similarity. Moreover, regional Similarity errors in this study are consistent with, or smaller than, the 7 $mWm^{-2}$ error reported by @lucazeau2019 for their preferred global model (with 7 proxy observables).
...
### Lateral Upper-plate Surface Heat Flow Variability

Lateral surface heat flow variability along subduction zone upper-plates may fall into 1 of 8 categories based on Similarity and Kriging predictions within upper-plate sectors. Each upper-plate can be characterized as having (dis)continuous Similarity predictions, (dis)continuous Kriging predictions, and (in)distinguishable Similarity vs. Kriging predictions. Discontinuous upper-plate surface heat flow is more commonly predicted by either interpolation method than continuous upper-plate surface heat flow (see Appendix \@ref(interpDiffAppendix)). Discontinuous surface heat flow is predicted by either Similarity or Kriging for 11/13 segments and by both Similarity and Kriging for 7/13 segments (e.g., Kyushu Ryukyu). In contrast, continuous upper-plate surface heat flow is predicted by either Similarity or Kriging for 6/13 segments and by both Similarity and Kriging for only 1/13 segments (Kamchatka Marianas). At least 5/13 segments show distinguishable Similarity and Kriging predictions in terms of upper-plate surface heat flow patterns, magnitude, or both (Figure \@ref(fig:centralAmericaUpper)).

(ref:centralAmericaUpperCap) Along strike variability of upper-plate surface heat flow near Central America. (a) Distances are computed orthogonally between the subduction zone segment boundary (bold black line) and interpolated upper-plate surface heat flow targets within eight 500 $km$-wide sectors (colored polygons). (b) The magnitude of average Similarity predictions (dashed curves) and Kriging predictions (solid curves) are distinguishable across all sectors with up to approximately 25 $mWm^{-2}$ of offset. (c) Both Similarity and Kriging predictions show discontinuous upper-plate surface heat flow with up to 25-30 $mWm^{-2}$ differences in average upper-plate surface heat flow among sectors. Segment boundary defined by @syracuse2006.
