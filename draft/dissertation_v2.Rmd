...
While ThermoGlobe currently contains `r format(nrow(hf), big.mark = ',')` observations, the regional coverage near subduction zones is uneven and irregularly-spaced (Table \@ref(tab:hfSummaryTable) & Figure \@ref(fig:globalhfComp)). Many methods may be considered to interpolate surface heat flow including inverse distance weighting, various splines and linear regressions, nearest-neighbor, general additive models, Kriging, and Similarity, to name a few. Generally speaking, these methods estimate surface heat flow across a grid of arbitrarily defined locations with unknown surface heat flow (the targets) by fitting mathematical models to available surface heat flow observations (the examples). The two interpolation methods compared in this study, Kriging and Similarity, are chosen because they represent end-member classes based on fundamentally different principles. Their comparative differences, therefore, may be important for advancing understanding of subduction zone processes, for targeting areas for future heat flow research, for improving either method in the context of subduction zone research, and for developing new layered interpolation methods that combine the strengths of Kriging and Similarity techniques.
...
(ref:domainConstructCap) Example of a interpolation domain constructed around Kyushu Ryukyu. Interpolation domains are defined by drawing a 1000 $km$-radius circular buffer (thin black line) around the segment boundary (bold black line). ThermoGlobe data are cropped within the buffered region by intersection (left). Note that Kyushu Ryukyu is one of the most densely sampled regions, yet still has considerable gaps. Similarity interpolation (right) fills in the gaps and presents a textured distribution by combining many underlying geologic proxy datasets. Data and interpolations from @lucazeau2019. Segment boundary defined by @syracuse2006.
...
(ref:centralAmericaDiffCap) Similarity vs. Kriging estimates for Central America. Arms of the Galápagos triple junction (GTJ), the East Pacific Rise (EPR) and Cocos Ridge (CR), are resolved by Similarity (topleft), but smoothed-out by Kriging (topright). Note the moderate positive differences between estimates within the Cocos Plate (CP) where Similarity estimates relatively high surface heat flow, but surface heat flow observations are low (bottomleft). Segment boundary (bold white line) as defined by @syracuse2006. White triangles are volcano locations also from @syracuse2006. ThermoGlobe data and Similarity estimates from @lucazeau2019.

```{r centralAmericaDiff, fig.cap='(ref:centralAmericaDiffCap)', fig.scap='Similarity vs. Kriging estimates for Central America'}

knitr::include_graphics('assets/figs/chpt3/CentralAmericaDiffComp.png')

```

(ref:kyushuRyukyuDiffCap) Similarity vs. Kriging estimates for Kyushyu Ryukyu. High observational density and spatial coverage near Kyushu Ryukyu produces interpolations with similar estimates and texture (top plots). The estimate and uncertainty differences (bottom plots) are thus small compared to other subduction zone segments. Segment boundary (bold white line) as defined by @syracuse2006. White triangles are volcano locations also from @syracuse2006. ThermoGlobe data and Similarity estimates from @lucazeau2019.

```{r kyushuRyukyuDiff, out.width='63%', fig.cap='(ref:kyushuRyukyuDiffCap)', fig.scap='Similarity vs. Kriging estimates for Kyushyu Ryukyu'}

knitr::include_graphics('assets/figs/chpt3/KyushuRyukyuDiffComp.png')

```

(ref:scotiaDiffCap) Similarity vs. Kriging estimates for Scotia. Similarity (topleft) resolves two tectonic features, the East Scotia Ridge (ESR) and a transform fault (TF) separating the Scotia and Sandwich Plates (SP, SAN) from the Antartic Plate (AP). Kriging (topright) is featureless because of incredibly sparse data. Few observations make Kriging a poor estimator of surface heat flow across the interpolation domain and provides no more information than computing the arithmetic mean of observations within the domain. Segment boundary (bold white line) as defined by @syracuse2006. White triangles are volcano locations also from @syracuse2006. ThermoGlobe data and Similarity prediction from @lucazeau2019.

```{r scotiaDiff, out.width='55%', fig.cap='(ref:scotiaDiffCap)', fig.scap='Similarity vs. Kriging estimates for Scotia'}

knitr::include_graphics('assets/figs/chpt3/ScotiaDiffComp.png')

```

(ref:vanuatuDiffCap) Similarity vs. Kriging estimates for Vanuatu. Similarity (topleft) resolves the spreading center separating the New Hebrides Plate (NHP) from the Balmoral Reef (BR) and Conway Reef (CR) microplates. Sufficient surface heat flow observations allow Kriging to resolve additional ridge segments and transform faults outlining BR and CR (topright). The difference between Similarity and Kriging estimates (bottomleft) are irregular and difficult to interpret. Segment boundary (bold white line) as defined by @syracuse2006. White triangles are volcano locations also from @syracuse2006. ThermoGlobe data and Similarity prediction from @lucazeau2019.

```{r vanuatuDiff, fig.cap='(ref:vanuatuDiffCap)', fig.scap='Similarity vs. Kriging estimates for Vanuatu'}
...
(ref:centralAmericaUpperCap) Along strike variability of upper-plate surface heat flow near Central America.

```{r centralAmericaUpper, fig.cap='(ref:centralAmericaUpperCap)', fig.scap='Along strike variability of upper-plate surface heat flow near Central America'}

knitr::include_graphics('assets/figs/chpt3/CentralAmericaUpperPlate.png')

```

(ref:kamchatkaMarianasUpperCap) Along strike variability of upper-plate surface heat flow near Kamchatka Marianas.

```{r kamchatkaMarianasUpper, fig.cap='(ref:kamchatkaMarianasUpperCap)', fig.scap='Along strike variability of upper-plate surface heat flow near Kamchatka Marianas'}

knitr::include_graphics('assets/figs/chpt3/KamchatkaMarianasUpperPlate.png')

```

(ref:kyushuRyukyuUpperCap) Along strike variability of upper-plate surface heat flow near Kyushu Ryukyu.

```{r kyushuRyukyuUpper, fig.cap='(ref:kyushuRyukyuUpperCap)', fig.scap='Along strike variability of upper-plate surface heat flow near Kyushu Ryukyu'}
...
The two interpolation approaches compared in this Chapter are fundamentally based on different underlying assumptions encapsulated by the First and Third Laws of Geography. Kriging is based on the First Law: *everything is related, but nearer things are more related* [@krige1951]. Similarity is based on the Third Law: *two points with similar geographic configurations should have similar values* [@zhu2018]. The comparisons in Section \@ref(chpt3Results) provide new context for researchers interested in using surface heat flow to investigate subduction geodynamics. While one approach is not evidently favorable in principle, Similarity produces more accurate interpolations in practice (at the scale and scope of active subduction zones), especially in cases with low observational density and irregularly-spacing.
...
kable(
  caption = 'nloptr minimization search',
  linesep = '',
  escape = F,
  booktabs = T,
  format = 'latex'
...
walk(
  c(
    ...
    cat('![Similarity vs. Kriging estimates for ', .x, '. Please refer to the main text for explanation of panels and colors.](', 'assets/figs/chpt3/', str_replace_all(.x, ' ', ''), 'DiffComp.png "Similarity vs. Kriging estimates for ', .x, '")', '\n\n', sep = '')
})
```
