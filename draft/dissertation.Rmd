...
(ref:hfSummaryPlotCap) Distribution of `ThermoGlobe` observations cropped within 1000 $km$ circular buffers around 13 active \gls{sz} segments. Heat flow distributions are centered between `r min(hf.summary$median)` and `r max(hf.summary$median)` $mWm^{-2}$ and highly skewed right. Skewness represents near-surface perturbations from geothermal systems and tectonic regions with high thermal activity. Diverse interactions between tectonic and near-surface processes are apparent from the broad, irregular \gls{shf} distributions. Density distributions are colored by quantiles (25%, 50%, 75%). Data from @lucazeau2019.
...
Optimized variogram models are given in Table \@ref(tab:vgrmSummaryTable) (see Appendix \@ref(krigeOpt) for details about the optimization procedure). Variogram ranges and sills vary substantially from segment to segment but do not correlate with cost (Figure \@ref(fig:vgrmSummaryPlot)). Despite a range of domain sizes, observation densities, and diverse tectonic environments, all variogram models converge on solutions with comparable cost (errors) within 15-20 iterations (Figure \@ref(fig:optTrace)). The variogram optimization methods are thus successfully generalizable across \gls{sz} segments without a need for case-by-case data manipulation or anisotropic variogram models.
...
Kriging cross-validation errors are within `r round(min(v.opt$cv.rmse)-7)` (`r v.opt$segment[which.min(v.opt$cv.rmse)]`) to `r round(max(v.opt$cv.rmse)-7)` (`r v.opt$segment[which.max(v.opt$cv.rmse)]`) $mWm^{-2}$ of the 7 $mWm^{-2}$ \gls{rmse} for @lucazeau2019's preferred Similarity model (7 proxy observables, Table \@ref(tab:vgrmSummaryTable)). Kriging errors can be relatively small compared to Similarity for domains with high observation density (e.g. `r v.opt$segment[which.min(v.opt$cv.rmse)]`; n = `r nrow(shp.hf.crop[[v.opt$segment[which.min(v.opt$cv.rmse)]]])`) but relatively large where observational density is comparatively low (e.g. `r v.opt$segment[which.max(v.opt$cv.rmse)]`; n = `r nrow(shp.hf.crop[[v.opt$segment[which.max(v.opt$cv.rmse)]]])`). Please note, however, that this comparison is between Kriging interpolations within 1000 $km$ buffers of \gls{sz} segments to global Similarity interpolations across all oceans and continents. It is unclear how Similarity errors will scale when the domain is restricted to the regional tectonic scale.
...
Summary statistics for interpolation *estimate* differences are given in Table \@ref(tab:diffSummaryTable) and Figure \@ref(fig:diffSummaryPlot). Summary statistics for interpolation *uncertainty* differences are given in Table \@ref(tab:sigmaDiffSummaryTable) and Figure \@ref(fig:sigmaDiffSummaryPlot). Note that target locations for Kriging estimates are defined across the same grid as Similarity estimates, so differences are computed point-by-point at the same locations. Differences between Similarity and Kriging estimates (Table \@ref(tab:diffSummaryTable), Figure \@ref(fig:diffSummaryPlot)) are broadly distributed. The median differences range from `r min(interp.diff.summary$median)` to `r max(interp.diff.summary$median)` $mWm^{-2}$ with inter quartile ranges from `r min(interp.diff.summary$IQR)` to `r max(interp.diff.summary$IQR)` $mWm^{-2}$. Similar to the distribution of heat flow in these areas, the minimum and maximum difference in predicted heat flow are extreme and represent the failure of one method to predict extreme outliers of the other.
...
### Interpolation uncertainty difference
...
```{r sigmaDiffSummaryPlot, fig.cap='Point-by-point differences of predicted heat flow between Similarity and Kriging interpolations (Similarity $-$ Krige). The differences for most subduction zone segments are median-centered at or near-zero with IQRs from `r min(interp.diff.summary$IQR)` to `r max(interp.diff.summary$IQR)` $mWm^{-2}$. Outliers (shadowy dots) extend to extreme positive and negative differences.', fig.scap='diffsummaryplot!!!'}
