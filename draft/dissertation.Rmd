...
>
> 2. Recovery rates of \gls{hp} rocks vary with \glspl{tkbc} (null: recovery rates are not different among \gls{sz})
...
The primary obstacle in addressing such questions is selecting from an unrestrained number of possible forward models by comparison to only a few, to a few dozens of, \gls{ptt} estimates from any particular exhumed terrane. This Chapter addresses the problem by realizing a large (`r format(nrow(fts.summary[fts.summary$rec,]), big.mark = ',')`) synthetic \gls{ptt} dataset from numerical simulations of subduction across a broad range of \gls{tkbc}. More than one million markers are traced from 64 numerical experiments representing the range of presently active oceanic-continental margins [from Chapter \@ref(chpt2), @syracuse2006; @wada2009]. Traced markers are classified as recovered, or lost, according to characteristics of their \gls{ptt} paths using a soft clustering classification algorithm. \gls{pt} distributions of markers are sensitive to convergence rate and oceanic-plate age, but insensitive to \glsfirst{upt} (contrary to expectations from Chapter \@ref(chpt2)). Further, recovery rates are moderately dependent on initial \gls{tkbc}, characterized by the thermal parameter $\Phi$. Four distinct geodynamic regimes are identified and their implications are discussed for interpreting aggregate metamorphic \gls{pt} datasets like `pd15`.
