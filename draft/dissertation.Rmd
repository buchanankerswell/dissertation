...
> **Keypoints:**
>
> - Geodynamics is fundamentally a data-starved field
>
> - Computational approaches allow inquiry despite sparse datasets
>
> - \glsfirst{sz} simulations, \glsfirst{shf}, and exhumed \glsfirst{hp} rocks are combined to better understand the diversity of deep \gls{sz} geodynamics

As pointed out by @gerya2014, a scarcity of observational constraints through time and space makes the study of geodynamics on Earth extraordinarily challenging (Figure \@ref(fig:sparse)). Luckily, application of various computational approaches---simulation, interpolation, and applied statistics---enable geodynamic inquiry despite sparse datasets. This dissertation leverages computation to evaluate the geodynamic diversity of one of the most important tectonic processes Earth, *subduction*.

(ref:sparseCap) The Geodynamicist's dilemma. Time-depth diagram representing the data availability on Earth. The rock record (red circles) encodes information about geodynamic processes throughout Earth's history, but only within the $\sim$ 100 $km$ of Earth's surface. Geophysical data (blue circles) provides images of Earth's deep interior, but only since the 20th century CE (or 10$^-7$ $Ga$). Direct observations (green circle) are limited to the present-day surface. Size of the circles represents the abundance of available data. Reprinted from @gerya2014 with permission.
...
Subduction occurs when two lithospheric plates converge and the denser plate *subducts* beneath the other. \Glsfirst{sz} are implicated in myriad geodynamic phenomena, including seismicity, metamorphism, volatile flux, volcanism, plate motions, and crustal deformation [@cizkova2013; @gonzalez2016; @hirauchi2010; @peacock1990; @peacock1991; @peacock1993; @peacock1996; @peacock1999a; @hacker2003; @vankeken2011; @grove2012; @gao2017]. These phenomena are largely defined by plate motions and mechanical behaviour along the plate interface [@furukawa1993; @peacock1994; @peacock1996; @syracuse2010]. Despite a diverse set of \glsfirst{tkbc} governing presently active \glspl{sz} (plate thicknesses, convergence velocity, subduction angle) and a range of seismologic, geophysical, and volcanic observations, previous work suggests the mechanical behaviour between plates may be rather uniform among \glspl{sz} at the 100's of $m$ to $km$ scale [@furukawa1994; @wada2008; @wada2009].

This dissertation is motivated by the following question. How diverse is inter-plate mechanical behaviour across the range of presently active \glspl{sz}? Each chapter focuses on quantifying an aspect of inter-plate mechanical behaviour using a different computational approach and dataset.

Chapter 2 directly simulates oceanic-continental subduction for 64 models with varying oceanic-plate age, convergence velocity, and upper-plate thickness. Mechanical \glspl{cd} are observed and multivariate linear regression is used to formulate an expression for estimating \glspl{cd} in \glspl{sz}. The expression requires inverting \glsfirst{shf} to estimate upper-plate thickness. Average upper-plate \gls{shf} for 13 presently active \glspl{sz} yield a narrow distribution of \glspl{cd}, supporting previous work and the idea of broadly uniform inter-plate mechanical behaviour among \glspl{sz}.

Chapter 3 takes a closer look at \gls{cd} by quantifying the spatial variability of \gls{shf} near presently active \glspl{sz} in 2D. Kriging is applied to a large global \gls{shf} dataset and compared to another interpolation approach based on different principles. One approach is favoured over the other, yet both show diverse spatial patterns of \gls{shf} in contradiction of globally similar \gls{cd} predicted in Chapter 2. Dataset scaling and new improvements to interpolation methods promise better tools for evaluating geodynamic variability among \glspl{sz}.

Finally, Chapter 4 applies machine learning techniques to recognize detachment (recovery) of subducting markers, representing rock fragments, from the numerical simulations in Chapter 2. A large (`r format(nrow(fts.summary[fts.summary$rec,]), big.mark = ',')`) dataset of marker \glsfirst{ptt} is compared across numerical experiments and to globally aggregated \gls{pt} estimates of \glsfirst{hp} metamorphic terranes (the rock record). The distribution of marker recovery along the plate interface and recovery rates both correlate with \gls{tkbc}, suggesting inter-plate mechanical behaviour varies among \glspl{sz}. Marker \gls{ptt} distributions are distinct from the rock record, likely pointing to inherent sampling bias introduced by globally aggregating undifferentiated \gls{hp} rocks into a single composite dataset.
...
\markboth{Chapter 4: Computing Rock Recovery}{Chapter 4: Computing Rock Recovery}
...
\markboth{Chapter 5: Summary}{Chapter 5: Summary}
