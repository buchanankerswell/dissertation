...
### Global interpolation differences
...
Two point-by-point comparisons between Similarity and Kriging methods are presented in this study---*estimates* and *uncertainties*. Summary statistics for *estimate* differences are given in Table \@ref(tab:diffSummaryTable). Summary statistics for *uncertainty* differences are given in Appendix \@ref(interpSigmaDiff).
...
Kyushyu Ryukyu has a comparable observation count (n = `r format(nrow(shp.hf.crop[['Kyushu Ryukyu']]), big.mark = ',')`) to Central America (n = `r format(nrow(shp.hf.crop[['Central America']]), big.mark = ',')`), but a simpler tectonic configuration (no spreading centers). Estimate and uncertainty differences between interpolations are small compared to other \gls{sz} segments. Kriging also resolves much of the texture in the Similarity interpolation because observation density and spatial coverage is relatively high (Figure \@ref(fig:kyushuRyukyuDiff)). The interpolation methods evidently converge when observation density and spatial coverage is adequate, although it is nontrivial to suggest minimum thresholds for using simple Kriging methods over Similarity.

(ref:kyushuRyukyuDiffCap) Similarity vs. Kriging estimates and uncertainties for Kyushyu Ryukyu. High observation density and spatial coverage near Kyushu Ryukyu produces interpolations with similar estimates and texture (top plots). The estimate and uncertainty differences (bottom plots) are thus small compared to other \gls{sz} segments. Note that computed differences off the scale in the bottom plots are colored grey. Bold and thin white lines represent the subduction zone segment boundary and plate depth, respectively, as defined by @syracuse2006. `ThermoGlobe` data and Similarity estimates from @lucazeau2019.
...
### Interpolation uncertainty difference {#interpSigmaDiff}
