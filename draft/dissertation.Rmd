...
> **Keypoints:**
>
> - Unsupervised classification recognizes marker recovery in 64 \gls{sz} simulations
>
> - Marker \gls{pt} distributions are distinct from the rock record
>
> - Marker recovery rates are non-uniform across diverse \glspl{sz} simulations

## Abstract {#chpt4Abstract}

\Glsfirst{hp} metamorphic rocks that are detached (recovered) from subducting oceanic-plates and exhumed to the Earth's surface become invaluable records of \glsfirst{pts} fields deep in \glspl{sz}. For example, comparing metamorphic \glsfirst{ptt} estimates from suites of \gls{hp} rocks allows for evaluation of \gls{pt} gradients along the plate interface. However, some incongruent interpretations concerning \gls{pt} gradients, recovery rates, and recovery depths arise when comparing globally aggregated \gls{ptt} estimates with \gls{pt} gradients from numerical geodynamic models. This study applies unsupervised classification to recognize marker recovery in numerical simulations of oceanic-continental convergence across a range of initial \glsfirst{tkbc} representing presently active \glspl{sz}. Over one-million markers traced from 64 simulated \glspl{sz} are classified as recovered, or lost, by implementing a soft clustering classification algorithm. Marker \gls{ptt} distributions are compared across models and with the rock record to address the following questions. How do recovery rates vary among \glspl{sz}? How is recovery distributed along the plate interface, and how does this vary among \glspl{sz}? Marker \gls{ptt} distributions and recovery rates show sensitivity to oceanic-plate age, convergence velocity, and relative detachment timing, but not to upper-plate lithospheric thickness. Marker \gls{ptt} distributions are also distinct from the rock record. Four geodynamic regimes with different recovery modes are recognized and discussed with implications for interpreting the rock record. This study highlights the immediate usefulness of augmenting observational constraints with large (`r format(nrow(fts.summary[fts.summary$rec,]), big.mark = ',')`) simulated datasets.
...
Markers compiled from all numerical experiments show distinct patterns of probability density and absolute \gls{pt} compared to `pd15` (Figure \@ref(fig:meta)). Marker density is asymmetric and bimodal with a high-probability peak in the lower-blueschist to upper-greenschist facies and a modest probability hill in the eclogite facies. `pd15` density, in contrast, is slightly shifted towards higher \gls{pt} gradients overall and is more uniformly distributed along a high-probability ridgeline spanning across upper-greenschist to lower-eclogite facies. \gls{hp} rocks recovered from 0.8 $GPa$ to 2.4 $GPa$ are especially overrepresented in `pd15` compared to markers compiled from all numerical experiments.

(ref:metaCap) Comparing marker PT distribution with the rock record. (a) The probability density of `r format(round(sum(class.summary$n)), big.mark = ',')` recovered markers from all 64 \gls{sz} simulations (filled grey contours) are bimodally distributed with a high-probability peak at lower-blueschist to upper-greenschist facies and a modest probability hill in the eclogite facies. Recovered markers neither completely overlap with the rock record (`pd15`; light grey density contours) in absolute PT distribution nor in terms of probability density. For example, summing the probability density along the P dimension (b) shows blueschists being especially overrepresented in the rock record (`pd15`; grey line) between 0.8 $GPa$ and 2.4 $GPa$ compared to recovered markers (black line). Eclogites are also overrepresented in the rock record to a lesser degree.
...
Figure \@ref(fig:detachDensity) shows marker \gls{pt} distributions at 10 percent increments of total subduction duration. Marker recovery exhibits three trends with respect to time. First, significantly fewer markers are recovered after approximately 40% of total subduction duration. Second, average \gls{pt} gradients rotate anti-clockwise from 10-15 $K/km$ to 5 $K/km$ after approximately 50% of total subduction duration. Finally, markers are recovered from increasingly higher pressures beginning from 0.2-1.3 $GPa$ to 3.5 $GPa$ by 40% of total subduction duration. Secular trends in marker recovery show significant greenschist to amphibolite facies recovery during the first 30-40% of the subduction life-cycle, followed by increasingly fewer recovered rocks overall with a higher proportion of blueschist and eclogites.Absolute \gls{pt} trends are most consistent with `pd15` during the first third of the subduction life-cycle, but marker density is most consistent with `pd15` during the final half of the subduction life-cycle.
...
Two notable patterns emerge when comparing recovery rates among all 64 subduction simulations (Figure \@ref(fig:recRate)). First, simulations with younger, thinner, and more slowly converging oceanic-plates (low-$\Phi$) have higher recovery rates than older, thicker, more quickly converging oceanic-plates (high-$\Phi$). Note the greyscale change across rows in Figure \@ref(fig:recRate). This negative association between $\Phi$ and recovery rate appears gradual and continuous. In contrast, a lack of association is apparent between \gls{upt} and recovery rate (note greyscale change down columns in Figure \@ref(fig:recRate)).

On average `r round(mean(class.summary$est.ratio, 1)*100)` percent of markers get recovered from subduction (Table \@ref(tab:recSummary)). While volumes of rock cannot be directly computed from recovery rates because markers are mathematical point features, volumes may be roughly estimated by assuming markers represent the entire 760 $\times$ 8 $km$ section of oceanic crust and seafloor sediments they were sampled from. Volumes of subducted material then range from `r round(min((1-class.summary$est.ratio)*760*8))` to `r round(max((1-class.summary$est.ratio)*760*8))` $km^3/km$, with a mean of `r round(mean((1-class.summary$est.ratio)*760*8))` $km^3/km$. Volumes of recovered material then range from `r round(min(class.summary$est.ratio*760*8))` to `r round(max(class.summary$est.ratio*760*8))` $km^3/km$, with a mean of `r round(mean(class.summary$est.ratio*760*8))` $km^3/km$.
