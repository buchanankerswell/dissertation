...
hf <- readr::read_csv('assets/r/chpt3/tglobe.csv', col_type = readr::cols())
dup <- hf %>% 
...
list.files('assets/r/chpt3/diff', pattern = '.RData', full.names = T) -> fpath
purrr::map_chr(
  list.files('assets/r/chpt3/diff', pattern = '.RData'),
  ~.x %>%
...
These methods are called *Similarity* methods (Figure \@ref(fig:globalComp)) and follow assumptions embedded in the Third Law of Geography: *the more similar the geographic configuration of two points, the more similar their values* [hereafter referred to as the Third Law, @zhu2018]. Global scale problems are particularly suited for this approach as it does not rely on sampling density. On the other hand, assumed interactions among many independent geologic proxies, each with its own assumptions and uncertainties, makes Similarity methods complex combinatorial problems vulnerable to decisions about the number of proxies, proxy dataset quality, and simplified geologic models used to compute observable classes [see @goutorbe2011 for method details].

(ref:globalCompCap) Global \gls{shf} observations (top) and similarity interpolation (bottom) cropped within 1000 $km$ buffers of 13 \gls{sz} segments. Note that \gls{shf} above 250 $mWm^{-2}$ is colored yellow-white. ThermoGlobe data from @lucazeau2019 & @jennings2021 and \gls{sz} boundaries as defined by @syracuse2006.

```{r globalComp, fig.cap='(ref:globalCompCap)', fig.scap='Global heat flow and \\gls{sz} boundaries'}

knitr::include_graphics('assets/figs/chpt3/ThermoGlobeBufferComp.png')

```

In contrast to the Third Law, there exists some degree of spatial dependence, or continuity, in the distribution of \gls{shf}. For example, a pair of \gls{shf} observations taken one meter apart will be strongly correlated, but the correlation will likely decrease with increasing distance between the points [@goovaerts1997]. This is encapsulated in the First Law of Geography (hereafter referred to as the First Law): *everything is related, but nearer things are more related* [@krige1951; @matheron1963]. Generally speaking, the spatial (dis)continuity of \gls{shf} reflects the continuity of subsurface geodynamics plus near-surface perturbations. Broad low \gls{shf} regions on continents outline cratons (Figure \@ref(fig:tglobeComp)), for example, and uniform \gls{shf} near \glspl{sz} are interpreted to reflect uniform lithospheric thickness [@currie2004; @currie2006; @hyndman2005] and mechanical coupling depths [@furukawa1993; @wada2009; @kerswell2020].
...
This study applies ordinary Kriging to the ThermoGlobe database [@lucazeau2019; @jennings2021] sampled within 1000 $km$ buffers around 13 active \glspl{sz} defined by @syracuse2006. Kriging interpolations are then directly compared (point-to-point) to Similarity interpolations from @lucazeau2019. Kriging and Similarity methods are generally comparable for most subduction segments, but uncertain at the \gls{sz} segment scale. The results highlight challenges in inferring \gls{sz} geodynamics from \gls{shf} datasets without significant assumptions. 

## Methods

### The ThermoGlobe database

The [ThermoGlobe database](http://heatflow.org) is available from the supplementary material of @lucazeau2019 and is accessible online at \url{http://heatflow.org} [@jennings2021]. It currently contains `r format(nrow(hf),big.mark=',')` data points, their locations in latitude/longitude, and important metadata---including a data quality rank (`Code 6`) from A to D. The reader is referred to @lucazeau2019 and \url{http://heatflow.org} for details on compilation, references, and historical perspective on ThermoGlobe and previous compilations. We use ThermoGlobe because it is the most recent database available, has been carefully compiled, and is open-access despite containing much proprietary data.

Like @lucazeau2019, we exclude 4,790 poor quality observations (`Code 6` = D) from our analysis. We further remove 350 data points without heat flow observations and two without geographic information. Note that `r format(sum(d.hf$code6=='Z'),big.mark=',')` observations have an undetermined quality (`Code 6` = Z). Multiple observations at the same location are parsed to avoid singular covariance matrices during Kriging:
...
where $N(h)$ is the number of pairs of points, $Z(u_i)$ and $Z(u_j)$, separated by a lag distance, $h$, $\delta$ and $n=15$ are used to define a binwidth for evaluating $\hat{\gamma}(h)$ with irregularly spaced data. In simple terms, Equation \@ref(eq:variogram) represents the similarity, or dissimilarity, between pairs of observations in space. Equation \@ref(eq:variogram) adheres to the First Law of Geography and is derived from the theory of *regionalized variables* [@matheron1963; @matheron2019], which formally defines a probabilistic framework for spatial interpolation of natural phenomena.
...
ThermoGlobe data and similarity interpolations from @lucazeau2019 are transformed onto a Pacific-centered Robinson \gls{crs} defined using the `proj` [@proj2021] string "`+proj=robin +lon_0=-155 +lon_wrap=-155 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs`". All geographic operations, including Kriging and comparisons with similarity interpolations, are performed in the above \gls{crs} using the general-purpose functions in the `R` package `sf` [@pebesma2018]. The Kriging domain is defined near individual arc segments in two steps: 1) 1000 $km$ buffers are drawn around the arc segments as defined by @syracuse2006. 2) The bounding box of the 1000 $km$ buffer is expanded by 10% on all sides (Figure \@ref(fig:segments)). Kriging interpolations are estimated on the same grid as @lucazeau2019 so differences can be taken point-by-point at the exact same locations.

(ref:segmentsCap) Constructing interpolation domains around \gls{sz} segments. (a) Bounding boxes are drawn around thirteen \gls{sz} segments by (b) drawing a 1000 $km$ buffer (lightest blue) around each segment and expanding the buffer's bounding box (medium blue) by 10% on all sides (darkest blue). (c) ThermoGlobe data is cropped within the largest rectangle. Data from @syracuse2006 and @lucazeau2019.


```{r segments, fig.cap='(ref:segmentsCap)', fig.scap=''}

knitr::include_graphics('assets/figs/chpt3/segs_comp.png')
...
hf.summary %>%
kable(
  caption = 'ThermoGlobe summary by segment',
...
footnote(
  general_title = '',
  general = c('\\\\uline{\\\\textit{note}}: All units are in $mWm^{-2}$'),
  threeparttable = T,
  escape = F
)
```
...
No apparent correlation exists between variogram model range and subduction zone segment length, number of heat flow observations, nor domain area (Figure \@ref(fig:variogramSummaryPlot)). Most subduction zone segments show spatial dependence of a few hundred kilometers or less, irrespective of the number of observations or segment size. The exceptions are Kyushu Ryukyu (range = `r variogram.summary[which(variogram.summary$Segment == 'Kyushu Ryukyu'),4]` $km$) and Vanuatu (range = `r variogram.summary[which(variogram.summary$Segment == 'Vanuatu'),4]` $km$), whose model variogram ranges are, perhaps, anomalously high.

```{r variogramSummaryTable}
variogram.summary %>%
mutate(
  'Range' = round(Range),
  'Length' = round(Length),
...
knitr::kable(
  caption = 'Kriging summary by segment',
  escape = F,
  booktabs = T,
  format = 'latex') %>%
add_header_above(c('', 'Variogram' = 3, '', 'Domain' = 4), align = 'c') %>%
kable_styling(latex_options = c('striped')) %>%
kable_classic() %>%
footnote(
  general_title = '',
  general = c('\\\\uline{\\\\textit{key}}: Sill $[mWm^{-2}]$, Range $[km]$, RMSE: cross-validation root mean square error $[mWm^{-2}]$, Length $[km]$, Area $[Mkm^2]$, n $[\\\\#~obs]$, Density $[n/Mkm^2]$'),
  threeparttable = T,
  escape = F
)
```
...
hf.diff.summary %>%
knitr::kable(
  caption = 'Interpolation differences summary',
  col.names = c('Segment', 'n', 'Min', 'Max', 'Median', 'IQR', 'Mean', '$\\sigma$'),
  escape = F,
  booktabs = T,
  format = 'latex') %>%
...
footnote(
  general_title = '',
  general = c('\\\\uline{\\\\textit{note}}: Difference is calculated as Similarity - Krige', '\\\\uline{\\\\textit{key}}: All units are in $mWm^{-2}$'),
  threeparttable = T,
  escape = F
)
```
