...
v.opt <-
  ...
  group_by(segment) %>%
  slice_min(cost) %>%
  select(-c(itr, vgrm.wt, vgrm.rmse, vgrm.cost, cv.wt, cv.cost, n.obs.sim))
interp.diff <-
  interp.diff.summary %>%
  group_by(segment) %>%
  filter(v.mod != 'Gau') %>%
  ...
  ungroup() %>%
  mutate(
    rmse.sim = round(rmse.luca$rmse.sim, 1),
...
footnote(
  general_title = '',
  general = c('\\\\uline{\\\\textit{note}}: Showing optimal Krige models from Table \\\\ref{tab:vgrmSummaryTable}, all units are $mWm^{-2}$'),
  threeparttable = T,
  escape = F
)
```
