...
> 5. Check for cluster centroids within the bounds ($\geq$ 3 $^\circ C/km$ AND $\leq$ 1300 $^\circ C$ AND $\leq$ 120 $km$)
> 6. Classify marker clusters found in step 5 as recovered
> 7. Classify all other markers as subducted
>
> $^*$ Maximum marker \glspl{pt} used for clustering are assessed before markers transform (dehydrate or melt) and before the accretionary wedge toe collides with the high-viscosity convergence region positioned at 500 $km$ from the left boundary (to avoid spurious maximum \glspl{pt} from sudden isothermal burial, see Appendix \@ref(vis) for examples)

\noindent Note that "hard" classification is possible by directly applying simple rules to markers without clustering [e.g. @roda2010]. However, "hard" methods are less generalizable than "soft" approaches like Gaussian mixture models, which can be implemented to study many possible features in numerical simulations with Lagrangian reference frames---not just recovery of subducted material. Figures \@ref(fig:class) & \@ref(fig:bigComp) show an illustrative example of marker classification for 1 of 64 numerical experiments. All other experiments are presented in Appendix \@ref(vis).
...
To better understand how rock recovery can vary among subduction zones with different boundary conditions, important modes of marker recovery are determined by finding peaks in marker density with respect to \gls{pt} and \gls{pt} gradients (Table \@ref(tab:recSummary) & Figure \@ref(fig:corrComp)). The tallest density peak (`mode1`) gives a sense of where the greatest abundance of markers are being recovered from. The deepest, or warmest, density peak (`mode2`) gives a sense of how the most deeply subducted markers (or markers with the highest gradients) vary across different numerical experiments. The overall portrait of changes in rock recovery across different subduction systems can be drawn from many (non)correlations among important recovery modes in \gls{pt} space and thermo-kinematic boundary conditions. This section distills that information by considering the major and minor effects of each boundary conditions under investigation.
...
Oceanic-plate age exhibits a strong effect on the average \gls{pt} gradients of recovered markers. Both major \gls{pt} gradient modes show a negative correlation with oceanic-plate age, indicating an increase from about 6.5 $^\circ C/km$ for old plates ($\geq$ 85 $Ma$) to about 8.5 $^\circ C/km$ for younger plates ($\leq$ 55 $Ma$). While oceanic-plate age does not correlate with the dominant temperature mode (`mode 1`), markers recovered from temperatures above about 350 $^\circ C$ (`mode 2`) show a small, but significant, inverse correlation---indicating oceanic-plate age affects the absolute temperature of the deeper plate interface (beyond the continental Moho). Notably, oceanic-plate age only slightly affects the dominant pressure mode (`mode 1`), showing a small positive correlation that indicates a slightly higher chance of recovery from beyond the continental Moho for the oldest oceanic-plates ($\geq$ 85 $Ma$). In other words, oceanic-plate age primarily affects the average \gls{pt} trajectory of recovered material, but does not significantly shift marker recovery along the plate interface. Finally, oceanic-plate age is not correlated with recovery rate. Generally speaking, simulations with the youngest oceanic-plate ages ($\leq$ 55 $Ma$) are most consistent with the rock record.

#### Convergence Velocity Effect

Convergence velocity exhibits the strongest effect on the depth of marker recovery along the plate interface. Markers from the slowest-converging simulations (40 $km/Ma$) are more likely to be recovered from depths below the continental Moho than simulations with faster convergence. For example, the major pressure mode (`mode 1`), which indicates where the most markers are recovered from with respect to pressure, does not change significantly until velocity drops below 66 $km/Ma$. However, the inverse correlation between convergence velocity and depth of recovery is most apparent from markers that are recovered beyond the continental Moho (the deepest and warmest pressure and temperature modes: `mode 2`). While changing convergence velocity shifts marker recovery up and down the plate interface, the major \gls{pt} gradient modes are relatively unaffected and do not correlate with convergence velocity. In other words, decreasing convergence velocity has the effect of shifting marker recovery deeper down the plate interface without changing the average \gls{pt} trajectory (thermal gradient) of subducted material. Finally, convergence velocity is not correlated with recovery rate. Generally speaking, simulations with the slowest convergence velocities ($<$ 60 $km/Ma$) are most consistent with the rock record.

#### Upper-plate Thickness Effect

Chapter \@ref(chpt2) demonstrates an association between upper-plate thickness and coupling depth in the same numerical experiments that markers are sampled from [@kerswell2020]. \gls{pt} distributions of markers are therefore expected to respond strongly to changes in upper-plate thickness, especially with respect to pressure (depth). However, a surprisingly negligible effect is observed. For example, upper-plate thickness does not correlate with either of the important pressure modes, nor does it correlate with the temperature mode for the warmest (usually the most deeply subducted) markers (`mode 2`), indicating an insignificant effect on the location of recovery along the plate interface. In contrast, upper-plate thickness exhibits a strong effect on the average \gls{pt} gradients of recovered markers and is the only thermo-kinematic boundary condition that significantly correlates with the major temperature mode (`mode 1`) and recovery rates. In other words, markers recovered at shallow depths beneath thin upper-plates are more likely to show warmer gradients and absolute temperatures. On the other hand, markers are recovered at significantly higher rates underneath thick upper-plates (above 10 % for 94 $km$-thick plates) than underneath thin upper-plates (around 7.5 % for $\leq$ 62 $km$-thick plates).


\begin{landscape}


(ref:corrCompCap) Correlations among important modes of marker recovery and thermo-kinematic boundary conditions. The dominant recovery mode (\texttt{mode 1}) indicates the position of the tallest density peak in \gls{pt} space (i.e. where most markers are recovered from). The deepest or warmest density peak (\texttt{mode 2}) indicates where the majority of deeply subducted markers are recovered from. Oceanic plate age and upper-plate thickness strongly affect the \gls{pt} trajectory of recovered markers (strong correlation with gradient modes), while convergence velocity shifts marker recovery up and down the plate interface (strong correlation with with P and T modes). Notably, upper-plate thickness is the only thermo-kinematic boundary condition correlated with recovery rates and the dominant T mode. Dark grey regions indicate the range of recovery modes inferred from the rock record (\texttt{pd15} and \texttt{ag18}). Symbols indicate the Spearman's correlation coeffecient that measures the significance of non-linear (monotonically-increasing) correlations. *** $\rho \leq$ 0.001, ** $\rho \leq$ 0.01, * $\rho \leq$ 0.05, - $\rho \geq$ 0.05. Boxplots with darker fills are highly significant (***), wheras boxplots with transparent fills are less significant (**, *) or insignificant (-).
...
### Recovery Modes and Rates

#### Constraints from Geophysical Observations

The locations of important recovery modes implied by numerical experiments in this study are generally consistent with the depths of important mechanical transitions inferred from seismic imaging and surface heat flow observations. For example, the dominant recovery mode common among all numerical experiments (`mode 1` at about 1 $GPa$ and 350 $^\circ C$, Table \@ref(tab:recSummary) & Figure \@ref(fig:marxComp)) are consistent with seismic velocity contrasts, thickening of a ubiquitous low-velocity layer, and locations of low-frequency and slow-slip seismic events near the base of the continental Moho [between 20-50 $km$ depth, @audet2016; @audet2018; @bostock2013; @calvert2011; @delph2021; @tewksbury2021b]. While considerable unknowns persist about the nature of activity in this region [@tewksbury2021b], it is often interpreted as a locus of fluid release, transitional brittle-ductile seismicity, and underplating of sediments and/or formation of a tectonic mélange.

Relatively uniform geophysical properties and seismic quiescence then extend beyond the shallow region of activity until the low-velocity layer terminates [@abers2020; @rondenay2008]---coinciding with the deeper recovery mode implied by numerical experiments (`mode 2` at about 2.5 $GPa$ and 625 $^\circ C$). This second transition is often interpreted as pervasive eclogitization of the oceanic crust and onset of mechanical plate coupling near 80 $km$ [@hacker2008; @vankeken2011]. The established locations of these transitions, as well as the gap separating them, are consistent with the most reliable recovery modes observed in numerical experiments (Figure \@ref(fig:marxComp)). While numerical experiments indicate considerable variance in the locations of recovery modes across individual simulations, the variance in marker recovery modes is on the same order as the variance in the locations of mechanical transition near the continental Moho [20-50 $km$, @abers2020], and plate coupling depths inferred from surface heat flow [70-80 $km$, Chapter \@ref(chpt2)\; @furukawa1993; @wada2009; @maunder2019; @kerswell2020].

#### Constraints from Exhumed Plate Interfaces

The overall picture drawn from \gls{pt} estimates of natural samples is consistent in terms of recovery modes, but inconsistent in terms of their locations in \gls{pt} space. This is especially apparent when comparing temperatures and thermal gradients between markers and natural samples. [@behr2018; @tewksbury2021a; @tewksbury2021b]
...
(ref:youngSlowCap) Combined distribution of recovered markers from all 4 simulations with the youngest oceanic-plates and slowest convergence velocities. (a) Most markers (black point cloud and blue Tanaka density contours) are recovered from two important modes: near the continental Moho around 1 $GPa$ and 350 $^\circ C$ and near the onset of plate coupling (antigorite dehydration depth) around 2.5 $GPa$ and 625 $^\circ C$, with some dispersion along the plate interface from 1-2 $GPa$. Note the gap in recovery around 2 $GPa$ and 550 $^\circ C$ that coincides with the highest-density of natural samples. (b) A broad single-step \gls{cdf} in P indicates high recovery rates between 0.8-1.5 $GPa$ (`r round((cdfP.summary$prob[which.min(abs(cdfP.summary$maxP/1e4-1.5))]*100)-(cdfP.summary$prob[which.min(abs(cdfP.summary$maxP/0.8e4-1))]*100))`%) compared to `pd15` and `ag18` (`r round(abs(pd15.P$prob[which.min(abs(pd15.P$P-0.8))]-pd15.P$prob[which.min(abs(pd15.P$P-1.5))])*100)`-`r round(abs(ag18.P$prob[which.min(abs(ag18.P$P-0.8))]-ag18.P$prob[which.min(abs(ag18.P$P-1.5))])*100)`%). (c) Another broad single-step \gls{cdf} in T indicates high recovery rates from $\leq$ 500 $^\circ C$ (`r round(cdfT.summary$prob[which.min(abs(cdfT.summary$maxT-273-500))]*100)`%) compared to natural samples (`r round(pd15.T$prob[which.min(abs(pd15.T$T-500))]*100)`-`r round(ag18.T$prob[which.min(abs(ag18.T$T-500))]*100)`%). (d) Virtually all markers `r round(cdfGrad.summary$prob[which.min(abs(cdfGrad.summary$maxGrad*1e3-15))]*100, 2)`% are recovered from $\leq$ 15 $^\circ C/km$) compared to `r round(ag18.grad$prob[which.min(abs(ag18.grad$grad-15))]*100)`-`r round(pd15.grad$prob[which.min(abs(pd15.grad$grad-15))]*100)`% for natural samples. Explanation of elements in (a-d) is the same as Figure \@ref(fig:rockPTComp).
