...
The magnitude of heat fluxing through the Earth's surface is directly related to the thermal state of Earth's lithosphere. Surface heat flow observations from continents and seafloors are thus critically important for understanding lithospheric thermal structure and thermal processing (e.g. by fluids) within subduction zones. However, evaluating regional surface heat flow patterns across large tectonic features with sparse, irregularly-spaced observations requires careful interpolation. This study compares two interpolation methods based on fundamentally different principles, *Kriging* and *Similarity*, to investigate spatial variability of surface heat flow near 13 presently active subduction zones. Kriging and Similarity predictions show a Kaleidoscope of surface heat flow distributions and profiles among subduction zone segments. Mixed profiles imply various degrees of (dis)continuity exist among subduction zone segments in terms of their (1) lithospheric thermal structure, (2) heat-transferring dynamics, and/or (3) observational density relative to the local spatial variability of surface heat flow. Comparing independent interpolation methods can therefore identify anomalies and methodological strengths. In principle, carefully layering Similarity and Kriging methods may combine their properties to produce more accurate regional interpolations.
...
This study compares regional Similarity and Kriging interpolations near 13 presently active subduction zones while considering the following questions: (1) how does surface heat flow continuously vary near subduction zones, especially within the upper-plate? (2) How do Kriging and Similarity predictions compare? (3) What do the differences (if any) imply about geodynamic variability among active subduction zones, especially with respect to uniform upper-plate thickness discussed in Chapter \@ref(chpt2)? First, ordinary Kriging is applied to ThermoGlobe data near 13 presently active subduction zone segments [defined by @syracuse2006]. Kriging predictions are then directly compared (point-by-point) to Similarity predictions from a previous global-scale study by @lucazeau2019. Interpolation comparisons yield a Kaleidoscope of predicted upper-plate surface heat flow distributions and profiles. The implications of mixed profiles are discussed, especially with respect to uniform upper-plate thickness discussed in Chapter \@ref(chpt2).
...
The Scotia segment illustrates a case where surface heat flow observations are incredibly sparse. Similarity is agnostic to observational density, so it predicts multiple tectonic features including the East Scotia Ridge and the WSW-ENE trending transform boundary separating the Scotia and Sandwich Plates from the Antarctic Plate (Figure \@ref(fig:scotiaDiff)). Similarity predictions distal from the plate boundaries are ostensibly dependent on oceanic plate age and other small differences in the set of geologic proxies. Kriging, on the other hand, shows a heat flow anomaly more or less in the region of the East Scotia Ridge, but does not resolve any structure in a way that is geologically useful. Few surface heat flow observations (n = `r nrow(shp.hf.crop[['Scotia']])`) result in smooth Kriging predictions that approximate the expected value (mean: `r hf.summary$mean[hf.summary$segment == 'Scotia']` $mWm^{-2}$) for most of the domain according to Equation \@ref(eq:linEstimate). 
...
Similarity and Kriging interpolations are easily distinguishable by eye at the regional scale when compared side-by-side (e.g. compare Figures \@ref(fig:centralAmericaDiff), \@ref(fig:kyushuRyukyuDiff), and \@ref(fig:scotiaDiff) with the remaining segments in Appendices \@ref(interpDiffAppendix) & \@ref(lateralDiffAppendix)). Similarity and Kriging interpolations are quickly discernible because their contrasting mathematical frameworks produce characteristic interpolations. However, the unique characteristics of Similarity and Kriging methods can be independently leveraged as strengths under certain circumstances.

For example, highly irregular observational density characterizes global surface heat flow observations [@lucazeau2019] and is common across all 13 subduction zone segments (Tables \@ref(tab:hfSummaryTable) & \@ref(tab:sectorSummaryTable))). However, because Similarity is inherently agnostic to the spatial configuration of observations [@goutorbe2011], accurate interpolations with well-defined plate boundaries are still possible for regions with relatively few observations (e.g. Scotia and New Britain Solomon, Figures \@ref(fig:scotiaDiff) & \@ref(fig:newBritainSolomonDiff)). In other words, Similarity is applied in the exact same manner across an area with 5 observations as an area with 5000 observations. Since surface heat flow observations near subduction zone segments are sparse and irregularly spaced, maintaining spatial-independence from observations is a desirable property during the interpolation process.

On the other hand, conserving "ground-truth" is an equally desirable property to maintain during the interpolation process. Local ordinary Kriging conserves ground-truth by remaining agnostic to all other factors but the spatial configuration of surface heat flow observations (see Appendix \@ref(krigeOpt)). For example, Kriging resolves tectonic features near Tonga New Zealand and Vanuatu that are discordant with Similarity predictions but compatible with ThermoGlobe data (Figures \@ref(fig:tongaNewZealandDiff) & \@ref(fig:vanuatuDiff)). Such contrasting features imply ThermoGlobe data violate one or more geologic proxy data sets used by Similarity. In other words, Kriging will tend to highlight anomalies (compared to Similarity) if they exist and have been observed.
...
2. The Kaleidoscope of upper-plate surface heat flow distributions and profiles implies various degrees of (dis)continuity exist among subduction zones in terms of their upper-plate thickness (contrary to expectations from Chapter \@ref(chpt2)), heat-transferring dynamics, and/or observational density relative to the local spatial variability of surface heat flow
