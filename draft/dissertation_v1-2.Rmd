...
While ThermoGlobe currently contains `r format(nrow(hf), big.mark = ',')` observations, the regional coverage near subduction zones is uneven and irregularly-spaced (Figure \@ref(fig:globalhfComp) & Table \@ref(tab:hfSummaryTable)). Many methods may be considered to interpolate surface heat flow including inverse distance weighting, various splines and linear regressions, nearest-neighbor, general additive models, Kriging, and Similarity, to name a few. Generally speaking, these methods predict surface heat flow across a grid of arbitrarily defined locations with unknown surface heat flow (the targets) by fitting mathematical models to available surface heat flow observations (the examples). The two interpolation methods compared in this study, Kriging and Similarity, are chosen because they represent end-member methodological classes based on fundamentally different principles. Their comparative differences, therefore, may be important for advancing understanding of subduction zone processes, for targeting areas for future heat flow research, for improving either method in the context of subduction zone research, and for developing new layered interpolation methods that combine the strengths of Kriging and Similarity techniques.
...
### Comparing Similarity and Kriging Interpolations {#interpDiff}
...
footnote(
  general_title = '',
  general = c('\\\\uline{\\\\textit{note}}: showing lowest-cost models from Table \\\\ref{tab:vgrmSummaryTableLong} (excluding Gaussian)', '\\\\uline{\\\\textit{key}}: $n_{max}$: max point-pairs, Sill $[(mWm^{-2})^2]$, Range $[km]$, Cost $[mWm^{-2}]$, $RMSE_S$: Similarity accuracy $[mWm^{-2}]$, $RMSE_K$: Kriging accuracy $[mWm^{-2}]$'),
  threeparttable = T,
  escape = F
)
```
...
Surface heat flow profiles in previous studies are computed with ThermoGlobe data sampled from within a single sector [@currie2004; @currie2006; @hyndman2005; @furukawa1993; @wada2009; @kerswell2020] with the principal goal of understanding surface heat flow patterns across (orthogonal to) subduction zone segments. However, this study aims at understanding along-strike surface heat flow patterns and therefore simply extends the previous approach to many adjacent sectors. Although simple to implement, evaluating surface heat flow variability along-strike with samples of ThermoGlobe data is ineffective. For example, sample sizes within adjacent sectors can often be drastically different (e.g. Central America, Sumatra Banda Sea, and Tonga New Zealand, Table \@ref(tab:sectorSummaryTable)). Fluctuating sample sizes and coverage among upper-plate sectors can make statistical comparisons equivocal.

Indeed, this study finds comparing ThermoGlobe data among upper-plate sectors problematic. For instance, ThermoGlobe data are often too few (n < 30 observations for `r nrow(sectors.df[sectors.df$n.obs < 30,])`/`r nrow(sectors.df)` sectors, Table \@ref(tab:sectorSummaryTable)) to compare among other sectors with statistical confidence. Many (n = `r nrow(sectors.df[sectors.df$n.obs == 1,])`) sectors have a single observation with a singular distribution (\gls{iqr} = 0) or very few observations spanning a large range (very large \gls{iqr}). That is, summary statistics necessary for gauging the (dis)similarity of ThermoGlobe data within a sector (e.g. either parametric: mean, standard deviation or nonparametric: median, interquartile range, Table \@ref(tab:sectorSummaryTable)) become unreliable with few ThermoGlobe observations (n < 5 observations for `r nrow(sectors.df[sectors.df$n.obs < 5,])`/`r nrow(sectors.df)` sectors). Finally, many sectors encompass zero ThermoGlobe data and therefore cannot be compared at all.

The limitations above arise from sampling irregularly-spaced data, but can be easily overcome by interpolation. For example, sampling a regular interpolation grid allows for more consistent sample sizes and spatial coverage across sectors. While many sectors defined in this study have few ThermoGlobe data (n < 5 observations for `r nrow(sectors.df[sectors.df$n.obs < 5,])`/`r nrow(sectors.df)` sectors, Table \@ref(tab:sectorSummaryTable)), the average number of interpolation targets within upper-plate sectors is `r round(mean(sectors.df$n.sim))`. Yet the advantages of regular grids (consistent sample sizes and regular spatial coverage) can be abstracted to ThermoGlobe data through interpolation (with any method). Surface heat flow variability among sectors is thus more confidently and consistently evaluated with distributions of Similarity and Kriging predictions rather than distributions of ThermoGlobe data alone.

#### Surface Heat Flow Variability

Comparisons among Similarity and Kriging predictions and ThermoGlobe data across thousands of square kilometers very clearly show mixed results---despite being derived from nearly-identical sets of processed ThermoGlobe data (see Section \@ref(chpt3Methods)). In summary, Similarity and Kriging predictions and ThermoGlobe data across `r nrow(sectors.df)` upper-plate sectors neither show broadly similar nor broadly dissimilar upper-plate surface heat flow within, or among subduction zone segments. Rather than observing consistently uniform upper-plate surface heat flow, a spectrum of surface heat flow profiles and distributions are observed (compare Figures \@ref(fig:kyushuRyukyuUpper), \@ref(fig:sumatraBandaSeaUpper), \@ref(fig:newBritainSolomonUpper), Appendix \@ref(lateralDiffAppendix), & Table \@ref(tab:sectorSummaryTable)).

On one hand, distinct patterns of surface heat flow among upper-plate sectors are observed for many subduction zone segments (e.g. Andes, Central America, Lesser Antilles, N Philippines, New Britain Solomon, S Philippines, Figures \@ref(fig:andesUpper), \@ref(fig:centralAmericaUpper), \@ref(fig:lesserAntillesUpper), \@ref(fig:nPhilippinesUpper), \@ref(fig:newBritainSolomonUpper), \@ref(fig:sPhilippinesUpper) & Table \@ref(tab:sectorSummaryTable)). These observations imply spatially heterogeneous lithospheric thickness (contrary to expectations from Chapter \@ref(chpt2)), spatially heterogeneous dynamics, spatially heterogeneous near-surface perturbations, and/or undersampling relative to the scale and magnitude of spatial variability. As a corollary, gross similarity in surface heat flow patterns among upper-plate sectors implies the opposite. For example, surface heat flow appears rather uniform across large regions for many other subduction zone segments (e.g. Alaska Aleutians, Kamchatka Marianas, Kyushu Ryukyu, Sumatra Banda Sea, Figures \@ref(fig:alaskaAleutiansUpper), \@ref(fig:kamchatkaMarianasUpper), \@ref(fig:kyushuRyukyuUpper), \@ref(fig:sumatraBandaSeaUpper) & Table \@ref(tab:sectorSummaryTable)). These observations support relatively uniform upper-plate thickness inferred from previous work [@currie2004; @currie2006] and inferred mechanical consequences for plate coupling [@furukawa1993; @kerswell2020; @wada2009].

However, the spectrum of results can aid in discovering targets for future investigation. For example, subduction zone segments with the greatest discrepancies between Similarity and Kriging predictions among sectors tend to have: (1) very few ThermoGlobe data (e.g. N Philippines, New Britain Solomon, S Philippines), (2) highly-irregular spatial coverage of ThermoGlobe data (e.g. Central America, Lesser Antilles), and/or (3) complex upper-plate tectonics (Vanuatu).
