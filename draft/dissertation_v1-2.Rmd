...
The amount of heat escaping Earth's surface can be related to the thermal state of Earth's lithosphere plus heat-transferring and heat-generating subsurface processes like hydrothermal circulation, radioactive decay, fault motion, plate coupling, and mantle convection [@fourier1827; @kelvin1863; @parsons1977; @pollack1977; @stein1992; @hutnak2008; @hasterok2013; @stein1994; @rudnick1998; @furlong2013; @gao2014; @furukawa1993; @wada2009; @kerswell2020; @currie2004; @currie2006]. Surface heat flow observations are therefore critically important for understanding lithospheric thermal structure, deformation and potential seismic hazards, groundwater hydrology and potential environmental impacts, and economic resource exploration (hydrocarbons, minerals, geothermal energy). Monumental efforts to take tens of thousands of continental and oceanic surface heat flow measurements (see citations in Section \@ref(tglobe)) and compile them into databases [@pollack1993; @hasterok2008; @lucazeau2019; @jennings2021] enable multi-disciplinary investigations into lithospheric and crustal processes. For decades surface heat flow observations have motivated research and exploration---as evident by more than 1,393 publications compiled in the largest and most recent surface heat flow database [ThermoGlobe, @jennings2021; @lucazeau2019].

While ThermoGlobe currently contains `r format(nrow(hf), big.mark = ',')` observations, the spatial coverage near subduction zones is uneven with highly irregular-spacing at the regional scale (Figure \@ref(fig:globalhfComp) & Table \@ref(tab:hfSummaryTable)). Note that many studies in ThermoGlobe include datasets of neatly organized observations, often collocated with seismic arrays. At a regional scale, however, individual measurements and clusters of neat arrays can be separated by hundreds to thousands of $km$. Many methods may be considered to interpolate regional surface heat flow, including inverse distance weighting, various splines and linear regressions, nearest-neighbor, general additive models, Kriging, and Similarity, to name a few. Generally speaking, these methods predict unknown surface heat flow (the targets) by a linear combination of available surface heat flow observations (the examples). The two interpolation methods compared in this study, Kriging and Similarity, are chosen because they represent end-member classes based on fundamentally different principles and mathematical frameworks. Their comparative differences, therefore, may be important for understanding lithospheric thermal structure, for identifying surface heat flow anomalies, for identifying practical limitations and strengths of each approach, and for developing new layered interpolation methods that combine the strengths of Kriging and Similarity techniques.
...
The rationale for using Kriging is embodied in the First Law of Geography: *everything is related, but nearer things are more related* [@krige1951; @matheron1963]. Kriging inherently attempts to predict the *ground-truth* by estimating the spatial continuity of observed surface heat flow. In stark contrast, Similarity is a spatially-independent method and predicts surface heat flow by integrating geographic, geologic, geochronologic, and geophysical information [see @goutorbe2011 for method details]. The technique involves evaluating the similarity between an unknown target and *every* surface heat flow observation in the global dataset, irrespective of their locations [i.e. a target and an example are similar if they have similar bathymetry, elevation, proximity to active or ancient orogens, seafloor age, upper mantle shear wave velocities, etc., @chapman1975; @lee1965; @sclater1970; @davies2013; @shapiro2004; @lucazeau2019]. The rationale for using Similarity is embodied in the Third Law of Geography: *the more similar the geographic configuration of two points, the more similar their values* [@zhu2018]. Global-scale problems and regional domains with relatively few surface heat flow data are particularly suited for Similarity as the method is independent of observational density.

It is especially notable for regional geodynamic investigations, however, that Kriging is explicitly spatially-dependent, whereas Similarity is not. Generally speaking, spatial (dis)continuity of surface heat flow observations reflect the (dis)continuity of subsurface processes. For example, it is inferred that broad regions of low surface heat flow on continents outline cratons, anomalously low surface heat flow in young oceanic crust imply significant permeability and heat extraction by seawater [@stein1994; @fischer2000; @hutnak2008; @hasterok2011], and patterns of trench-orthogonal surface heat flow profiles imply uniform upper-plate lithospheric thickness [@currie2004; @currie2006; @hyndman2005] and mechanical coupling depths [@furukawa1993; @wada2009; @kerswell2020]. Making predictions without explicitly considering spatial continuity may overlook important surface heat flow patterns and/or anomalies---leading to erroneous surface heat flow inversions and interpretations.

This Chapter compares Similarity and Kriging interpolations near 13 presently active subduction zones while considering the following questions: (1) how does surface heat flow continuously vary near subduction zones, especially along strike across the upper-plate? (2) How do Kriging and Similarity predictions compare? (3) What do the differences (if any) imply about geodynamic variability among active subduction zones, especially with respect to uniform upper-plate thickness discussed in Chapter \@ref(chpt2)?
