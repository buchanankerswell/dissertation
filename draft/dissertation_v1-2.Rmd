...
(ref:bigCompCap) Summary of classification results for model cdi62. (a-d) Although the absolute range of recovered markers (black point cloud and blue Tanaka density contours) overlaps with `pd15` and `ag18`, marker density is concentrated along cooler thermal gradients compared to natural samples and clusters within three distinct regions coinciding with the continental Moho (about 1 $GPa$), the (a)seismic transition (about 1.6 $GPa$ and 350 $^\circ C$), and onset of plate coupling (about 2.3 $GPa$ and 600 $^\circ C$. (e) The evolution of model cdi62 shows two major modes of underplating at shallower depths, followed by a narrowing of the plate interface and a final recovery mode near the onset of plate coupling. Note the few (`r round((1-stats.summary$ratio.below.30kbar.est[stats.summary$model == 'cdi62'])*100, 1)` %) misclassified markers recovered from $\geq$ 3 $GPa$. See Figure \@ref(fig:rockPTComp) for explanation of (a-d).
...
> |     than `ag18` (`r round(ag18.grad$prob[which.min(abs(ag18.grad$grad-15))]*100)-round(ag18.grad$prob[which.min(abs(ag18.grad$grad-5))]*100)`% between 5-15 $^\circ C/km$) and `pd15` (`r round(pd15.grad$prob[which.min(abs(pd15.grad$grad-15))]*100)-round(pd15.grad$prob[which.min(abs(pd15.grad$grad-5))]*100)`% between 5-15
> |     $^\circ C/km$)
> |     Figure \@ref(fig:marxComp)d
> |
> |  5. Like `pd15` and `ag18`, there is an insignificant amount of markers
> |     recovered from thermal gradients not typically recognized, or not
> |     typically measured, in natural samples (`r round(cdfGrad.summary$prob[which.min(abs(cdfGrad.summary$maxGrad*1e3-5))]*100)`% from $\leq$ 5 $^\circ C/km$)
> |     Figure \@ref(fig:marxComp)d
> |
...
> |     compared to `pd15` and `ag18` (between `r round((1-pd15.grad$prob[which.min(abs(pd15.grad$grad-15))])*100)`-`r round((1-ag18.grad$prob[which.min(abs(ag18.grad$grad-15))])*100)`% from $\geq$ 15 $^\circ C/km$)
> |     Figure \@ref(fig:marxComp)d

(ref:marxCompCap) Combined distribution of recovered markers from all 64 simulations. (a) Most markers (black point cloud and blue Tanaka density contours) are recovered from two major modes near the continental Moho around 1 $GPa$ and 350 $^\circ C$ and near the onset of plate coupling (antigorite dehydration depth) around 2.5 $GPa$ and 625 $^\circ C$, with some dispersion along the plate interface from 1-2 $GPa$. Note the gap in recovery around 2 $GPa$ and 550 $^\circ C$ that coincides with the highest-density of natural samples. (b) A broad single-step \gls{cdf} in P shows high recovery rates between 0.8-1.5 $GPa$ (`r round((cdfP.summary$prob[which.min(abs(cdfP.summary$maxP/1e4-1.5))]*100)-(cdfP.summary$prob[which.min(abs(cdfP.summary$maxP/0.8e4-1))]*100))`%) compared to `pd15` and `ag18` (`r round(abs(pd15.P$prob[which.min(abs(pd15.P$P-0.8))]-pd15.P$prob[which.min(abs(pd15.P$P-1.5))])*100)`-`r round(abs(ag18.P$prob[which.min(abs(ag18.P$P-0.8))]-ag18.P$prob[which.min(abs(ag18.P$P-1.5))])*100)`%). (c) Another broad single-step \gls{cdf} in T shows high recovery rates from $\leq$ 500 $^\circ C$ (`r round(cdfT.summary$prob[which.min(abs(cdfT.summary$maxT-273-500))]*100)`%) compared to natural samples (`r round(pd15.T$prob[which.min(abs(pd15.T$T-500))]*100)`-`r round(ag18.T$prob[which.min(abs(ag18.T$T-500))]*100)`%) with most deeply subducting markers recovered just before antigorite dehydration in the upper-plate mantle wedge. (d) Virtually all markers `r round(cdfGrad.summary$prob[which.min(abs(cdfGrad.summary$maxGrad*1e3-15))]*100, 2)`% are recovered from $\leq$ 15 $^\circ C/km$) compared to `r round(ag18.grad$prob[which.min(abs(ag18.grad$grad-15))]*100)`-`r round(pd15.grad$prob[which.min(abs(pd15.grad$grad-15))]*100)`% for natural samples. Explanation of (a-d) is the same as Figure \@ref(fig:rockPTComp).
...
To better understand how rock recovery can vary among subduction zones with different boundary conditions, important modes of marker recovery are determined by finding peaks in marker density with respect to \gls{pt} and \gls{pt} gradients (Figure \@ref(fig:corrComp)). The tallest density peak (`mode1`) gives a sense of where the greatest abundance of markers are being recovered from, and the highest density peak in terms of value (`mode2`) gives a sense of how the most deeply subducted markers (or markers with the highest gradients) vary across different numerical experiments. The overall picture drawn from many (non)correlations can be summarised as follows.
...
add_header_above(
  c(
    ...
    'rate', 'md1 P', 'md2 P', 'md1 T', 'md2 T', 'md1 $\\\\nabla$', 'md2 $\\\\nabla$'
  ),
  line_sep = 0,
  escape = F
) %>%
...
(ref:youngSlowCap) Combined distribution of recovered markers from all 4 simulations with the youngest oceanic-plates and slowest convergence velocities. (a) Most markers (black point cloud and blue Tanaka density contours) are recovered from two major modes near the continental Moho around 1 $GPa$ and 350 $^\circ C$ and near the onset of plate coupling (antigorite dehydration depth) around 2.5 $GPa$ and 625 $^\circ C$, with some dispersion along the plate interface from 1-2 $GPa$. Note the gap in recovery around 2 $GPa$ and 550 $^\circ C$ that coincides with the highest-density of natural samples. (b) A broad single-step \gls{cdf} in P shows high recovery rates between 0.8-1.5 $GPa$ (`r round((cdfP.summary$prob[which.min(abs(cdfP.summary$maxP/1e4-1.5))]*100)-(cdfP.summary$prob[which.min(abs(cdfP.summary$maxP/0.8e4-1))]*100))`%) compared to `pd15` and `ag18` (`r round(abs(pd15.P$prob[which.min(abs(pd15.P$P-0.8))]-pd15.P$prob[which.min(abs(pd15.P$P-1.5))])*100)`-`r round(abs(ag18.P$prob[which.min(abs(ag18.P$P-0.8))]-ag18.P$prob[which.min(abs(ag18.P$P-1.5))])*100)`%). (c) Another broad single-step \gls{cdf} in T shows high recovery rates from $\leq$ 500 $^\circ C$ (`r round(cdfT.summary$prob[which.min(abs(cdfT.summary$maxT-273-500))]*100)`%) compared to natural samples (`r round(pd15.T$prob[which.min(abs(pd15.T$T-500))]*100)`-`r round(ag18.T$prob[which.min(abs(ag18.T$T-500))]*100)`%) with most deeply subducting markers recovered just before antigorite dehydration in the upper-plate mantle wedge. (d) Virtually all markers `r round(cdfGrad.summary$prob[which.min(abs(cdfGrad.summary$maxGrad*1e3-15))]*100, 2)`% are recovered from $\leq$ 15 $^\circ C/km$) compared to `r round(ag18.grad$prob[which.min(abs(ag18.grad$grad-15))]*100)`-`r round(pd15.grad$prob[which.min(abs(pd15.grad$grad-15))]*100)`% for natural samples. Explanation of (a-d) is the same as Figure \@ref(fig:rockPTComp).
