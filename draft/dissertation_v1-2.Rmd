...
>
> 3. Between `r round(pd15.T$prob[which.min(abs(pd15.T$T-625))]*100)`-`r round(ag18.T$prob[which.min(abs(ag18.T$T-625))]*100)`% of recovered rocks equilibrate at of $\leq$ 625 $^\circ C$
>
...
Unfortunately, previous work comparing the rock record directly with numerical models has generally produced ambiguous interpretations concerning rates and distributions of rock recovery. For example, comparisons of different geodynamic codes with diverse subsets of the rock record show various extents of agreement in terms of overlapping \gls{pt} paths and thermal gradients [@penniston2015; @yamato2007; @yamato2008; @roda2010; @angiboust2012b; @roda2012; @burov2014; @ruh2015; @plunder2018; @roda2020; @holt2021]. Initial conditions for numerical experiments [oceanic-plate age, convergence velocity, subduction dip angle, upper-plate thickness, and heating sources, @kohn2018; @ruh2015; @penniston2015; @vankeken2019], differential recovery rates from subduction zones with favorable thermo-kinematic boundary conditions [@abers2017; @vankeken2018], and comparisons among suites of undifferentiated \gls{hp} rocks [e.g. potentially grouping rocks recovered during subduction initiation with rocks recovered during "steady-state" subduction, see @agard2018; @agard2020] are all potential explanations for (non)overlapping \gls{pt} estimates and thermal gradients. Such assorted results and sensitivities warrant further investigation into the general response of \gls{pt} distributions and recovery rates to broad ranges of thermo-kinematic boundary conditions.

A fundamental challenge in interpreting the rock record stems from small sample sizes (typically less than a few dozen \gls{pt} estimates from any given exhumed terrane) compared to the largely unconstrained range of conditions suitable for rock recovery. Arguments can be made that the rock record selectively samples rare conditions because recovery of subducted material occurs sporadically during short-lived mechanical transitions [@agard2016], and/or that exhumation is limited to end-member subduction systems [@abers2017; @vankeken2018]. While natural examples of fossil plate interfaces have been studied extensively [@agard2018; @platt1986; @tewksbury2021a; @ioannidi2020; @plunder2013; @plunder2015; @angiboust2011; @angiboust2015; @kitamura2012; @wakabayashi2015; @fisher2021; @kotowski2019], these type localities are few and differ significantly in terms of their geometry, composition (rock types), and interpreted deformation histories. This Chapter addresses the problem of small sample sizes by simulating a large (n = `r format(nrow(marx.class.summary[marx.class.summary$recovered,]), big.mark = ',')`) \gls{ptt} dataset from numerical geodynamic models. The assumption is that a large dataset is insensitive to outliers and represents a statistically robust picture of recovery rates and \gls{pt} distributions in subduction zones. Indeed, clear (non)correlations among recovery rates, \gls{pt} distributions, and thermo-kinematic boundary conditions are found---indicating a range of (in)plausible conditions for reproducing the rock record.
...
include_graphics('assets/figs/chpt4/cdc78_k10_class.png')
