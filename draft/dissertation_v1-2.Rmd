...
Surface heat flow profiles across subduction zone segments are computed in previous studies by sampling ThermoGlobe data from within a single sector [@currie2004; @currie2006; @hyndman2005; @furukawa1993; @wada2009; @kerswell2020]. Although simple to implement, sampling ThermoGlobe data within adjacent sectors is ineffective for evaluating surface heat flow variability along-strike. For example, sample sizes within adjacent sectors can often be drastically different (e.g. Central America, Sumatra Banda Sea, and Tonga New Zealand, Table \@ref(tab:sectorSummaryTable)). Fluctuating sample sizes and coverage among upper-plate sectors can make statistical comparisons equivocal.

Indeed, this study finds comparing ThermoGlobe data among upper-plate sectors problematic. For instance, ThermoGlobe data are often too few (n < 30 observations for `r nrow(sectors.df[sectors.df$n.obs < 30,])`/`r nrow(sectors.df)` sectors, Table \@ref(tab:sectorSummaryTable)) to compare with statistical confidence. Moreover, many (n = `r nrow(sectors.df[sectors.df$n.obs == 1,])`) sectors have a single observation with a singular distribution (\gls{iqr} = 0) or very few observations spanning a large range (very large \gls{iqr}). Many sectors are omitted in the analysis because they simply encompass zero ThermoGlobe data, although Similarity and Kriging predictions could still be compared.

Limitations arising from sampling irregularly-spaced data are easily overcome by a regular sampling grid. A regular grid allows for more consistent sample sizes and spatial coverage across sectors. For example, many sectors defined in this study have few ThermoGlobe data (n < 10 observations for `r nrow(sectors.df[sectors.df$n.obs < 10,])`/`r nrow(sectors.df)` sectors, Table \@ref(tab:sectorSummaryTable)), while the average number of interpolation grid points within upper-plate sectors in this study is `r round(mean(sectors.df$n.sim))`. However, the advantages of regular grids (consistent sample sizes and regular spatial coverage) can be abstracted to ThermoGlobe data through interpolation (e.g. Kriging, Similarity, or other). Surface heat flow variability among sectors is thus more confidently and consistently evaluated with Similarity and Kriging predictions compared to ThermoGlobe data alone.
