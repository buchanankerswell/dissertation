...
Sumatra Banda Sea characterizes a subduction zone segment with moderately consistent surface heat flow for thousands of $km$ along-strike. For example, medians and interquartile ranges of Similarity and Kriging predictions overlap well for some sectors, but not others (e.g. sectors 1, 10, & 11; Figure \@ref(fig:sumatraBandaSeaUpper)). Median Similarity and Kriging predictions differ by `r sectors.summary$avg.med.dif[sectors.summary$segment == 'Sumatra Banda Sea']` $\pm$ `r 2*sectors.summary$sd.med.dif[sectors.summary$segment == 'Sumatra Banda Sea']` $mWm^{-2}$ and interquartile ranges differ by `r sectors.summary$avg.iqr.dif[sectors.summary$segment == 'Kyushu Ryukyu']` $\pm$ `r 2*sectors.summary$sd.iqr.dif[sectors.summary$segment == 'Kyushu Ryukyu']` $mWm^{-2}$ on average across all sectors (Table \@ref(tab:sectorSummaryTable)). Notably, Similarity and Kriging predictions appear to broadly oscillate between high and low surface heat flow across adjacent sectors in a wave-like fashion. The wave-like signal has a wavelength of about 2500 $km$. Similar oscillating sine-like surface heat flow signals may exist in other segments (e.g. Kamchatka Marianas; Figure \@ref(fig:kamchatkaMarianasUpper)), although not as apparent as Sumatra Banda Sea (see Appendix \@ref(lateralDiffAppendix)).

Meanwhile, Similarity and Kriging profiles show obvious differences. Similarity predictions are distributed narrowly and increase monotonically from the trench to 500 $km$ into the upper-plate, whereas Kriging profiles generally ramp up more steeply and disperse beginning at approximately 200 $km$ from the trench. Kriging profiles show up to 25-30 $mWm^{-2}$ difference in surface heat flow among sectors at 300-500 $km$ from the trench compared to a relatively narrow spread of Similarity profiles. Although Kriging profiles appear more consistent with the range of (total n = `r sectors.summary$n.obs[sectors.summary$segment == 'Sumatra Banda Sea']`) ThermoGlobe observations compared to Similarity profiles, Kriging predictions do not generally overlap with observations better than Similarity (compare Figure \@ref(fig:sumatraBandaSeaUpper)c with \@ref(fig:sumatraBandaSeaUpper)b).
...
New Britain Solomon characterizes a subduction zone segment with inconsistent surface heat flow and poor overlap between Similarity and Kriging predictions. Only one sector (sector 8) shows overlapping interquartile ranges of Similarity and Kriging predictions, whereas all other sectors strongly diverge (Figure \@ref(fig:newBritainSolomonUpper)). For example, median Kriging predictions range `r sectors.summary$range.med.krg[sectors.summary$segment == 'New Britain Solomon']` $mWm^{-2}$ across all sectors, whereas median Similarity predictions range much more (`r sectors.summary$range.med.sim[sectors.summary$segment == 'New Britain Solomon']` $mWm^{-2}$). Moreover, Similarity and Kriging medians across all sectors differ by `r sectors.summary$avg.med.dif[sectors.summary$segment == 'New Britain Solomon']` $\pm$ `r 2*sectors.summary$sd.med.dif[sectors.summary$segment == 'New Britain Solomon']` $mWm^{-2}$ on average. Notably, an apparent wave-like oscillation between higher and lower surface heat flow across adjacent sectors is observed again---especially for Similarity predictions.

Meanwhile, Similarity and Kriging profiles are also obviously distinguishable. For example, Kriging profiles are smooth and closely parallel ThermoGlobe data, whereas Similarity profiles show higher heat flow where geodynamically expected (e.g. in sector 4, which intersects a volcanic center and ridge segment; Figure \@ref(fig:newBritainSolomonUpper)). Thus, methodological differences apparently drive divergence between Similarity and Kriging predictions in this case.
...
The total number of ThermoGlobe data across all sectors is relatively small (from n = `r range(sectors.summary$n.obs)[1]` to `r range(sectors.summary$n.obs)[2]`) compared to Similarity and Kriging predictions (from `r range(sectors.summary$n.sim)[1]` to `r range(sectors.summary$n.sim)[2]`). For example, many (n = `r nrow(sectors.df[sectors.df$n.obs == 0,])`) sectors have a single observation (singular distribution; \gls{iqr} = 0) or only a few observations that may vary wildly---resulting in very wide distribution (Table \@ref(tab:sectorSummaryTable)).

While distributions of ThermoGlobe data may show differences in surface heat flow and/or corroborate Similarity and Kriging predictions among certain sectors, the wildly ranging number of data points and heterogeneous spatial coverage among sectors make statistical comparisons equivocal.
