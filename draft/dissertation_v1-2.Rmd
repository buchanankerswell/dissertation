...
sectors.df <-
  tibble(
    ...
    sector = unlist(map(shp.sectors, ~map_chr(.x$interp, ~.x$split_fID[1])), use.names = F),
...
sectors.summary <-
  ...
  group_by(segment) %>%
  summarise(
    range.sim = range(med.sim)[2] - range(med.sim)[1],
    range.krg = range(med.krg)[2] - range(med.krg)[1]
  )
```
...
sectors.df %>%
select(-c(min.sim, max.sim, min.krg, max.krg)) %>%
kable(
  col.names = c('Segment', 'Sector', 'Median', 'IQR', 'Median', 'IQR', '$\\Delta$Median'),
  caption = 'Similarity and Kriging predictions by sector',
  linesep = '',
  escape = F,
  digits = 3,
  longtable = T,
