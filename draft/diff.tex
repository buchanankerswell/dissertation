...
%DIF DEL dissertation_v1-1.tex   Sun Dec 12 11:42:55 2021
%DIF ADD dissertation_v1-2.tex   Mon Jan  3 17:00:07 2022
...
% Make title page
  \title{Computational Approaches to Understanding Subduction \DIFaddbegin \DIFadd{Zone }\DIFaddend Geodynamics, Surface Heat Flow, and \DIFdelbegin \DIFdel{the }\DIFdelend Metamorphic Rock Record}
...
\def\maketitle{
  ...
  \begin{titlepage}
    ...
    \begin{center}
        % Title
%DIF 241c244
%DIF <         {\huge Computational Approaches to Understanding Subduction Geodynamics, Surface Heat Flow, and the Metamorphic Rock Record \par}
%DIF -------
        {\huge Computational Approaches to Understanding Subduction Zone Geodynamics, Surface Heat Flow, and Metamorphic Rock Record \par} %DIF > 
%DIF -------
        \vspace*{0.5in}
...
%DIF 280c283
%DIF <         Dissertation Title: & {Computational Approaches to Understanding Subduction Geodynamics, Surface Heat Flow, and the Metamorphic Rock Record}
%DIF -------
        Dissertation Title: & {Computational Approaches to Understanding Subduction Zone Geodynamics, Surface Heat Flow, and Metamorphic Rock Record} %DIF > 
%DIF -------
      \end{tabularx}
    \end{singlespace}
    \begin{tabularx}{\textwidth}{@{}lX} 
...
\DIFaddend % Body (everything in .Rmd beneath YAML)
\hypertarget{chpt1}{%
...
%DIFDELCMD < {\centering \includegraphics[width=1\linewidth,]{assets/figs/chpt2/fig6} 
%DIFDELCMD < 
...
\DIFdelbegin %DIFDELCMD < \hypertarget{chpt3}{%
%DIFDELCMD < \chapter{A Comparison of Heat Flow Interpolations Near Subduction Zones}\label{chpt3}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{chpt3}{%
...
The magnitude of heat fluxing through the Earth's surface \DIFdelbegin \DIFdel{is directly proportional to the magnitude of temperature gradients and strain rates within }\DIFdelend \DIFaddbegin \DIFadd{depends on the thermal state of }\DIFaddend Earth's \DIFdelbegin \DIFdel{crust and upper mantle. Global heat flow databases are thus promising sources for generating and testing hypotheses about subduction geodynamics}\DIFdelend \DIFaddbegin \DIFadd{lithosphere. Surface heat flow observations from continents and seafloors are thus critically important for understanding the thermal structure and thermal processing (e.g.~by fluids) of lithosphere at subduction zones}\DIFaddend . However, evaluating \DIFaddbegin \DIFadd{regional }\DIFaddend surface heat flow \DIFaddbegin \DIFadd{patterns }\DIFaddend across large tectonic features requires careful \DIFdelbegin \DIFdel{projection and }\DIFdelend interpolation of sparse\DIFdelbegin \DIFdel{observations}\DIFdelend \DIFaddbegin \DIFadd{, irregularly-spaced observations across distances from 10\(^{-1}\) to 10\(^3\) \(km\)}\DIFaddend . This study compares two interpolation methods based on fundamentally different principles, \DIFdelbegin \DIFdel{Kriging and Similarity}\DIFdelend \DIFaddbegin \emph{\DIFadd{Similarity}} \DIFadd{and }\emph{\DIFadd{Kriging}}\DIFaddend , to investigate spatial variability of surface heat flow near 13 presently active subduction \DIFdelbegin \DIFdel{zones. Kriging and Similarity are grossly comparable at regional scales with median differences from -2 to 14 \(mWm^{-2}\). More importantly, Kriging and Similarity both show distinctive trench-perpendicular surface heat flow patterns along strike within the upper-plates of 7 out of 13 segments. Moreover, average }\DIFdelend \DIFaddbegin \DIFadd{zone segments. Samples of Similarity and Kriging predictions across adjacent }\DIFaddend upper-plate \DIFaddbegin \DIFadd{sectors show a Kaleidoscope of }\DIFaddend surface heat flow \DIFdelbegin \DIFdel{can differ up to 50 \(mWm^{-2}\) along a single subduction zone segment. Subduction zone segments with discontinuous upper-plate surface heat flow may reflect spatially heterogeneous lithospheric thickness (contrary to expectations from Chapter \ref{chpt2}) , spatially heterogeneous dynamics, spatially heterogeneous near-surface perturbations, }\DIFdelend \DIFaddbegin \DIFadd{distributions and profiles among subduction zone segments. Such mixed profiles imply various degrees of (dis)continuity exist among subduction zone segments in terms of their (1) lithospheric thermal structure, (2) heat-transferring subsurface dynamics, }\DIFaddend and/or \DIFdelbegin \DIFdel{undersampling }\DIFdelend \DIFaddbegin \DIFadd{(3) observational density }\DIFaddend relative to the \DIFdelbegin \DIFdel{scale and magnitude of spatial variability . Subduction zone segments with continuous upper-plate }\DIFdelend \DIFaddbegin \DIFadd{local (unknown) spatial variability of }\DIFaddend surface heat flow\DIFdelbegin \DIFdel{may imply the opposite. Potential explanations for a nearly 50\%-50\% split between segments with (dis)continuous upper-plate }\DIFdelend \DIFaddbegin \DIFadd{. Anomalies and methodological strengths identified by comparing independent interpolation methods may aid in identifying future survey targets and developing more accurate regional }\DIFaddend surface heat flow \DIFdelbegin \DIFdel{patterns is yet to be identified and warrants further investigation}\DIFdelend \DIFaddbegin \DIFadd{interpolations near subduction zones}\DIFaddend .

\hypertarget{chpt3Intro}{%
\section{Introduction}\label{chpt3Intro}}

\DIFdelbegin \DIFdel{Surface heat flow databases (}%DIFDELCMD < \protect\hyperlink{ref-hasterok2008}{Hasterok \& Chapman, 2008}%%%
\DIFdel{; }%DIFDELCMD < \protect\hyperlink{ref-jennings2021}{Jennings \& Hasterok, 2021}%%%
\DIFdel{; }%DIFDELCMD < \protect\hyperlink{ref-lucazeau2019}{Lucazeau, 2019}%%%
\DIFdel{; }%DIFDELCMD < \protect\hyperlink{ref-pollack1993}{Pollack et al., 1993}%%%
\DIFdel{) enable geodynamic investigations by relating the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend amount of heat escaping Earth's surface \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{depends on the thermal state of Earth's lithosphere, plus }\DIFaddend heat-transferring and heat-generating subsurface processes like hydrothermal circulation, radioactive decay, fault motion, and mantle convection (\protect\hyperlink{ref-currie2004}{Currie et al., 2004}; \protect\hyperlink{ref-currie2006}{Currie \& Hyndman, 2006}; \protect\hyperlink{ref-fourier1827}{Fourier, 1827}; \protect\hyperlink{ref-furlong2013}{Furlong \& Chapman, 2013}; \protect\hyperlink{ref-furukawa1993}{Furukawa, 1993}; \protect\hyperlink{ref-gao2014}{Gao \& Wang, 2014}; \protect\hyperlink{ref-hasterok2013}{Hasterok, 2013}; \protect\DIFaddbegin \hyperlink{ref-hutnak2008}{Hutnak et al., 2008}\DIFadd{; }\protect\DIFaddend \hyperlink{ref-kelvin1863}{Kelvin, 1863}; \protect\hyperlink{ref-kerswell2020}{Kerswell et al., 2020}; \protect\hyperlink{ref-parsons1977}{Parsons \& Sclater, 1977}; \protect\hyperlink{ref-pollack1977}{Pollack \& Chapman, 1977}; \protect\hyperlink{ref-rudnick1998}{Rudnick et al., 1998}; \protect\hyperlink{ref-stein1992}{Stein \& Stein, 1992}, \protect\hyperlink{ref-stein1994}{1994}; \protect\hyperlink{ref-wada2009}{Wada \& Wang, 2009}). Surface heat flow observations \DIFdelbegin \DIFdel{have motivated research in many scientific and industrial fields as evident by more than }\DIFdelend \DIFaddbegin \DIFadd{are thus critically important for understanding lithospheric evolution, crustal deformation and seismic hazards, groundwater hydrology and environmental impacts, and exploration of economic resources (e.g.~hydrocarbon, mineral, and geothermal energy). Monumental efforts to take tens of thousands of continental and oceanic surface heat flow measurements (see }\protect\hyperlink{tglobe}{ThermoGlobe References} \DIFadd{with }\DIFaddend 1,393 \DIFdelbegin \DIFdel{publications compiled in the largest and most recent }\DIFdelend \DIFaddbegin \DIFadd{citations) and compile them into databases (}\protect\hyperlink{ref-hasterok2008}{Hasterok \& Chapman, 2008}\DIFadd{; }\protect\hyperlink{ref-jennings2021}{Jennings \& Hasterok, 2021}\DIFadd{; }\protect\hyperlink{ref-lucazeau2019}{Lucazeau, 2019}\DIFadd{; }\protect\hyperlink{ref-pollack1993}{Pollack et al., 1993}\DIFadd{) enable multi-disciplinary investigations of lithospheric and crustal processes.
}

\DIFadd{The most recent global }\DIFaddend surface heat flow database\DIFdelbegin \DIFdel{(ThermoGlobe, }\DIFdelend \DIFaddbegin \DIFadd{, }\emph{\DIFadd{ThermoGlobe}} \DIFadd{(}\DIFaddend \protect\hyperlink{ref-jennings2021}{Jennings \& Hasterok, 2021}; \protect\hyperlink{ref-lucazeau2019}{Lucazeau, 2019})\DIFdelbegin \DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{While ThermoGlobe }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend currently contains 69,729 observations\DIFdelbegin \DIFdel{, the regional }\DIFdelend \DIFaddbegin \DIFadd{. Yet the spatial }\DIFaddend coverage near subduction zones is \DIFdelbegin \DIFdel{uneven and irregularly-spaced (Table \ref{tab:hfSummaryTable} \& Figure \ref{fig:globalhfComp} ). Many methods may be considered to interpolate }\DIFdelend \DIFaddbegin \DIFadd{relatively sparse (n = 13,368 for this study) and highly-irregular at the regional scale (10\(^2\) to 10\(^3\) \(km\), see Figure \ref{fig:globalhfComp} \& Table \ref{tab:hfSummaryTable}). Note that ThermoGlobe includes many datasets of high-resolution }\DIFaddend surface heat flow \DIFdelbegin \DIFdel{including inverse distance weighting, various splines and linear regressions, nearest-neighbor, general additive models, Kriging, and Similarity, to name a few. Generally speaking, these methods predict }\DIFdelend \DIFaddbegin \DIFadd{arrays, often collocated with seismic arrays, that span \(\leq\) 10\(^2\) \(km\) in total length. While high-resolution surveys can resolve fine spatial variations in }\DIFaddend surface heat flow \DIFdelbegin \DIFdel{across a grid of arbitrarily defined locations with unknown }\DIFdelend \DIFaddbegin \DIFadd{at study sites, probing variations along a subduction zone segment requires evaluation of ThermoGlobe data across larger-scales. Thus, the primary challenge in quantifying segment-scale }\DIFaddend surface heat flow \DIFdelbegin \DIFdel{(the targets) by fitting mathematical models to available }\DIFdelend \DIFaddbegin \DIFadd{variations is evaluating sparse, irregularly-spaced observations separated by distances from 10\(^{-1}\) to 10\(^3\) \(km\). The solution implemented by this study involves two steps: (1) interpolating irregularly-spaced ThermoGlobe data onto a regular grid, and then (2) sampling the interpolated }\DIFaddend surface heat flow \DIFdelbegin \DIFdel{observations (the examples).
}\DIFdelend \DIFaddbegin \DIFadd{across adjacent upper-plate sectors.
}
...
\caption[Similarity interpolation of near 13 active subduction zone segments]{\DIFaddFL{Regional surface heat flow near subduction zone segments. (a) ThermoGlobe data from Lucazeau (}\protect\hyperlink{ref-lucazeau2019}{2019}\DIFaddFL{) cropped within 1000 \(km\)-radius circular buffers around 13 active subduction zone segments show uneven regional coverage. For example, note the relatively high observational density in the NW Pacific compared to other regions. (b) In contrast, a Similarity interpolation cropped within the same buffers presents an evenly-distributed approximation of regional surface heat flow. Similarity interpolation from Lucazeau (}\protect\hyperlink{ref-lucazeau2019}{2019}\DIFaddFL{). Subduction zone boundaries (bold white lines) and volcanoes (gold diamonds) defined by Syracuse \& Abers (}\protect\hyperlink{ref-syracuse2006}{2006}\DIFaddFL{). Plate boundaries (bold black lines) defined by Lawver et al. (}\protect\hyperlink{ref-lawver2018}{2018}\DIFaddFL{). AA: Alaska Aleutians, AN: Andes, CA: Central America, KM: Kamchatka Marianas, KR: Kyushu Ryukyu, LA: Lesser Antilles, NBS: New Britain Solomon, NP: N Philippines, SBS: Sumatra Banda Sea, SC: Scotia, SP: S Philippines, TNZ: Tonga New Zealand, VN: Vanuatu.}}\label{fig:globalhfComp}
\end{figure}

\DIFaddend The two interpolation methods compared in this study, \DIFdelbegin \DIFdel{Kriging and Similarity}\DIFdelend \DIFaddbegin \emph{\DIFadd{Kriging}} \DIFadd{and }\emph{\DIFadd{Similarity}}\DIFaddend , are chosen because they represent end-member \DIFdelbegin \DIFdel{methodological }\DIFdelend classes based on fundamentally different principles \DIFaddbegin \DIFadd{and mathematical frameworks}\DIFaddend . Their comparative differences, therefore, may be important for \DIFdelbegin \DIFdel{advancing understanding of subduction zone processes, for targeting areas for future heat flow research, for improving either method in the context of subduction zone research, and for developing new layered interpolation methods that combine }\DIFdelend \DIFaddbegin \DIFadd{understanding lithospheric thermal structure, identifying surface heat flow anomalies, evaluating practical limitations of each approach, and developing new methods combining }\DIFaddend the strengths of Kriging and Similarity techniques. \DIFaddbegin \DIFadd{The general rationale for applying Kriging and Similarity methods are embodied in the First and Third Laws of Geography, respectively:
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \begin{table}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{quote}
\uline{Three Laws of Geography}\DIFaddFL{:
}\DIFaddendFL 

\DIFdelbeginFL %DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{tab:hfSummaryTable}%%%
\DIFdelFL{ThermoGlobe heat flow summary by segment}}
%DIFAUXCMD
%DIFDELCMD < \centering
%DIFDELCMD < \begin{threeparttable}
%DIFDELCMD < \begin{tabular}[t]{lrrrrrrr}
%DIFDELCMD < \toprule
%DIFDELCMD < %%%
\DIFdelFL{Segment }%DIFDELCMD < & %%%
\DIFdelFL{n }%DIFDELCMD < & %%%
\DIFdelFL{Min }%DIFDELCMD < & %%%
\DIFdelFL{Max }%DIFDELCMD < & %%%
\DIFdelFL{Median }%DIFDELCMD < & %%%
\DIFdelFL{IQR }%DIFDELCMD < & %%%
\DIFdelFL{Mean }%DIFDELCMD < & %%%
\DIFdelFL{$\sigma$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1. Everything is related, but nearer things are more related}\DIFaddendFL \\
\DIFdelbeginFL %DIFDELCMD < \midrule
%DIFDELCMD < \cellcolor{gray!6}{Alaska Aleutians} & \cellcolor{gray!6}{290} & \cellcolor{gray!6}{6} & \cellcolor{gray!6}{196} & \cellcolor{gray!6}{66} & \cellcolor{gray!6}{27} & \cellcolor{gray!6}{71} & \cellcolor{gray!6}{28}%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\hspace*{0.333em}\hspace*{0.333em}\hspace*{0.333em}(}\protect\hyperlink{ref-krige1951}{Krige, 1951}\DIFaddFL{; }\protect\hyperlink{ref-matheron1963}{Matheron, 1963}\DIFaddFL{)}\DIFaddendFL \\
\DIFdelbeginFL \DIFdelFL{Andes }%DIFDELCMD < & %%%
\DIFdelFL{1398 }%DIFDELCMD < & %%%
\DIFdelFL{7 }%DIFDELCMD < & %%%
\DIFdelFL{250 }%DIFDELCMD < & %%%
\DIFdelFL{108 }%DIFDELCMD < & %%%
\DIFdelFL{120 }%DIFDELCMD < & %%%
\DIFdelFL{119 }%DIFDELCMD < & %%%
\DIFdelFL{66}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2. Geographic phenomena are inherently heterogeneous}\DIFaddendFL \\
\DIFdelbeginFL %DIFDELCMD < \cellcolor{gray!6}{Central America} & \cellcolor{gray!6}{1443} & \cellcolor{gray!6}{8} & \cellcolor{gray!6}{250} & \cellcolor{gray!6}{89} & \cellcolor{gray!6}{123} & \cellcolor{gray!6}{110} & \cellcolor{gray!6}{67}%%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\hspace*{0.333em}\hspace*{0.333em}\hspace*{0.333em}(}\protect\hyperlink{ref-goodchild2004}{Goodchild, 2004}\DIFaddFL{)}\DIFaddendFL \\
\DIFdelbeginFL \DIFdelFL{Kamchatka Marianas }%DIFDELCMD < & %%%
\DIFdelFL{2269 }%DIFDELCMD < & %%%
\DIFdelFL{1 }%DIFDELCMD < & %%%
\DIFdelFL{248 }%DIFDELCMD < & %%%
\DIFdelFL{78 }%DIFDELCMD < & %%%
\DIFdelFL{51 }%DIFDELCMD < & %%%
\DIFdelFL{83 }%DIFDELCMD < & %%%
\DIFdelFL{42}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{3. Points with similar geographic configurations share other attributes}\DIFaddendFL \\
\DIFdelbeginFL %DIFDELCMD < \cellcolor{gray!6}{Kyushu Ryukyu} & \cellcolor{gray!6}{1895} & \cellcolor{gray!6}{3} & \cellcolor{gray!6}{250} & \cellcolor{gray!6}{76} & \cellcolor{gray!6}{42} & \cellcolor{gray!6}{84} & \cellcolor{gray!6}{42}\\
%DIFDELCMD < %%%
\DIFdelFL{Lesser Antilles }%DIFDELCMD < & %%%
\DIFdelFL{3010 }%DIFDELCMD < & %%%
\DIFdelFL{13 }%DIFDELCMD < & %%%
\DIFdelFL{242 }%DIFDELCMD < & %%%
\DIFdelFL{41 }%DIFDELCMD < & %%%
\DIFdelFL{8 }%DIFDELCMD < & %%%
\DIFdelFL{46 }%DIFDELCMD < & %%%
\DIFdelFL{18}%DIFDELCMD < \\
%DIFDELCMD < \cellcolor{gray!6}{N Philippines} & \cellcolor{gray!6}{568} & \cellcolor{gray!6}{3} & \cellcolor{gray!6}{231} & \cellcolor{gray!6}{71} & \cellcolor{gray!6}{26} & \cellcolor{gray!6}{75} & \cellcolor{gray!6}{33}\\
%DIFDELCMD < %%%
\DIFdelFL{New Britain Solomon }%DIFDELCMD < & %%%
\DIFdelFL{101 }%DIFDELCMD < & %%%
\DIFdelFL{3 }%DIFDELCMD < & %%%
\DIFdelFL{143 }%DIFDELCMD < & %%%
\DIFdelFL{58 }%DIFDELCMD < & %%%
\DIFdelFL{34 }%DIFDELCMD < & %%%
\DIFdelFL{61 }%DIFDELCMD < & %%%
\DIFdelFL{26}%DIFDELCMD < \\
%DIFDELCMD < \cellcolor{gray!6}{S Philippines} & \cellcolor{gray!6}{460} & \cellcolor{gray!6}{1} & \cellcolor{gray!6}{224} & \cellcolor{gray!6}{71} & \cellcolor{gray!6}{32} & \cellcolor{gray!6}{74} & \cellcolor{gray!6}{34}\\
%DIFDELCMD < %%%
\DIFdelFL{Scotia }%DIFDELCMD < & %%%
\DIFdelFL{25 }%DIFDELCMD < & %%%
\DIFdelFL{13 }%DIFDELCMD < & %%%
\DIFdelFL{145 }%DIFDELCMD < & %%%
\DIFdelFL{81 }%DIFDELCMD < & %%%
\DIFdelFL{62 }%DIFDELCMD < & %%%
\DIFdelFL{79 }%DIFDELCMD < & %%%
\DIFdelFL{43}%DIFDELCMD < \\
%DIFDELCMD < \cellcolor{gray!6}{Sumatra Banda Sea} & \cellcolor{gray!6}{1418} & \cellcolor{gray!6}{1} & \cellcolor{gray!6}{247} & \cellcolor{gray!6}{59} & \cellcolor{gray!6}{63} & \cellcolor{gray!6}{68} & \cellcolor{gray!6}{42}\\
%DIFDELCMD < %%%
\DIFdelFL{Tonga New Zealand }%DIFDELCMD < & %%%
\DIFdelFL{354 }%DIFDELCMD < & %%%
\DIFdelFL{5 }%DIFDELCMD < & %%%
\DIFdelFL{218 }%DIFDELCMD < & %%%
\DIFdelFL{49 }%DIFDELCMD < & %%%
\DIFdelFL{40 }%DIFDELCMD < & %%%
\DIFdelFL{59 }%DIFDELCMD < & %%%
\DIFdelFL{37}%DIFDELCMD < \\
%DIFDELCMD < \cellcolor{gray!6}{Vanuatu} & \cellcolor{gray!6}{137} & \cellcolor{gray!6}{2} & \cellcolor{gray!6}{223} & \cellcolor{gray!6}{61} & \cellcolor{gray!6}{62} & \cellcolor{gray!6}{80} & \cellcolor{gray!6}{52}\\
%DIFDELCMD < \bottomrule
%DIFDELCMD < \end{tabular}
%DIFDELCMD < \begin{tablenotes}
%DIFDELCMD < \item \uline{\textit{key}}%%%
\DIFdelFL{: n: }%DIFDELCMD < [%%%
\DIFdelFL{\# of observations}%DIFDELCMD < ]%%%
\DIFdelFL{, all other units are in $mWm^{-2}$
}%DIFDELCMD < \item \uline{\textit{note}}%%%
\DIFdelFL{: ThermoGlobe data are filtered for quality, restricted to }%DIFDELCMD < [%%%
\DIFdelFL{0, 250) $mWm^{-2}$, and cropped within 1000 $km$-radius circular buffers of segment boundaries
}%DIFDELCMD < \end{tablenotes}
%DIFDELCMD < \end{threeparttable}
%DIFDELCMD < \end{table}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{\hspace*{0.333em}\hspace*{0.333em}\hspace*{0.333em}(}\protect\hyperlink{ref-zhu2018}{Zhu et al., 2018}\DIFadd{)
}\end{quote}
\DIFaddend 
...
%DIFDELCMD < {\centering \includegraphics[width=1\linewidth,]{assets/figs/chpt3/ThermoGlobeBufferComp} 
%DIFDELCMD < 
...
%DIFDELCMD < %%%
%DIFDELCMD < \caption[Similarity interpolation of near 13 active subduction zone segments]{%
{%DIFAUXCMD
\DIFdelFL{Surface heat flow near subduction zone segments. (a) ThermoGlobe data from Lucazeau (}%DIFDELCMD < \protect\hyperlink{ref-lucazeau2019}{2019}%%%
\DIFdelFL{) cropped within 1000 \(km\)-radius circular buffers around 13 active subduction zone segments show uneven regional coverage. For example, note the relatively high observational density in the NW Pacific compared to other regions. (b) In contrast, Similarity interpolation cropped within the same buffers presents an evenly-distributed approximation of regional surface heat flow. Similarity interpolation from Lucazeau (}%DIFDELCMD < \protect\hyperlink{ref-lucazeau2019}{2019}%%%
\DIFdelFL{). Subduction zone boundaries (bold white lines) defined by Syracuse \& Abers (}%DIFDELCMD < \protect\hyperlink{ref-syracuse2006}{2006}%%%
\DIFdelFL{). Plate boundaries (solid green lines) defined by Lawver et al. (}%DIFDELCMD < \protect\hyperlink{ref-lawver2018}{2018}%%%
\DIFdelFL{). Segments: AA }%DIFDELCMD < {[}%%%
\DIFdelFL{Alaska Aleutians}%DIFDELCMD < {]}%%%
\DIFdelFL{, AN }%DIFDELCMD < {[}%%%
\DIFdelFL{Andes}%DIFDELCMD < {]}%%%
\DIFdelFL{, CA }%DIFDELCMD < \protect\hyperlink{central-america}{Central America}%%%
\DIFdelFL{, KM }%DIFDELCMD < {[}%%%
\DIFdelFL{Kamchatka Marianas}%DIFDELCMD < {]}%%%
\DIFdelFL{, KR }%DIFDELCMD < \protect\hyperlink{kyushu-ryukyu}{Kyushu Ryukyu}%%%
\DIFdelFL{, LA }%DIFDELCMD < {[}%%%
\DIFdelFL{Lesser Antilles}%DIFDELCMD < {]}%%%
\DIFdelFL{, NBS }%DIFDELCMD < {[}%%%
\DIFdelFL{New Britain Solomon}%DIFDELCMD < {]}%%%
\DIFdelFL{, NP }%DIFDELCMD < {[}%%%
\DIFdelFL{N Philippines}%DIFDELCMD < {]}%%%
\DIFdelFL{, SBS }%DIFDELCMD < {[}%%%
\DIFdelFL{Sumatra Banda Sea}%DIFDELCMD < {]}%%%
\DIFdelFL{, SC }%DIFDELCMD < \protect\hyperlink{scotia}{Scotia}%%%
\DIFdelFL{, SP }%DIFDELCMD < {[}%%%
\DIFdelFL{S Philippines}%DIFDELCMD < {]}%%%
\DIFdelFL{, TNZ }%DIFDELCMD < {[}%%%
\DIFdelFL{Tonga New Zealand}%DIFDELCMD < {]}%%%
\DIFdelFL{, VN }%DIFDELCMD < {[}%%%
\DIFdelFL{Vanuatu}%DIFDELCMD < {]}%%%
\DIFdelFL{.}}%DIFAUXCMD
%DIFDELCMD < \label{fig:globalhfComp}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{In one class of methods (including Kriging)
, interpolation involves adjusting certain model parameters that depend on the relative positions of examples and targets. Because model parameters are spatially dependent, these methods inherently attempt to conserve the }\DIFdelend \DIFaddbegin \DIFadd{Generally speaking, the }\DIFaddend spatial continuity of \DIFdelbegin \DIFdel{observed }\DIFdelend surface heat flow \DIFdelbegin \DIFdel{. Kriging, therefore, predicts a more accurate (or truthful) }\emph{\DIFdel{on-the-ground}} %DIFAUXCMD
\DIFdel{distribution of }\DIFdelend \DIFaddbegin \DIFadd{reflects variations in subsurface thermal structure (including radiogenic heat production) and heat-transferring processes. For example, broad regions of low }\DIFaddend surface heat flow \DIFdelbegin \DIFdel{compared to Similarity methods (described below). The rationale for using Kriging is embodied in the First Law of Geography: }\emph{\DIFdel{everything is related, but nearer things are more related}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{on continents outline cratons (}\protect\hyperlink{ref-nyblade1993}{Nyblade \& Pollack, 1993}\DIFadd{), anomalously low surface heat flow in young oceanic crust imply significant heat extraction by seawater }\DIFaddend (\protect\DIFdelbegin %DIFDELCMD < \hyperlink{ref-krige1951}{Krige, 1951}%%%
\DIFdelend \DIFaddbegin \hyperlink{ref-fisher2000}{Fisher \& Becker, 2000}\DIFaddend ; \protect\DIFdelbegin %DIFDELCMD < \hyperlink{ref-matheron1963}{Matheron, 1963}%%%
\DIFdel{).
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{However, ordinary Kriging must satisfy a condition known as }\emph{\DIFdel{second order weak stationarity}}%DIFAUXCMD
\DIFdel{. That is, the relationship among }\DIFdelend \DIFaddbegin \hyperlink{ref-hasterok2011}{Hasterok et al., 2011}\DIFadd{; }\protect\hyperlink{ref-hutnak2008}{Hutnak et al., 2008}\DIFadd{; }\protect\hyperlink{ref-stein1994}{Stein \& Stein, 1994}\DIFadd{), and trench-orthogonal }\DIFaddend surface heat flow \DIFdelbegin \DIFdel{observations is constant and only depends on the relative separation (lag) distance between pairs of observations. This assumption may be violated within tectonically-diverse regions where the underlying geodynamics may quickly transition from subduction to spreading centers, for example. However, stationarity need only apply over the local Krigingdomain, so stationarity can be maintained by careful choice of Kriging parameters.
Although not necessary in this study, stationarity assumptions can relax and other techniques that respect the Second Law of Geography (}\emph{\DIFdel{spatial phenomena are inherently heterogeneous}}%DIFAUXCMD
\DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{profiles imply uniform upper-plate lithospheric thickness (}\DIFaddend \protect\DIFdelbegin %DIFDELCMD < \hyperlink{ref-goodchild2004}{Goodchild, 2004}%%%
\DIFdel{), such as directional Kriging or Markov-Bayes techniques (}\DIFdelend \DIFaddbegin \hyperlink{ref-currie2004}{Currie et al., 2004}\DIFadd{; }\DIFaddend \protect\DIFdelbegin %DIFDELCMD < \hyperlink{ref-bardossy1997}{Bárdossy, 1997}%%%
\DIFdel{) , can be applied.
}\DIFdelend \DIFaddbegin \hyperlink{ref-currie2006}{Currie \& Hyndman, 2006}\DIFadd{; }\protect\hyperlink{ref-hyndman2005}{Hyndman et al., 2005}\DIFadd{) and mechanical coupling depths (}\protect\hyperlink{ref-furukawa1993}{Furukawa, 1993}\DIFadd{; }\protect\hyperlink{ref-kerswell2020}{Kerswell et al., 2020}\DIFadd{; }\protect\hyperlink{ref-wada2009}{Wada \& Wang, 2009}\DIFadd{) among subduction zones. For Kriging, such patterns and anomalies may be resolved (assuming adequate observational coverage) because the method is inherently dependent on the spatial continuity of observed surface heat flow.
}\DIFaddend 

\DIFdelbegin \DIFdel{In stark contrast to }\DIFdelend \DIFaddbegin \DIFadd{In contrast, Similarity may impose different patterns than Kriging because the method only depends on the similarity between two points in terms of their }\emph{\DIFadd{geographic configuration}} \DIFadd{(the makeup and structure of geographic variables over some spatial neighbourhood around a point, }\protect\hyperlink{ref-zhu2018}{Zhu et al., 2018}\DIFadd{). Rather than interpolating (}\emph{\DIFadd{sensu stricto}}\DIFadd{) like }\DIFaddend Kriging, Similarity \DIFdelbegin \DIFdel{methods are spatially independent and predict }\DIFdelend \DIFaddbegin \DIFadd{predicts }\DIFaddend surface heat flow by \DIFdelbegin \DIFdel{integrating }\DIFdelend \DIFaddbegin \DIFadd{comparing }\DIFaddend geographic, geologic, geochronologic, and geophysical information \DIFaddbegin \DIFadd{between a target point and the entire ThermoGlobe dataset }\DIFaddend (see \protect\hyperlink{ref-goutorbe2011}{Goutorbe et al., 2011} for method details). \DIFdelbegin \DIFdel{The technique involves evaluating the number of similar observable geologic proxies between a target and }\emph{\DIFdel{every}} %DIFAUXCMD
\DIFdel{example in the entire global dataset (e. g.~a target and an exampleare similar }\DIFdelend \DIFaddbegin \DIFadd{Similarity predictions are fundamentally geologically-reasoned estimates of surface heat flow. For example, two points have similar surface heat flow }\DIFaddend if they have similar bathymetry, \DIFdelbegin \DIFdel{elevation}\DIFdelend \DIFaddbegin \DIFadd{lithology}\DIFaddend , proximity to active or ancient orogens, seafloor age, upper mantle shear wave \DIFdelbegin \DIFdel{velocities}\DIFdelend \DIFaddbegin \DIFadd{velocity}\DIFaddend , etc. \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend \protect\hyperlink{ref-chapman1975}{Chapman \& Pollack, 1975}; \protect\hyperlink{ref-davies2013}{Davies, 2013}; \protect\hyperlink{ref-lee1965}{Lee \& Uyeda, 1965}; \protect\hyperlink{ref-lucazeau2019}{Lucazeau, 2019}; \protect\hyperlink{ref-sclater1970}{Sclater \& Francheteau, 1970}; \protect\hyperlink{ref-shapiro2004}{Shapiro \& Ritzwoller, 2004}).
\DIFdelbegin \DIFdel{Surface heat flow is then predicted at a target location as a function of the target's similarity with each example and the surface heat flow observed at each example, irrespective of their locations. Global-scale problems and regional domains with relatively few surface heat flow data are particularly suited for Similarity as the method is independent of observational density. The rationale for using Similarity is embodied in the Third Law of Geography: }\emph{\DIFdel{the more similar the geographic configuration of two points, the more similar their values}} %DIFAUXCMD
\DIFdel{(}%DIFDELCMD < \protect\hyperlink{ref-zhu2018}{Zhu et al., 2018}%%%
\DIFdel{).
}\DIFdelend 

\DIFdelbegin \DIFdel{However, it is especially notable for regional geodynamic investigations that Kriging is explicitly spatially dependent, whereas Similarity is not. Generally speaking, spatial (dis)continuity of surface heat flow observations reflect the (dis)continuity of subsurface geodynamics (assuming insignificant near-surface perturbations). For example, broad regions of low surface heat flow on continents outline cratons and repeated surface heat flow patterns near subduction zones are interpreted to reflect uniform upper-plate lithospheric thickness (}%DIFDELCMD < \protect\hyperlink{ref-currie2004}{Currie et al., 2004}%%%
\DIFdel{; }%DIFDELCMD < \protect\hyperlink{ref-currie2006}{Currie \& Hyndman, 2006}%%%
\DIFdel{; }%DIFDELCMD < \protect\hyperlink{ref-hyndman2005}{Hyndman et al., 2005}%%%
\DIFdel{) and mechanical coupling depths (}%DIFDELCMD < \protect\hyperlink{ref-furukawa1993}{Furukawa, 1993}%%%
\DIFdel{; }%DIFDELCMD < \protect\hyperlink{ref-kerswell2020}{Kerswell et al., 2020}%%%
\DIFdel{; }%DIFDELCMD < \protect\hyperlink{ref-wada2009}{Wada \& Wang, 2009}%%%
\DIFdel{). While Similarity may be accurate in many cases, simply imposing surface heat flow by layering geologic proxies without explicitly considering the spatial continuity of surface heat flow observations is concerning for at least two reasons. It may 1) discount random unpredictable subsurface geodynamics, and/or 2) discount random unpredictable near-surface perterbations. Either case leads to erroneous surface heat flow inversions and interpretations of subsurface geodynamics. It is therefore crucial for regional geodynamic investigations to characterize the true spatial continuity of surface heat flow observations whenever possible.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{This Chapter compares surface heat flow interpolations produced by }\DIFdelend \DIFaddbegin \DIFadd{This study compares regional }\DIFaddend Similarity and Kriging \DIFdelbegin \DIFdel{methods for }\DIFdelend \DIFaddbegin \DIFadd{interpolations near }\DIFaddend 13 presently active subduction zones while considering the following questions\DIFdelbegin \DIFdel{. How }\DIFdelend \DIFaddbegin \DIFadd{: (1) how }\DIFaddend does surface heat flow vary \DIFdelbegin \DIFdel{in 2D }\DIFdelend near subduction zones, especially \DIFdelbegin \DIFdel{along strike within upper-plates? Are }\DIFdelend \DIFaddbegin \DIFadd{within the upper-plate? (2) How do }\DIFaddend Kriging and Similarity \DIFdelbegin \DIFdel{methods comparable when applied to surface heat flow near subduction zones? }\DIFdelend \DIFaddbegin \DIFadd{predictions compare? (3) }\DIFaddend What do the differences (if any) imply about geodynamic variability among active subduction zones\DIFdelbegin \DIFdel{, especially with respect to uniform upper-plate thickness discussed in Chapter \ref{chpt2}? }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{? }\DIFaddend First, ordinary Kriging is applied to \DIFdelbegin \DIFdel{the ThermoGlobe dataset (}%DIFDELCMD < \protect\hyperlink{ref-jennings2021}{Jennings \& Hasterok, 2021}%%%
\DIFdel{; }%DIFDELCMD < \protect\hyperlink{ref-lucazeau2019}{Lucazeau, 2019}%%%
\DIFdel{) cropped within 1000 \(km\)-radius circular buffers around }\DIFdelend \DIFaddbegin \DIFadd{ThermoGlobe data near }\DIFaddend 13 presently active subduction \DIFdelbegin \DIFdel{zones defined by Syracuse \& Abers (}\DIFdelend \DIFaddbegin \DIFadd{zone segments (defined by }\DIFaddend \protect\DIFdelbegin %DIFDELCMD < \hyperlink{ref-syracuse2006}{2006}%%%
\DIFdelend \DIFaddbegin \hyperlink{ref-syracuse2006}{Syracuse \& Abers, 2006}\DIFaddend ). Kriging predictions are then directly compared (point-by-point) to Similarity \DIFdelbegin \DIFdel{interpolations }\DIFdelend \DIFaddbegin \DIFadd{predictions }\DIFaddend from a previous global-scale study by Lucazeau (\protect\hyperlink{ref-lucazeau2019}{2019}). \DIFdelbegin \DIFdel{The results show Kriging and Similarity methods are grossly comparable at regional scales (\(\geq\) 100's of \(km\)), but more importantly, show distinct differences in }\DIFdelend \DIFaddbegin \DIFadd{Interpolation comparisons yield a Kaleidoscope of predicted }\DIFaddend upper-plate surface heat flow \DIFdelbegin \DIFdel{for 7 out of 13 subduction zone segments. Average }\DIFdelend \DIFaddbegin \DIFadd{distributions and profiles. The implications of mixed }\DIFaddend upper-plate \DIFdelbegin \DIFdel{surface heat flow can vary up to 50 \(mWm^{-2}\) across a single subduction zone segment---implying non-uniform upper-plate thickness in contradiction to expectations from }\DIFdelend \DIFaddbegin \DIFadd{profiles are discussed, especially with respect to uniform lithospheric thickness discussed in }\DIFaddend Chapter \ref{chpt2}.
\DIFdelbegin \DIFdel{More subtle differences between Kriging and Similarity results point to inherent methodological strengths that may be combined in layered interpolation methods.
}\DIFdelend 
...
Like Lucazeau (\protect\hyperlink{ref-lucazeau2019}{2019}), 4,661 poor quality observations (\texttt{Code\ 6} = D), 350 data points without heat flow observations, and 2 without geographic information are excluded from the analysis. Note that quality control of such a large dataset is an ongoing endeavor and 11,712 observations currently have an undetermined quality (\texttt{Code\ 6} = Z). Duplicate observations at the same location are parsed (to avoid singular covariance matrices during Kriging) by selecting only the best quality measurement. If duplicate measurements are of equal quality, one is randomly chosen. Finally, \DIFdelbegin \DIFdel{only }\DIFdelend surface heat flow observations \DIFdelbegin \DIFdel{in the range (0 - 250}%DIFDELCMD < {]} %%%
\DIFdel{\(mWm^{-2}\) are included in Kriging . Similarity interpolations from Lucazeau (}%DIFDELCMD < \protect\hyperlink{ref-lucazeau2019}{2019}%%%
\DIFdel{) is also }\DIFdelend \DIFaddbegin \DIFadd{for Kriging and Similarity predictions are both }\DIFaddend limited to the range (0 - 250{]} \(mWm^{-2}\). Observations outside of the range (0 - 250{]} \(mWm^{-2}\) are considered anomalous \DIFaddbegin \DIFadd{high }\DIFaddend (e.g.~collected near geothermal systems, \protect\hyperlink{ref-lucazeau2019}{Lucazeau, 2019}) and \DIFaddbegin \DIFadd{unrepresentative of lithospheric-scale thermal structure. Moreover, anomalously high observations }\DIFaddend constitute a small fraction of measurements (4,883 out of 69,729) forming long tails on either side of the global surface heat flow distribution. The final dataset used for Kriging contains 13,368 observations after filtering for quality, missing values, and heat flow range, parsing duplicate pairs, and cropping within subduction zone buffers (\DIFdelbegin \DIFdel{Table \ref{tab:hfSummaryTable} \& Figure \ref{fig:hfSummaryPlot} }\DIFdelend \DIFaddbegin \DIFadd{Figure \ref{fig:hfSummaryPlot} \& Table \ref{tab:hfSummaryTable}}\DIFaddend ).

\DIFdelbegin %DIFDELCMD < \begin{figure}[htbp]
%DIFDELCMD < 

%DIFDELCMD < {\centering \includegraphics[width=1\linewidth,]{assets/figs/chpt3/hfSummary} 
%DIFDELCMD < 
...
%DIFDELCMD < %%%
%DIFDELCMD < \caption[Distributions of ThermoGlobe observations near 13 active subduction zone segments]{%
{%DIFAUXCMD
\DIFdelFL{Distribution of ThermoGlobe observations from Lucazeau (}%DIFDELCMD < \protect\hyperlink{ref-lucazeau2019}{2019}%%%
\DIFdelFL{) cropped within 1000 \(km\)-radius circular buffers around 13 active subduction zone segments. Heat flow distributions are centered between 41 and 108 \(mWm^{-2}\) and highly right-skewed. Skewness represents near-surface perturbations from geothermal systems and tectonic regions with high thermal activity. Diverse interactions between tectonic and near-surface processes are apparent from the broad, irregular surface heat flow distributions. Distributions are colored by quartiles (25\%, 50\%, 75\%).}}%DIFAUXCMD
%DIFDELCMD < \label{fig:hfSummaryPlot}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \hypertarget{map-projection-and-interpolation-grid}{%
...
{\centering \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.6\linewidth,]{assets/figs/chpt3/KyushuRyukyuComp} 
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1\linewidth,]{assets/figs/chpt3/SumatraBandaSeaInterp} 
\DIFaddendFL 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{tab:vgrmSummaryTable}%%%
\DIFdelFL{Optimum variogram models by segment}}
%DIFAUXCMD
%DIFDELCMD < \centering
...
%DIFDELCMD < \hypertarget{interpolation-accuracy}{%
%DIFDELCMD < \subsection{Interpolation Accuracy}\label{interpolation-accuracy}}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Regional Kriging errors from 9.7 to 48 \(mWm^{-2}\) all exceed the maximum regional Similarity errors from 25.8 \(mWm^{-2}\) (Table \ref{tab:diffSummaryTable}). Kriging errors can be relatively small compared to Similarity for domains with high observational density (Lesser Antilles; n = 3, 010, \(\Delta\)RMSE = -2.7) but relatively large where observational density is comparatively low (Vanuatu; n = 137, \(\Delta\)RMSE = 30.2). On average, Kriging error rates are 1.1 times higher than Similarity. Moreover, regional Similarity errors in this study are consistent with, or smaller than, the 7 \(mWm^{-2}\) error reported by Lucazeau (}\DIFdelend \DIFaddbegin \DIFadd{This study interpolates ThermoGlobe data onto interpolation grid points (option 2 from above) with a weighted average applied to all surface heat flow observations found within a 55 \(km\) radius of an interpolation grid point (approximately 0.5\(^\circ\) radius circle at the equator). However, instead of applying equal weights (arithmetic mean) as Lucazeau (}\DIFaddend \protect\hyperlink{ref-lucazeau2019}{2019})\DIFdelbegin \DIFdel{for their preferred global model (with 7 proxy observables) }\DIFdelend \DIFaddbegin \DIFadd{, this study applies weights inversely proportional to the distance between an observation and grid point (after }\protect\hyperlink{ref-shepard1968}{Shepard, 1968}\DIFadd{):
}\begin{equation}
  \DIFadd{\begin{aligned}
    Z(u_c) =&
    \begin{cases}
...
\DIFadd{where \(Z(u_c)\) is the expected surface heat flow at a grid point \(u_c\), \(\hat{Z}(u_i)\) is the weighted average of all surface heat flow observed at location \(u_i\), \(N\) is the number of observations contained within 55 \(km\) (approximately 0.5\(^\circ\)) of the grid point, \(\omega_i\) is the weight applied to observation \(Z(u_i)\), \(d(u_i, u_c)\) is a function measuring the distance between observation location \(u_i\) and grid point \(u_c\), and \(p=2\) is a power constant affecting the smoothness of the interpolation}\DIFaddend .

\DIFdelbegin %DIFDELCMD < \begin{table}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \hypertarget{chpt3Results}{%
...
%DIFDELCMD < \end{table}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{interpDiff}{%
\subsection{Similarity and Kriging Interpolations}\label{interpDiff}}
\DIFaddend 

\DIFdelbegin %DIFDELCMD < \hypertarget{interpolation-comparison}{%
%DIFDELCMD < \subsection{Interpolation Comparison}\label{interpolation-comparison}}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \hypertarget{global-differences}{%
\subsubsection{Global Differences}\label{global-differences}}

Global differences between Similarity and Kriging interpolations across all subduction zone segments are centered near zero with median differences ranging from -2 to 14 \(mWm^{-2}\), but broadly distributed with \DIFdelbegin \DIFdel{interquartile ranges }\DIFdelend \DIFaddbegin \glspl{iqr} \DIFaddend from 15 to 47 \(mWm^{-2}\) and long tails extending from -147 to 205 \(mWm^{-2}\) (Table \ref{tab:diffSummaryTable}). Distributions of interpolation differences are either approximately symmetrical, or slightly right-skewed \DIFaddbegin \DIFadd{(Figure \ref{fig:diffSummaryPlot})}\DIFaddend . Slight right skew and positive \DIFdelbegin \DIFdel{medians }\DIFdelend \DIFaddbegin \DIFadd{median differences }\DIFaddend indicate a general tendency \DIFdelbegin \DIFdel{of Similarity }\DIFdelend to predict higher surface heat flow \DIFaddbegin \DIFadd{by Similarity }\DIFaddend compared to Kriging. \DIFdelbegin \DIFdel{While slight right-skew and positive medians are common among segments, only Central America is strongly right-skewed and tonga New Zealand has a slightly negative median, indicating a general tendency of Kriging to predict higher surface heat flow compared to Similarity (Figure \ref{fig:diffSummaryPlot}).
}%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[htbp]
%DIFDELCMD < 

%DIFDELCMD < {\centering \includegraphics[width=1\linewidth,]{assets/figs/chpt3/interpDiffSummary} 
%DIFDELCMD < 
...
%DIFDELCMD < %%%
%DIFDELCMD < \caption[Differences between Similarity and Kriging interpolations]{%
{%DIFAUXCMD
\DIFdelFL{Differences between Similarity and Kriging interpolations by segment, computed as }%DIFDELCMD < {[}%%%
\DIFdelFL{Similarity\(-\)Krige}%DIFDELCMD < {]}%%%
\DIFdelFL{. Differences are centered near zero with medians ranging from -2 to 14 \(mWm^{-2}\), but broadly distributed with interquartile ranges from 15 to 47 \(mWm^{-2}\) and long tails extending from -147 to 205 \(mWm^{-2}\). Positive medians and right skew indicate a general tendency towards higher surface heat flow predictions by Similarity compared to Kriging. The broadest distributions (Andes and Central America) reflect less subtle differences between methods. Distributions are colored by quartiles (25\%, 50\%, 75\%). Similarity interpolation from Lucazeau (}%DIFDELCMD < \protect\hyperlink{ref-lucazeau2019}{2019}%%%
\DIFdelFL{).}}%DIFAUXCMD
%DIFDELCMD < \label{fig:diffSummaryPlot}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 
...
Examples given in this section highlight the range of differences observed between Similarity and Kriging \DIFdelbegin \DIFdel{across complex tectonic environments }\DIFdelend \DIFaddbegin \DIFadd{interpolations across subduction zone segments with spreading centers }\DIFaddend (Central America), \DIFdelbegin \DIFdel{simple tectonic environments }\DIFdelend \DIFaddbegin \DIFadd{without spreading centers }\DIFaddend (Kyushyu Ryukyu), and regions with very few observations (Scotia). \DIFaddbegin \DIFadd{Please refer to Appendix \ref{interpDiffAppendix} for the remaining set of visualized interpolations.
}\DIFaddend 

\hypertarget{central-america}{%
\paragraph{Central America}\label{central-america}}

\DIFdelbegin \DIFdel{Similarity predicts }\DIFdelend \DIFaddbegin \DIFadd{Distance to plate boundaries is a key geologic proxy used by Similarity to predict surface heat flow. Consequently, Similarity predicts high surface heat flow along }\DIFaddend the arms of the Galápagos triple junction \DIFdelbegin \DIFdel{to the SW of the Central America segment, but Kriging does not }\DIFdelend \DIFaddbegin \DIFadd{and within the converging Cocos Plate }\DIFaddend (Figure \ref{fig:centralAmericaDiff}). \DIFdelbegin \DIFdel{Consequently, }\DIFdelend \DIFaddbegin \DIFadd{Kriging, on the other hand, predicts quite low surface heat flow within the Cocos Plate despite close proximity to the nearby spreading centers. Low surface heat flow observed within }\DIFaddend the \DIFdelbegin \DIFdel{interpolation comparison highlights two distinct regions---large differences along the arms of the triple junction and within the subducting Cocos Plate , and small differences to the E and NE of the }\DIFdelend Cocos Plate \DIFdelbegin \DIFdel{behind the volcanic arc (Figure \ref{fig:centralAmericaDiff}) . Note the moderate interpolation differences within the Cocos Plate in Figure \ref{fig:centralAmericaDiff} where Similarity predicts relatively }\DIFdelend \DIFaddbegin \DIFadd{(e.g. }\protect\hyperlink{ref-hutnak2008}{Hutnak et al., 2008}\DIFadd{) constrains Kriging predictions to similarly low values. Moreover, disagreement between Similarity and Kriging appears along the volcanic arc. While Similarity predicts moderately }\DIFaddend high surface heat flow \DIFdelbegin \DIFdel{by proximity to the nearby spreading centers, but Kriging predictions are relatively low }\DIFdelend \DIFaddbegin \DIFadd{near the volcanic arc region (and within the upper-plate), Kriging predicts moderately low surface heat flow for much of the upper-plate}\DIFaddend .



\begin{figure}[htbp]

{\centering \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.67\linewidth,]{assets/figs/chpt3/CentralAmericaDiffComp} 
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.69\linewidth,]{assets/figs/chpt3/CentralAmericaDiffComp} 
\DIFaddendFL 

}

\caption[Similarity and Kriging interpolations for Central America]{Similarity and Kriging interpolations for Central America. Arms of the Galápagos triple junction (GTJ), the East Pacific Rise (EPR), and Cocos Ridge (CR), are predicted by Similarity (a), but smoothed-out by Kriging (b). Note the differences between predictions within the Cocos Plate (CP) where Similarity predicts relatively high surface heat flow \DIFaddbeginFL \DIFaddFL{(by proximity to spreading centers) }\DIFaddendFL compared to Kriging \DIFaddbeginFL \DIFaddFL{(from low measurements)}\DIFaddendFL . Segment boundary (bold white line) and volcanoes (gold diamonds) defined by Syracuse \& Abers (\protect\hyperlink{ref-syracuse2006}{2006}). Similarity interpolation from Lucazeau (\protect\hyperlink{ref-lucazeau2019}{2019}). Plate boundaries (\DIFdelbeginFL \DIFdelFL{solid green }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{bold black }\DIFaddendFL lines) defined by Lawver et al. (\protect\hyperlink{ref-lawver2018}{2018}).}\label{fig:centralAmericaDiff}
\end{figure}

\hypertarget{kyushu-ryukyu}{%
\paragraph{Kyushu Ryukyu}\label{kyushu-ryukyu}}

\DIFdelbegin \DIFdel{In contrast, Similarity }\DIFdelend \DIFaddbegin \DIFadd{Similarity predicts a very textured distribution of surface heat flow throughout the Kyushu Ryukyu domain (Figure \ref{fig:kyushuRyukyuDiff}). Similarity predictions clearly show the influence of multiple volcanic arc chains, plate boundaries, and the age of the subducting oceanic lithosphere. Kriging predictions corroborate much of the texture predicted by Similarity, especially to the NE of the main Kyushu Ryukyu segment boundary, but show smoothed-out distribution within other regions. Textural differences notwithstanding, regional Similarity }\DIFaddend and Kriging differences are small and narrowly distributed near Kyushu Ryukyu (median difference: 4, IQR: 22 \(mWm^{-2}\)) compared to Central America (median difference: 12, IQR: 47 \(mWm^{-2}\)\DIFaddbegin \DIFadd{; Table \ref{tab:diffSummaryTable}}\DIFaddend ). Kyushyu Ryukyu has a comparable number of observations (n = 1,895) as Central America (n = 1,443), but a simpler \DIFdelbegin \DIFdel{tectonic }\DIFdelend \DIFaddbegin \DIFadd{plate }\DIFaddend configuration (no spreading centers) and more regularly spaced observations. \DIFdelbegin \DIFdel{Kriging near Kyushu Ryukyu corroborates much of the texture predicted in the Similarity interpolation (Figure \ref{fig:kyushuRyukyuDiff}). A combination of high observation density, relatively uniform distribution of measurements, and relatively simple geology cause Similarity and Kriging results to converge}\DIFdelend \DIFaddbegin \DIFadd{Thus, Kyushu Ryukyu characterizes an interpolation domain with a combination of relatively simple plate configuration, relatively high observational density, and relatively regular observational coverage}\DIFaddend .



\begin{figure}[htbp]

{\centering \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.63\linewidth,]{assets/figs/chpt3/KyushuRyukyuDiffComp} 
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.62\linewidth,]{assets/figs/chpt3/KyushuRyukyuDiffComp} 
\DIFaddendFL 

}

\caption[Similarity and Kriging interpolations for Kyushyu Ryukyu]{Similarity and Kriging interpolations for Kyushyu Ryukyu. (a) Similarity predicts a textured interpolation \DIFdelbeginFL \DIFdelFL{within the tectonically complex region}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{that is clearly influenced by multiple volcanic chains and plate boundaries}\DIFaddendFL . (b) \DIFaddbeginFL \DIFaddFL{The }\DIFaddendFL Kriging interpolation is generally smoother, but corroborate much of the same texture predicted by Similarity \DIFdelbeginFL \DIFdelFL{because of }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{due to }\DIFaddendFL relatively high observational density and \DIFaddbeginFL \DIFaddFL{relatively }\DIFaddendFL regular \DIFdelbeginFL \DIFdelFL{spatial }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{observational }\DIFaddendFL coverage \DIFdelbeginFL \DIFdelFL{near Kyushu Ryukyu}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{across the segment}\DIFaddendFL . Segment boundary (bold white line) and volcanoes (gold diamonds) defined by Syracuse \& Abers (\protect\hyperlink{ref-syracuse2006}{2006}). Similarity interpolation from Lucazeau (\protect\hyperlink{ref-lucazeau2019}{2019}). Plate boundaries (\DIFdelbeginFL \DIFdelFL{solid green }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{bold black }\DIFaddendFL lines) defined by Lawver et al. (\protect\hyperlink{ref-lawver2018}{2018}).}\label{fig:kyushuRyukyuDiff}
\end{figure}

\hypertarget{scotia}{%
\paragraph{Scotia}\label{scotia}}

The Scotia segment illustrates a case where surface heat flow observations are incredibly sparse. \DIFdelbegin \DIFdel{Similarity resolves }\DIFdelend \DIFaddbegin \DIFadd{Yet Similarity predicts multiple tectonic features including }\DIFaddend the East Scotia Ridge and the WSW-ENE trending transform boundary separating the Scotia and Sandwich Plates from the Antarctic Plate (Figure \ref{fig:scotiaDiff}). \DIFdelbegin \DIFdel{Kriging indicates a }\DIFdelend \DIFaddbegin \DIFadd{Note that Similarity is constructed from geologic data and is agnostic to observational density. Thus, Similarity highlights plate boundaries, the distribution of seafloor age, and other small differences in other geologic proxies. Kriging, on the other hand, shows a high }\DIFaddend heat flow anomaly more or less in the region of the East Scotia Ridge, \DIFaddbegin \DIFadd{and a few low heat flow anomalies on the Antarctic Plate, }\DIFaddend but does not resolve \DIFaddbegin \DIFadd{any }\DIFaddend structure in a way that is geologically useful\DIFdelbegin \DIFdel{because of very few }\DIFdelend \DIFaddbegin \DIFadd{. Few }\DIFaddend surface heat flow observations (n = 25) \DIFdelbegin \DIFdel{. Kriging predictions thus }\DIFdelend \DIFaddbegin \DIFadd{result in smooth Kriging predictions that }\DIFaddend approximate the expected \DIFaddbegin \DIFadd{value }\DIFaddend (mean: 79 \(mWm^{-2}\)) \DIFdelbegin \DIFdel{value for the entire }\DIFdelend \DIFaddbegin \DIFadd{for most of the }\DIFaddend domain according to Equation \eqref{eq:linEstimate}.
...
\caption[Similarity and Kriging interpolations for Scotia]{Similarity and Kriging interpolations for Scotia. \DIFaddbeginFL \DIFaddFL{Despite incredibly sparse data }\DIFaddendFL (\DIFaddbeginFL \DIFaddFL{n = 25), (}\DIFaddendFL a) Similarity identifies two tectonic features, the East Scotia Ridge (ESR) and a transform fault (TF) separating the Scotia and Sandwich Plates (SP, SAN) from the Antartic Plate (AP). \DIFaddbeginFL \DIFaddFL{(b) }\DIFaddendFL Kriging predicts a high heat flow anomaly in the region of the ESR, \DIFaddbeginFL \DIFaddFL{and a few low heat flow anomalies in the AP, }\DIFaddendFL but otherwise appears featureless \DIFdelbeginFL \DIFdelFL{because of incredibly }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{due to }\DIFaddendFL sparse data. Segment boundary (bold white line) and volcanoes (gold diamonds) defined by Syracuse \& Abers (\protect\hyperlink{ref-syracuse2006}{2006}). Similarity interpolation from Lucazeau (\protect\hyperlink{ref-lucazeau2019}{2019}). Plate boundaries (\DIFdelbeginFL \DIFdelFL{solid green }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{bold black }\DIFaddendFL lines) defined by Lawver et al. (\protect\hyperlink{ref-lawver2018}{2018}).}\label{fig:scotiaDiff}
\end{figure}
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{upSectorsResults}{%
\subsubsection{Upper-Plate Sector Samples}\label{upSectorsResults}}
\DIFaddend 

\DIFdelbegin \DIFdel{Lateral }\DIFdelend \DIFaddbegin \DIFadd{Sampling the interpolation grid and ThermoGlobe data from adjacent upper-plate sectors allows for first-order quantitative evaluation of the along-strike variability in upper-plate }\DIFaddend surface heat flow\DIFdelbegin \DIFdel{variability across upper-plates may be categorized in 1 of 8 ways based on }\DIFdelend \DIFaddbegin \DIFadd{. For example, comparing sectors yields a variety of surface heat flow patterns (Table \ref{tab:sectorSummaryTable}, Figures \ref{fig:kyushuRyukyuUpper}, \ref{fig:sumatraBandaSeaUpper}, \ref{fig:newBritainSolomonUpper} \& Appendix \ref{lateralDiffAppendix}). Generally speaking, distributions of Similarity and Kriging predictions show a range of overlap and appear to fluctuate systematically across adjacent upper-plate sectors for some subduction zone segments. Trench-orthogonal profiles of }\DIFaddend Similarity and Kriging \DIFdelbegin \DIFdel{prediction patterns within }\DIFdelend \DIFaddbegin \DIFadd{predictions similarly show a range of shapes and distributions among }\DIFaddend upper-plate sectors \DIFdelbegin \DIFdel{(Table \ref{tab:lateralDiffTable}) . Each upper-plate can be characterized as having (dis)continuous Similarity prediction patterns, and}\DIFdelend \DIFaddbegin \DIFadd{and among subduction zone segments. With respect to ThermoGlobe data, observations within sectors are often too few (\(n <\) 20 observations for 59}\DIFaddend /\DIFdelbegin \DIFdel{or (dis)continuous Kriging prediction patterns, and/or (in)comparable Similarity vs. ~Kriging prediction patterns. Discontinuous }\DIFdelend \DIFaddbegin \DIFadd{100 sectors; Table \ref{tab:sectorSummaryTable}) to compare distributions confidently with other sectors.
}

\DIFadd{Below are three examples of subduction zone segments that characterize the range of observed }\DIFaddend upper-plate \DIFdelbegin \DIFdel{surface heat flow patterns are more commonly predicted by either interpolation method than continuous }\DIFdelend \DIFaddbegin \DIFadd{surface heat flow patterns.
}

\hypertarget{kyushu-ryukyu-1}{%
\paragraph{Kyushu Ryukyu}\label{kyushu-ryukyu-1}}

\DIFadd{Kyushu Ryukyu characterizes a subduction zone segment with relatively consistent }\DIFaddend upper-plate \DIFaddbegin \DIFadd{surface heat flow for thousands of \(km\) along-strike. In this case, }\emph{\DIFadd{consistent}} \DIFadd{refers to comparable Similarity and Kriging predictions }\emph{\DIFadd{and}} \DIFadd{consistent surface heat flow distributions and profiles across sectors. That is, medians and interquartile ranges of Similarity and Kriging predictions overlap well within a narrow range (with the exception of sector 8, Figure \ref{fig:kyushuRyukyuUpper}). Across all sectors, median Similarity and Kriging predictions differ by only 8.1 \(\pm\) 8.4 \(mWm^{-2}\) on average (Table \ref{tab:sectorSummaryTable}). Moreover, interquartile ranges of Similarity and Kriging predictions across all sectors are relatively consistent---differing by 14.6 \(\pm\) 23.6 \(mWm^{-2}\) on average. Notably, upper-plate }\DIFaddend surface heat flow \DIFdelbegin \DIFdel{patterns (see Appendix \ref{lateralDiffAppendix}) }\DIFdelend \DIFaddbegin \DIFadd{appears to increase systematically from the NE to SW across sectors 8-6 before leveling out through sectors 5-1}\DIFaddend .
\DIFdelbegin \DIFdel{Discontinuous }\DIFdelend \DIFaddbegin 

\DIFadd{Meanwhile, ThermoGlobe data within Kyushu Ryukyu upper-plate sectors (n = 339) vary considerably. Wide distributions of ThermoGlobe data appear near the trench and at approximately 200 \(km\) from the trench (coinciding with locations of volcanic vents, Figure \ref{fig:kyushuRyukyuUpper}). Yet, smoothed trench-orthogonal Similarity and Kriging profiles gently arc through the approximate midrange of ThermoGlobe data. Profile shapes are consistent across sectors and show relatively little spread (\(\leq\) 25 \(mWm^{-2}\)). All profiles gradually rise from approximately 50 \(mWm^{-2}\) at the trench to maximums of approximately 75-100 \(mWm^{-2}\) before gradually decreasing to approximately 75 \(mWm^{-2}\) at 500 \(km\) into the upper-plate.
}
...
\caption[Surface heat flow profiles for Kyushu Ryukyu upper-plate sectors]{\DIFaddFL{Surface heat flow profiles for Kyushu Ryukyu upper-plate sectors. (a) Profiles are computed by finding orthogonal distances between the segment boundary (trench; bold black line) and surface heat flow predictions within eight 500 \(km\)-wide sectors (colored polygons). (b) Similarity and Kriging predictions across sectors are largely indistinguishable with overlapping interquartile ranges (boxes). (c) Profiles (colored curves with 95\% confidence intervals) are remarkably consistent across sectors for Kriging and Similarity predictions. Colored squares are ThermoGlobe data from Lucazeau (}\protect\hyperlink{ref-lucazeau2019}{2019}\DIFaddFL{). Segment boundary and volcanoes (gold diamonds) defined by Syracuse \& Abers (}\protect\hyperlink{ref-syracuse2006}{2006}\DIFaddFL{). Plate boundaries (bold black lines) defined by Lawver et al. (}\protect\hyperlink{ref-lawver2018}{2018}\DIFaddFL{). Profile curves in (c) are LOESS regressions through three-point running averages (small colored data points).}}\label{fig:kyushuRyukyuUpper}
\end{figure}

\hypertarget{sumatra-banda-sea}{%
\paragraph{Sumatra Banda Sea}\label{sumatra-banda-sea}}

\DIFadd{Sumatra Banda Sea characterizes a subduction zone segment with moderately consistent upper-plate }\DIFaddend surface heat flow \DIFdelbegin \DIFdel{patterns are predicted by either Similarity or Kriging for 12/13 segments and by both }\DIFdelend \DIFaddbegin \DIFadd{for thousands of \(km\) along-strike. In this case, }\emph{\DIFadd{moderately consistent}} \DIFadd{refers to mostly comparable (overlaping) Similarity and Kriging predictions that distinctively fluctuate in a similar manner across sectors. That is, medians and interquartile ranges of }\DIFaddend Similarity and Kriging \DIFdelbegin \DIFdel{for 7/13 segments }\DIFdelend \DIFaddbegin \DIFadd{predictions overlap well for some sectors, but not others }\DIFaddend (e.g.\DIFdelbegin \DIFdel{, Kyushu Ryukyu; Figure \ref{fig:kyushuRyukyuUpper}). In contrast, continuous upper-plate }\DIFdelend \DIFaddbegin \DIFadd{~sectors 1, 10, \& 11, Figure \ref{fig:sumatraBandaSeaUpper}). Median Similarity and Kriging predictions differ by 9.4 \(\pm\) 14.2 \(mWm^{-2}\) and interquartile ranges differ by 15.6 \(\pm\) 46.2 \(mWm^{-2}\) on average across all sectors (Table \ref{tab:sectorSummaryTable}). Notably, Similarity and Kriging predictions appear to broadly oscillate between higher and lower }\DIFaddend surface heat flow \DIFdelbegin \DIFdel{patterns are predicted by either Similarity or Kriging for 6/13 segments and by both }\DIFdelend \DIFaddbegin \DIFadd{across adjacent sectors with a wavelength on the order of 10\(^3\) \(km\) (several sectors).
}

\DIFadd{Meanwhile, Similarity and Kriging profiles show obvious differences. For example, Similarity predictions are distributed narrowly and increase monotonically from the trench to 500 \(km\) into the upper-plate, whereas Kriging profiles generally ramp up more steeply and begin to disperse at approximately 200 \(km\) from the trench. Similarity profiles remain narrowly distributed through at least 300 \(km\) from the trench, whereas Kriging profiles show up to 25-30 \(mWm^{-2}\) spread among sectors at 300-500 \(km\) from the trench.
}
...
\caption[Surface heat flow profiles for Sumatra Banda Sea upper-plate sectors]{\DIFaddFL{Surface heat flow profiles for Sumatra Banda Sea upper-plate sectors. (a) Profiles are computed by finding orthogonal distances between the segment boundary (trench; bold black line) and surface heat flow predictions within eight 500 \(km\)-wide sectors (colored polygons). (b) Similarity and Kriging predictions across sectors are moderately distinguishable with mostly overlapping interquartile ranges (boxes). (c) Profiles (colored curves with 95\% confidence intervals) of Kriging predictions show greater overall spread than Similarity profiles (e.g.~\(\geq\) 200 \(km\) from the trench). Colored squares are ThermoGlobe data from Lucazeau (}\protect\hyperlink{ref-lucazeau2019}{2019}\DIFaddFL{). Segment boundary and volcanoes (gold diamonds) defined by Syracuse \& Abers (}\protect\hyperlink{ref-syracuse2006}{2006}\DIFaddFL{). Plate boundaries (bold black lines) defined by Lawver et al. (}\protect\hyperlink{ref-lawver2018}{2018}\DIFaddFL{). Profile curves in (c) are LOESS regressions through three-point running averages (small colored data points).}}\label{fig:sumatraBandaSeaUpper}
\end{figure}

\hypertarget{new-britain-solomon}{%
\paragraph{New Britain Solomon}\label{new-britain-solomon}}

\DIFadd{New Britain Solomon characterizes a subduction zone segment with inconsistent upper-plate surface heat flow and poor overlap between }\DIFaddend Similarity and Kriging \DIFdelbegin \DIFdel{for only 1/13 segments (Kamchatka Marianas; Figure \ref{fig:kamchatkaMarianasUpper}). At least 7/13 segments show incomparable upper-plate }\DIFdelend \DIFaddbegin \DIFadd{predictions. Only one sector (sector 8) shows overlapping interquartile ranges of Similarity and Kriging predictions, whereas all other sectors strongly diverge (Figure \ref{fig:newBritainSolomonUpper}). For example, median Kriging predictions range by 9.6 \(mWm^{-2}\) across all sectors, whereas median Similarity predictions range by 44.1 \(mWm^{-2}\). Moreover, Similarity and Kriging medians across all sectors differ by 23.2 \(\pm\) 33.8 \(mWm^{-2}\) on average. Notably, an apparent wave-like oscillation between higher and lower }\DIFaddend surface heat flow \DIFdelbegin \DIFdel{patterns predicted by Similarity and Kriging (}\DIFdelend \DIFaddbegin \DIFadd{across adjacent sectors is observed---especially for Similarity predictions.
}

\DIFadd{Meanwhile, Similarity and Kriging profiles are obviously distinguishable. For example, Kriging profiles are smooth and closely parallel ThermoGlobe data, whereas Similarity profiles show higher average surface heat flow (Figure \ref{fig:newBritainSolomonUpper}). In contrast to flat Kriging profiles, high surface heat flow regions along Similarity profiles clearly show the influence of certain tectonic features (}\DIFaddend e.g.~\DIFdelbegin \DIFdel{Central America; Figure \ref{fig:centralAmericaUpper}). }\DIFdelend \DIFaddbegin \DIFadd{in sector 4, which intersects a volcanic center and ridge segment). Moreover, small confidence intervals around Kriging profiles suggest small uncertainties compared to Similarity. However, Kriging is determined to find the smallest variance solution by definition and can easily overfit the small number (n = 9) of ThermoGlobe data. Divergence between Similarity and Kriging predictions near New Britain Solomon thus appear to be driven by methodological differences and a tendency for Kriging to overfit small sample sets.
}\DIFaddend 
...
{\centering \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\linewidth,]{assets/figs/chpt3/CentralAmericaUpperPlate} 
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1\linewidth,]{assets/figs/chpt3/NewBritainSolomonUpperPlate} 
\DIFaddendFL 

}

\DIFdelbeginFL %DIFDELCMD < \caption[Along strike variability of upper-plate surface heat flow near Central America]{%%%
\DIFdelFL{Along strike variability of upper-plate surface }\DIFdelendFL \DIFaddbeginFL \caption[Surface heat flow profiles for New Britain Solomon upper-plate sectors]{\DIFaddFL{Surface }\DIFaddendFL heat flow \DIFdelbeginFL \DIFdelFL{near Central America}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{profiles for New Britain Solomon upper-plate sectors}\DIFaddendFL . (a) \DIFdelbeginFL \DIFdelFL{Distances }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Profiles }\DIFaddendFL are computed \DIFdelbeginFL \DIFdelFL{orthogonally }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{by finding orthogonal distances }\DIFaddendFL between the \DIFdelbeginFL \DIFdelFL{subduction zone }\DIFdelendFL segment boundary (\DIFaddbeginFL \DIFaddFL{trench; }\DIFaddendFL bold black line) and \DIFdelbeginFL \DIFdelFL{interpolated upper-plate }\DIFdelendFL surface heat flow \DIFdelbeginFL \DIFdelFL{targets }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{predictions }\DIFaddendFL within eight 500 \(km\)-wide sectors (colored polygons). (b) Similarity \DIFdelbeginFL \DIFdelFL{predictions (dashed curves) }\DIFdelendFL and Kriging predictions \DIFdelbeginFL \DIFdelFL{(solid curves) are distinguishable }\DIFdelendFL across \DIFdelbeginFL \DIFdelFL{all }\DIFdelendFL sectors \DIFaddbeginFL \DIFaddFL{are very distinguishable }\DIFaddendFL with \DIFdelbeginFL \DIFdelFL{up to approximately 25 \(mWm^{-2}\) of average offset. }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{non-overlapping interquartile ranges }\DIFaddendFL (\DIFaddbeginFL \DIFaddFL{boxes), except for sector 8. (}\DIFaddendFL c) \DIFdelbeginFL \DIFdelFL{Both Similarity and }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Profiles (colored curves with 95\% confidence intervals) of }\DIFaddendFL Kriging predictions \DIFaddbeginFL \DIFaddFL{are lower and }\DIFaddendFL show \DIFdelbeginFL \DIFdelFL{discontinuous upper-plate surface heat flow with up }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{a narrow distribution compared }\DIFaddendFL to \DIFdelbeginFL \DIFdelFL{25-30 \(mWm^{-2}\) differences in average upper-plate surface heat flow among sectors, but }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Similarity profiles. Colored squares }\DIFaddendFL are \DIFdelbeginFL \DIFdelFL{distinguishable }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{ThermoGlobe data }\DIFaddendFL from \DIFdelbeginFL \DIFdelFL{each other because of an average offset of approximately 25 \(mWm^{-2}\)}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Lucazeau (}\protect\hyperlink{ref-lucazeau2019}{2019}\DIFaddFL{)}\DIFaddendFL . Segment boundary \DIFaddbeginFL \DIFaddFL{and volcanoes (gold diamonds) }\DIFaddendFL defined by Syracuse \& Abers (\protect\hyperlink{ref-syracuse2006}{2006}). \DIFdelbeginFL \DIFdelFL{Curves }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Plate boundaries (bold black lines) defined by Lawver et al. (}\protect\hyperlink{ref-lawver2018}{2018}\DIFaddFL{). Profile curves }\DIFaddendFL in (c) are LOESS regressions through \DIFdelbeginFL \DIFdelFL{five-point }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{three-point }\DIFaddendFL running averages \DIFaddbeginFL \DIFaddFL{(small colored data points)}\DIFaddendFL .}\DIFdelbeginFL %DIFDELCMD < \label{fig:centralAmericaUpper}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:newBritainSolomonUpper}
\DIFaddendFL \end{figure}

\DIFdelbegin %DIFDELCMD < \clearpage
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{optimum-kriging-parameters}{%
\subsection{Optimum Kriging Parameters}\label{optimum-kriging-parameters}}
\DIFaddend 

\DIFdelbegin %DIFDELCMD < \hypertarget{chpt3Discussion}{%
%DIFDELCMD < \section{Discussion}\label{chpt3Discussion}}
%DIFDELCMD < 

%DIFDELCMD < \hypertarget{similarity-and-kriging-comparisons}{%
%DIFDELCMD < \subsection{Similarity and Kriging Comparisons}\label{similarity-and-kriging-comparisons}}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Because Similarity and Kriging are fundamentally based on different underlying mathematical frameworks, comparing }\DIFdelend \DIFaddbegin \DIFadd{Optimized Kriging parameters vary substantially from segment to segment (Table \ref{tab:vgrmSummaryTable}). However, despite a range of domain sizes, observational densities, and diverse plate configurations, Kriging parameters converge on solutions for all Kriging domains (Figure \ref{fig:optTrace}) and show no systematic correlation with cost (Figure \ref{fig:vgrmSummaryPlot}). Differences in cost are apparently explained by systematic regional differences in }\DIFaddend surface heat flow \DIFdelbegin \DIFdel{predictions can improve understanding of subduction zone processes in the following ways. }\DIFdelend \DIFaddbegin \DIFadd{distributions (i.e.~differences in the constant terms \(\sigma_{vgrm}\) and \(\sigma_{interp}\) in Equation \eqref{eq:costExp}) rather than sensitivity to Kriging parameters.
}\DIFaddend 

\DIFdelbegin \DIFdel{First, comparable }\DIFdelend \DIFaddbegin \begin{table}

\caption{\label{tab:vgrmSummaryTable}\DIFaddFL{Optimum variogram models and interpolation accuracy by segment}}
\centering
\resizebox{\linewidth}{!}{
\begin{threeparttable}
...
\DIFadd{Regional Kriging errors (ranging from 9.7 to 48 \(mWm^{-2}\)) all exceed Similarity error rates from the same regions, except for the Lesser Antilles (Table \ref{tab:vgrmSummaryTable}). Kriging errors can be relatively small compared to Similarity for domains with high observational density (Lesser Antilles; n = 3,010, \(\Delta\)RMSE = -2.7) but relatively large where observational density is comparatively low (Vanuatu; n = 137, \(\Delta\)RMSE = 30.2). On average, Kriging error rates are 2.3 times higher than Similarity. Regional Similarity error rates for most subduction zone segments are slightly higher than the 7 \(mWm^{-2}\) Similarity error rate reported by Lucazeau (}\protect\hyperlink{ref-lucazeau2019}{2019}\DIFadd{) (for their preferred global model with 7 proxy observables). However, Similarity error rates in Table \ref{tab:vgrmSummaryTable} are consistent with global Similarity error rates computed by cross-validation on a 1\(^\circ \times\) 1\(^\circ\) (from 11.6 to 29.0 \(mW/m^{-2}\)) reported previously by Goutorbe et al. (}\protect\hyperlink{ref-goutorbe2011}{2011}\DIFadd{).
}
...
\DIFadd{If the goal is understanding subduction zone structure and geodynamics, comparing two independent interpolation methods has distinct advantages. For example, this study finds many cases of }\DIFaddend Similarity and Kriging \DIFdelbegin \DIFdel{interpolations can corroborate expectedand/}\DIFdelend \DIFaddbegin \DIFadd{predictions corroborating known, expected, }\DIFaddend or predicted tectonic features. \DIFdelbegin \DIFdel{For example, Similarity and Kriging both predict }\DIFdelend \DIFaddbegin \DIFadd{These include: (1) }\DIFaddend low surface heat flow \DIFdelbegin \DIFdel{regions }\DIFdelend defining the oceanic plate and forearc \DIFaddbegin \DIFadd{regions }\DIFaddend along the Kamchatka Marianas segment (Figure \ref{fig:kamchatkaMarianasDiff})\DIFdelbegin \DIFdel{. Similarity and Kriging both predict }\DIFdelend \DIFaddbegin \DIFadd{, (2) }\DIFaddend high surface heat flow anomalies defining the volcanic center and transform fault separating the South American Plate and Caribbean Plates near the Lesser Antilles Segment (Figure \ref{fig:lesserAntillesDiff})\DIFdelbegin \DIFdel{. Moreover, Similarity and Kriging both predict }\DIFdelend \DIFaddbegin \DIFadd{, (3) the general seafloor thermal structure near the N Philippines segment (Figure \ref{fig:nPhilippinesDiff}), (4) }\DIFaddend a broad region of high surface heat flow within the NW part of the Sumatra Banda Sea segment upper-plate (Figure \ref{fig:sumatraBandaSeaDiff}), \DIFdelbegin \DIFdel{corroborating a uniformly-thin upper-plate lithosphere inferred from previous studies (}%DIFDELCMD < \protect\hyperlink{ref-currie2006}{Currie \& Hyndman, 2006}%%%
\DIFdel{; }%DIFDELCMD < \protect\hyperlink{ref-hyndman2005}{Hyndman et al., 2005}%%%
\DIFdel{) . Other examples of general agreement between interpolation methods are observed }\DIFdelend \DIFaddbegin \DIFadd{and (5) high surface heat flow defining volcanic arc chains }\DIFaddend near the Kyushu Ryukyu \DIFaddbegin \DIFadd{segment }\DIFaddend (Figure \ref{fig:kyushuRyukyuDiff})\DIFdelbegin \DIFdel{and N Philippines segments (Figure \ref{fig:nPhilippinesDiff}).
Tectonic features predicted by Similarity and Kriging are confirmed independently by prior geologic information and local surface heat flow observations.
}\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend 

\DIFdelbegin \DIFdel{Second, inconsistent }\DIFdelend \DIFaddbegin \DIFadd{While corroboration is an advantageous outcome when comparing independent interpolation methods, finding inconsistencies between }\DIFaddend Similarity and Kriging \DIFdelbegin \DIFdel{predictions can identify tectonic features that are unexpected or currently unresolved by either method}\DIFdelend \DIFaddbegin \DIFadd{methods is at least as valuable}\DIFaddend . For example, \DIFaddbegin \DIFadd{this study finds many cases of }\DIFaddend Similarity and Kriging predictions \DIFdelbegin \DIFdel{do not confirm the same featuresnear the Central America segment , including }\DIFdelend \DIFaddbegin \DIFadd{identifying unexpected or poorly resolved tectonic features. These include: (1) much of the thermal structure along the Andes segment (Figure \ref{fig:andesDiff}), (2) }\DIFaddend the location and extent of two spreading centers, the tip of a transform fault, and the \DIFdelbegin \DIFdel{thickness }\DIFdelend \DIFaddbegin \DIFadd{thermal structure }\DIFaddend of the Cocos Plate \DIFaddbegin \DIFadd{near the Central America segment }\DIFaddend (Figure \ref{fig:centralAmericaDiff})\DIFdelbegin \DIFdel{. A }\DIFdelend \DIFaddbegin \DIFadd{, (3) plate boundaries near the New Britain Solomon (Figure \ref{fig:newBritainSolomonDiff}) and Scotia segments (Figure \ref{fig:scotiaDiff}), (4) a large low surface heat flow anomaly near the Sumatra Banda Sea segment (east of Borneo at approximately 120\(^\circ\)E and 5\(^\circ\)S, Figure \ref{fig:sumatraBandaSeaDiff}), (5) a }\DIFaddend high heat flow anomaly defining a transform fault near the \DIFdelbegin \DIFdel{N }\DIFdelend \DIFaddbegin \DIFadd{north }\DIFaddend tip of the Tonga New Zealand segment \DIFdelbegin \DIFdel{is predicted by Kriging, but not by Similarity }\DIFdelend (Figure \ref{fig:tongaNewZealandDiff})\DIFdelbegin \DIFdel{. Further, there is significant disagreement on }\DIFdelend \DIFaddbegin \DIFadd{, and (6) }\DIFaddend the location of microplate boundaries \DIFdelbegin \DIFdel{predicted by Similarity vs.~Kriging }\DIFdelend near the Vanuatu segment (Figure \ref{fig:vanuatuDiff}).
\DIFdelbegin \DIFdel{Features predicted by only one interpolation method either violate prior geologic information}\DIFdelend \DIFaddbegin 

\DIFadd{Inconsistent interpolations identify tectonic features that either violate geologic proxy datasets}\DIFaddend , violate local surface heat flow observations, or simply lack sufficient observational coverage to \DIFdelbegin \DIFdel{resolve }\DIFdelend \DIFaddbegin \DIFadd{be resolved }\DIFaddend by Kriging. In any case, \DIFdelbegin \DIFdel{inconsistent Similarity and Kriging predictions can identify specific features }\DIFdelend \DIFaddbegin \DIFadd{the above examples demonstrate the utility of comparing independent interpolation methods in identifying }\DIFaddend relevant targets for future \DIFdelbegin \DIFdel{data acquisition and investigation}\DIFdelend \DIFaddbegin \DIFadd{investigation and data acquisition (discussed more below)}\DIFaddend . The comparisons in Section \ref{chpt3Results} and Appendices \ref{interpDiffAppendix} \& \ref{lateralDiffAppendix} \DIFdelbegin \DIFdel{thus }\DIFdelend \DIFaddbegin \DIFadd{therefore }\DIFaddend provide important context for subduction zone research.

\DIFdelbegin %DIFDELCMD < \hypertarget{similarity-and-kriging-accuracies}{%
%DIFDELCMD < \subsection{Similarity and Kriging Accuracies}\label{similarity-and-kriging-accuracies}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{comparing-upper-plate-sectors}{%
\subsection{Comparing Upper-Plate Sectors}\label{comparing-upper-plate-sectors}}
\DIFaddend 

\DIFdelbegin \DIFdel{While one approach is not evidently more favorable based on first principles, regional Similarity error rates are considerably lower than Kriging (Table \ref{tab:diffSummaryTable}. However, relatively low error rates do not necessarily imply Similarity produces more accurate interpolations. It is difficult to assess the true comparative accuracy between Similarity and Kriging interpolations for two reasons. First, practically reducing Kriging error rates is possible by choosing different Kriging parameters }\DIFdelend \DIFaddbegin \hypertarget{issues-with-irregularly-spaced-data}{%
\subsubsection{Issues with Irregularly-Spaced Data}\label{issues-with-irregularly-spaced-data}}

\DIFadd{Surface heat flow profiles in previous studies are computed with observations sampled from within a single sector (}\protect\hyperlink{ref-currie2004}{Currie et al., 2004}\DIFadd{; }\protect\hyperlink{ref-currie2006}{Currie \& Hyndman, 2006}\DIFadd{; }\protect\hyperlink{ref-furukawa1993}{Furukawa, 1993}\DIFadd{; }\protect\hyperlink{ref-hyndman2005}{Hyndman et al., 2005}\DIFadd{; }\protect\hyperlink{ref-kerswell2020}{Kerswell et al., 2020}\DIFadd{; }\protect\hyperlink{ref-wada2009}{Wada \& Wang, 2009}\DIFadd{). While extending a single-sector sampling approach to many adjacent sectors is simple to implement, inherent pitfalls are immediately obvious when comparing ThermoGlobe samples among sectors. For example, the spatial density and regularity of ThermoGlobe data within adjacent sectors can often be drastically different }\DIFaddend (e.g.~\DIFdelbegin \DIFdel{the maximum number of point-pairs used for local Kriging \(n_{max}\)). While minimizing Kriging cross-validation error by an optimization procedure is a standard geostatistical approach, it does not necessarily guarantee the most accurate interpolation. For example, error rates may be reduced by highly localizing Kriging (by decreasing the number of maximum point-pairs \(n_{max}\) used for Kriging at each target location). Consequently, regional interpolation accuracy may be compromised if observations are irregularly-spaced and clustered in local high-density areas. Low Kriging error rates are therefore generally an indication of good interpolation results, but do not fully determine accuracy without evaluating the spatial relationships among observations used to calculate the error rates}\DIFdelend \DIFaddbegin \DIFadd{Central America, Sumatra Banda Sea, and Tonga New Zealand, Table \ref{tab:sectorSummaryTable}). Fluctuating sample sizes among upper-plate sectors can make statistical comparisons of ThermoGlobe data equivocal}\DIFaddend .

\DIFdelbegin \DIFdel{This study computes Kriging error rates using all surface heat flow observations within the interpolation domain. In contrast, Similarity error rates are computed using a subset of observations that are explicitly co-located with the 0.5\(^\circ \times\) 0.5\(^\circ\) grid of target locations taken from Lucazeau (}%DIFDELCMD < \protect\hyperlink{ref-lucazeau2019}{2019}%%%
\DIFdel{) . Similarity error rates may increase (or decrease)relative to Kriging error rates if computed using allobservations within each interpolation domain. Computing Similarity error rates on the same set of observations as Kriging would require reproducing the Similarity results of Lucazeau (}%DIFDELCMD < \protect\hyperlink{ref-lucazeau2019}{2019}%%%
\DIFdel{)from the constituent proxy datasets, which are not provided as supplementary files.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Differences in error rates notwithstanding, Similarity has a distinct advantage compared to Kriging in regions with relatively low observational coverage }\DIFdelend \DIFaddbegin \DIFadd{Indeed, this study finds comparing ThermoGlobe data among upper-plate sectors problematic. For instance, ThermoGlobe data are often too few (n \textless{} 20 observations for 59/100 sectors, Table \ref{tab:sectorSummaryTable}) to compare with statistical confidence. Many sectors (n = 11) have a single observation with a singular distribution (}\gls{iqr} \DIFadd{= 0) or few observations spanning a large range (very large }\gls{iqr}\DIFadd{). Many sectors encompass zero ThermoGlobe data and therefore cannot be compared at all. In other words, summary statistics necessary for gauging the }\DIFaddend (\DIFaddbegin \DIFadd{dis)continuity of surface heat flow among sectors (}\DIFaddend e.g.~\DIFdelbegin \DIFdel{Scotia; Figure \ref{fig:scotiaDiff}) . While the detailed regional texture imposed by similarity is artificial insofar as it does not represent known spatial changes in surface heat flow, discrete tectonic features imposed by Similarity appear to be geodynamically consistent in most cases. Similarity may produce ostensibly accurate solutions even in cases with zero observations by integrating multiple sources of geologic information into a composite estimate of surface heat flow . This is a strength of the Similarity method that can be leveraged where limited observations preclude accurate Kriging interpolations . Regardless of methodology, the accuracy and resolution of interpolations compared in this study are sufficient to probe hypotheses about regional geodynamic uniformity among active subduction zones on the basis }\DIFdelend \DIFaddbegin \DIFadd{median, }\gls{iqr}\DIFadd{, Table \ref{tab:sectorSummaryTable}) can be generally considered unreliable for up to 59/100 sectors.
}

\DIFadd{The above limitation arising from sampling irregularly-spaced data can be easily overcome by interpolation. That is because sampling a regular interpolation grid allows for more consistent sample sizes and spatial coverage across sectors. For example, many sectors defined in this study have few ThermoGlobe data (n \textless{} 5 observations for 37/100 sectors, Table \ref{tab:sectorSummaryTable}), yet the average number of interpolation targets within those same sectors is 51---about 10 times the sample size on average. Surface heat flow variability among sectors is thus more confidently and consistently evaluated with interpolations derived from ThermoGlobe data rather than ThermoGlobe data alone.
}

\hypertarget{discontinuity-of-upper-plate-surface-heat-flow}{%
\subsubsection{(Dis)continuity of Upper-Plate Surface Heat Flow}\label{discontinuity-of-upper-plate-surface-heat-flow}}

\DIFadd{Despite being derived from nearly-identical sets of processed ThermoGlobe data, comparisons of Similarity and Kriging predictions across thousands of square kilometers of upper-plate sectors very clearly show mixed results. In summary, Similarity and Kriging predictions from 100 upper-plate sectors neither show broadly continuous nor broadly discontinuous upper-plate surface heat flow within, or among subduction zone segments. Rather than observing consistency and uniformity in upper-plate surface heat flow, a Kaleidoscope }\DIFaddend of surface heat flow \DIFdelbegin \DIFdel{(discussed below}\DIFdelend \DIFaddbegin \DIFadd{profiles and distributions are observed (compare Figures \ref{fig:kyushuRyukyuUpper}, \ref{fig:sumatraBandaSeaUpper}, \ref{fig:newBritainSolomonUpper}, and see Appendix \ref{lateralDiffAppendix} \& Table \ref{tab:sectorSummaryTable}}\DIFaddend ).
...
%DIFDELCMD < %%%
\DIFdel{Lateral variability in }\DIFdelend \DIFaddbegin \DIFadd{How consistent and continuous is }\DIFaddend upper-plate \DIFaddbegin \DIFadd{surface heat flow within, and among subduction zone segments? The Kaleidoscope of surface heat flow profiles and distributions observed across all 13 subduction zone segments is somewhat ambiguous in this regard. While Similarity and Kriging predictions show discontinuous upper-plate }\DIFaddend surface heat flow patterns \DIFdelbegin \DIFdel{may imply spatially heterogeneous lithospheric thickness (contrary to expectations from Chapter \ref{chpt2}), spatially heterogeneous dynamics, spatially heterogeneous near-surface perturbations, and/or undersampling relative to the scale and magnitude of spatial variability.As a corollary, lateral similarity }\DIFdelend \DIFaddbegin \DIFadd{for some segments (e.g.~Lesser Antilles and Vanuatu, Figures \ref{fig:lesserAntillesUpper} \& \ref{fig:vanuatuUpper}), other segments show rather continuous patterns (e.g.~Central America, Kamchatka Marianas, Kyushu Ryukyu, N Philippines, Figures \ref{fig:centralAmericaUpper}, \ref{fig:kamchatkaMarianasUpper}, \ref{fig:kyushuRyukyuUpper}, \ref{fig:nPhilippinesUpper}), and still other segments show mixed patterns depending on the interpolation method (e.g.~Alaska Aleutians, Andes, New Britain Solomon, S Philippines, Sumatra Banda Sea, Tonga New Zealand, Figures \ref{fig:alaskaAleutiansUpper}, \ref{fig:andesUpper}, \ref{fig:newBritainSolomonUpper}, \ref{fig:sPhilippinesUpper}, \ref{fig:sumatraBandaSeaUpper}, \ref{fig:tongaNewZealandUpper}).
}

\DIFadd{On one hand, Similarity and Kriging interpolations can show nearly-identical profiles continuing along-strike for 1000's of \(km\) (e.g.~Kamchatka Marianas, Kyushu Ryukyu, Sumatra Banda Sea, Figures \ref{fig:kamchatkaMarianasUpper}, \ref{fig:kyushuRyukyuUpper}, \ref{fig:sumatraBandaSeaUpper}). These segments demonstrates the possibility of large-scale continuity }\DIFaddend in upper-plate surface heat flow \DIFdelbegin \DIFdel{patterns imply the opposite. While uniformly thin upper-plate lithospheres anddepths of plate coupling among subduction zones are inferred from similar patterns of upper-plate surface heat flow among subduction zone segments (}\DIFdelend \DIFaddbegin \DIFadd{and may imply spatially homogeneous lithospheric thermal structure and/or spatially homogeneous heat-transferring dynamics inferred from previous work (Chapter \ref{chpt2}, }\DIFaddend \protect\hyperlink{ref-currie2004}{Currie et al., 2004}; \protect\hyperlink{ref-currie2006}{Currie \& Hyndman, 2006}; \protect\hyperlink{ref-furukawa1993}{Furukawa, 1993}; \protect\hyperlink{ref-kerswell2020}{Kerswell et al., 2020}; \protect\hyperlink{ref-wada2009}{Wada \& Wang, 2009})\DIFdelbegin \DIFdel{, these inferences are drawn selected transects. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{In contrast, regional }\DIFdelend \DIFaddbegin \DIFadd{. However, an alternative explanation is that continuous }\DIFaddend surface heat flow \DIFdelbegin \DIFdel{interpolations across thousands of square kilometers presented in this study clearly show very mixed results (Table \ref{tab:lateralDiffTable}).
Some upper-plates appear rather uniform across large regions (e. g., Kamchatka Marianas; Figures \ref{fig:kamchatkaMarianasDiff} \& \ref{fig:kamchatkaMarianasUpper})---supporting uniformly }\DIFdelend \DIFaddbegin \DIFadd{is a consequence of undersampling relative to the local spatial variability of surface heat flow. Moreover, most segments show neither completely continuous nor discontinuous }\DIFaddend upper-plate \DIFaddbegin \DIFadd{surface heat flow patterns (Table \ref{tab:sectorSummaryTable}).
}

\DIFadd{Another type of continuity observed across some segments is an apparent wave-like oscillation between higher and lower surface heat flow across multiple adjacent upper-plate sectors. The most notable case is the Sumatra Banda Sea segment (Figure \ref{fig:sumatraBandaSeaUpper}), where median Similarity and Kriging predictions oscillate with a wavelength on the order of 10\(^3\) \(km\) (approximately 5-7 sectors). Such large-wavelength oscillations may imply gradual along-strike variation in upper-plate }\DIFaddend thickness\DIFdelbegin \DIFdel{inferred from previous work. However, many other subduction zone segments show distinct patterns }\DIFdelend \DIFaddbegin \DIFadd{, coupling depths, and/or lithosphere-asthenosphere geodynamics. Note that near surface perturbations probably do not significantly factor into large-scale oscillations because hydrothermal effects are expected to be locally distributed in concordance with thin sediment cover (\textless{} 400 \(m\)) and/or close proximity to seamounts (\textless{} 60 \(km\), }\protect\hyperlink{ref-hasterok2011}{Hasterok et al., 2011}\DIFadd{).
}

\hypertarget{identifying-survey-targets}{%
\subsubsection{Identifying Survey Targets}\label{identifying-survey-targets}}

\DIFadd{The Kaleidoscope }\DIFaddend of surface heat flow \DIFdelbegin \DIFdel{within several adjacent upper-plate sectors(}\DIFdelend \DIFaddbegin \DIFadd{patterns computed from Similarity and Kriging predictions offers opportunities for discovering future survey targets by identifying the least-constrained sectors. For example, segments with the greatest Similarity-Kriging discrepancies among sectors tend to have: (1) very few ThermoGlobe data (}\DIFaddend e.g.\DIFdelbegin \DIFdel{, Central America; Figures \ref{fig:centralAmericaDiff} \& \ref{fig:centralAmericaUpper}). These results stress the importance of characterizing the lateral variability of }\DIFdelend \DIFaddbegin \DIFadd{~Alaska Aleutians, N Philippines, New Britain Solomon, S Philippines), (2) highly-irregular spatial coverage of ThermoGlobe data (e.g.~Andes, Central America, Lesser Antilles), and/or (3) complex upper-plate tectonics (Vanuatu). A simple query of the ThermoGlobe dataset by sector can identify individual sectors with low and/or highly-irregular observational density and/or large Similarity-Kriging discrepancies. Thus, current observational gaps in regional }\DIFaddend surface heat flow \DIFdelbegin \DIFdel{to better qualify claims of broad uniformity among (and within ) active subduction zone segments}\DIFdelend \DIFaddbegin \DIFadd{can be efficiently identified by comparing independent interpolation methods within multiple-sectors}\DIFaddend .

\DIFdelbegin %DIFDELCMD < \hypertarget{layered-interpolation-approach}{%
%DIFDELCMD < \subsection{Layered Interpolation Approach}\label{layered-interpolation-approach}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{comparing-similarity-and-kriging-accuracies}{%
\subsection{Comparing Similarity and Kriging Accuracies}\label{comparing-similarity-and-kriging-accuracies}}
\DIFaddend 

\DIFdelbegin \DIFdel{Regional interpolation differences and distinct upper-plate surface heat flow patterns predicted by Similarity and Kriging arise from methodological differences that highlight the strengths of either method. }\DIFdelend \DIFaddbegin \DIFadd{Neither Similarity nor Kriging methods are evidently more favorable for every domain on first principles. However, regional Similarity error rates are considerably lower than Kriging (for the same domain) in almost every case (Table \ref{tab:diffSummaryTable}). Moreover, lower error rates across all segments (except Lesser Antilles) highlight Similarity's ability to generalize well across domains with unique tectonic configurations.
}

\DIFadd{Note, however, that lower error rates do not necessarily imply more accurate predictions. }\DIFaddend For example, \DIFdelbegin \DIFdel{geodynamically consistent predictions are possible in }\DIFdelend \DIFaddbegin \DIFadd{it is practically possible to reduce Kriging error rates by overfitting ThermoGlobe data. Choosing different Kriging parameters and/or highly localizing Kriging could easily cause overfitting. It is therefore possible to lower error rates while simultaneously compromising regional interpolation accuracy. At 2.3 times greater error rates than Similarity on average, however, Kriging error rates do not suggest overfitting is an issue (Tables \ref{tab:vgrmSummaryTable} and \ref{tab:vgrmSummaryTableLong}).
}

\DIFadd{Differences in error rates notwithstanding, Similarity has a distinct advantage compared to Kriging when applied to }\DIFaddend regions with relatively \DIFdelbegin \DIFdel{few observations by Similarity }\DIFdelend \DIFaddbegin \DIFadd{low observational density and/or highly-irregular spatial coverage. For example, Similarity predictions appear to be remarkably consistent with known tectonic features even in cases with few observations }\DIFaddend (e.g.\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{~}\DIFaddend Scotia and New Britain Solomon\DIFdelbegin \DIFdel{; }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend Figures \ref{fig:scotiaDiff} \& \ref{fig:newBritainSolomonDiff})\DIFdelbegin \DIFdel{, whereas resolution of unpredictable or unexpected tectonic features is possible by Kriging }\DIFdelend \DIFaddbegin \DIFadd{. Integrating geologic proxies is therefore preferred when limited observations preclude practically useful Kriging interpolations.
}

\hypertarget{layered-interpolation-approach}{%
\subsection{Layered Interpolation Approach}\label{layered-interpolation-approach}}

\DIFadd{Similarity and Kriging interpolations are easily distinguishable by eye at the regional scale when compared side-by-side }\DIFaddend (e.g.\DIFdelbegin \DIFdel{, Tonga New Zealand; Figure \ref{fig:tongaNewZealandDiff}). Combining the strengths of both approaches is possible using a layered interpolation approach. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{~compare Figures \ref{fig:centralAmericaDiff}, \ref{fig:kyushuRyukyuDiff}, and \ref{fig:scotiaDiff} with the remaining segments in Appendices \ref{interpDiffAppendix} \& \ref{lateralDiffAppendix}). The same unique properties of Similarity and Kriging methods that make them quickly discernible by eye can be independently leveraged. }\DIFaddend For example, \DIFdelbegin \DIFdel{a Bayesian framework can be used in the following manner.
First, regional Similarity is used to initialize a geodynamically consistent interpolation from geologic proxy datasets (layer 0: the priors). The prior Similarity predictions are then updated by Kriging predictions with a weighting scheme to incorporate the on-the-ground reality of }\DIFdelend \DIFaddbegin \DIFadd{because Similarity is inherently agnostic to the spatial configuration of observations (}\protect\hyperlink{ref-goutorbe2011}{Goutorbe et al., 2011}\DIFadd{), accurate interpolations with well-defined plate boundaries are still possible for regions with relatively few observations (e.g.~Scotia and New Britain Solomon, Figures \ref{fig:scotiaDiff} \& \ref{fig:newBritainSolomonDiff}). Since surface heat flow observations near subduction zone segments are commonly sparse and irregularly spaced, spatial-independence from observations is a desirable property to maintain during the interpolation process.
}

\DIFadd{On the other hand, conserving the ``ground-truth'' is equally desirable property to maintain during the interpolation process. Local ordinary Kriging conserves ground-truth by remaining agnostic to all other factors but the spatial configuration of surface heat flow observations (see Appendix \ref{krigeOpt}). For example, Kriging resolves tectonic features near Tonga New Zealand and Vanuatu that are discordant with Similarity predictions but compatible with ThermoGlobe data (Figures \ref{fig:tongaNewZealandDiff} \& \ref{fig:vanuatuDiff}). Another example is the Cocos Plate near Central America where Similarity predicts relatively high heat flow by proximity to two spreading centers, yet observations of anomalously low }\DIFaddend surface heat flow \DIFdelbegin \DIFdel{observations.}\DIFdelend \DIFaddbegin \DIFadd{(e.g. }\protect\hyperlink{ref-hutnak2008}{Hutnak et al., 2008}\DIFadd{) constrain Kriging predictions to low values. Such contrasting predictions imply ThermoGlobe data violate one or more geologic proxy data sets used by Similarity. In other words, Kriging will tend to highlight anomalies (compared to Similarity) if they exist and have been observed.
}

\DIFadd{In principle, carefully layering Similarity and Kriging methods may combine their properties to produce more accurate regional interpolations. }\DIFaddend A layered approach simultaneously respects the First \DIFaddbegin \DIFadd{(}\protect\hyperlink{ref-krige1951}{Krige, 1951}\DIFadd{) }\DIFaddend and Third Laws of Geography \DIFaddbegin \DIFadd{(}\protect\hyperlink{ref-zhu2018}{Zhu et al., 2018}\DIFadd{) }\DIFaddend by integrating geologic and spatial information. \DIFdelbegin \DIFdel{Such an approach is generalizable to other geodynamic investigations and can be more accurate in principle, but is yet to be tried in practice (at least in the context of regional surface heat flow near subduction zone segments)}\DIFdelend \DIFaddbegin \DIFadd{Any number of methods may be applied to combine Similarity and Kriging predictions. As a basic example: (1) compare Similarity and Kriging layers to detect anomalies, (2) compute weights proportional to the squared difference between Similarity and Kriging predictions to emphasized (or subdue) anomalies, (3) combine Similarity and Kriging layers using a weighted average scheme. Similar methods may be the topic of future research}\DIFaddend .

\hypertarget{conclusions-1}{%
\section{Conclusions}\label{conclusions-1}}

This study \DIFdelbegin \DIFdel{compares Kriging and Similarity interpolations to distinguish spatial }\DIFdelend \DIFaddbegin \DIFadd{evaluates regional }\DIFaddend patterns of surface heat flow near subduction zones \DIFdelbegin \DIFdel{. Differences }\DIFdelend \DIFaddbegin \DIFadd{by comparing Similarity and Kriging interpolations across adjacent upper-plate sectors. Methodological differences }\DIFaddend between Similarity and Kriging \DIFdelbegin \DIFdel{predictions }\DIFdelend \DIFaddbegin \DIFadd{yield a Kaleidoscope of upper-plate surface heat flow distributions and profiles among subduction zone segments. Such mixed results }\DIFaddend highlight at least three important points of consideration \DIFaddbegin \DIFadd{regarding regional surface heat flow near active subduction zones}\DIFaddend :

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \DIFdelbegin \DIFdel{In many cases Similarity and Kriging interpolations show distinct patterns of surface heat flow that highlight methodological differences between the two approaches, yet also corroborate each other in many other cases
}\DIFdelend \DIFaddbegin \DIFadd{Accurate regional interpolations of irregularly-spaced ThermoGlobe data are key to understanding broad (segment-scale) variations in lithospheric thermal structure near subduction zones
}\DIFaddend \item
  \DIFdelbegin \DIFdel{Both Similarity and Kriging interpolations show patterns of continuous and discontinuous }\DIFdelend \DIFaddbegin \DIFadd{The Kaleidoscope of upper-plate }\DIFaddend surface heat flow \DIFdelbegin \DIFdel{among upper-plate sectors, countering arguments of broadly uniform upper-plate thickness and plate coupling depths among (or within)subduction zone segments
}\DIFdelend \DIFaddbegin \DIFadd{distributions and profiles implies various degrees of (dis)continuity exist among subduction zones in terms of their lithospheric thermal structure (contrary to expectations from Chapter \ref{chpt2}), heat-transferring subsurface dynamics, and/or observational density relative to the local spatial variability of (unknown) surface heat flow
}\DIFaddend \item
  \DIFdelbegin \DIFdel{Layered interpolation approaches can combine the strengths of Similarity (geodynamic consistency)and Kriging (spatial consistency) to }\DIFdelend \DIFaddbegin \DIFadd{Layered interpolation approaches may }\DIFaddend produce more accurate surface heat flow predictions \DIFaddbegin \DIFadd{by combining the properties of Similarity and Kriging methods
...
Although \gls{pt} distributions of recovered markers for some simulations are consistent with \texttt{pd15} in absolute terms, probability densities bear little resemblance to \texttt{pd15} for most simulations. From an absolute perspective, marker \gls{pt} distributions from simulations with moderate to fast convergence velocities (\(\geq\) 66 \(km/Ma\)\DIFdelbegin \DIFdel{; }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend Figure \ref{fig:cvDensity}), older oceanic-plates (\(\geq\) 85 \(Ma\)\DIFdelbegin \DIFdel{; }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend Figure \ref{fig:ageDensity}), or thin upper-plates (\(\leq\) 62 \(km\)\DIFdelbegin \DIFdel{; }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend Figure \ref{fig:zupDensity}) could be considered consistent with \texttt{pd15}. However, it would be very unlikely to reproduce \texttt{pd15} by randomly sampling markers from the above simulations. Only the slowest-converging simulations recover \gls{hp} rocks with similar relative frequency across pressures of \(\sim\) 0.8 to 2.4 \(GPa\) compared to \texttt{pd15} (Figure \ref{fig:ecdfs}).
...
However, 2D interpolations in Chapter \ref{chpt3} show diverse spatial patterns of surface heat flow---some implying homogeneous thermal structure, others implying heterogeneous thermal structure---near 13 presently active subduction zones. While Kriging methods yield inconsistent results for segments with low observational densities, \DIFdelbegin \DIFdel{Similarity methods show }\DIFdelend \DIFaddbegin \DIFadd{the Similarity method shows }\DIFaddend geologically-consistent interpolations of surface heat flow across all segments. Both methods, where applicable, generally agree and present surface heat flow interpolations in contradiction of broadly uniform upper-plate lithospheric thickness inferred from other (1D) sampling strategies (e.g., \protect\hyperlink{ref-currie2004}{Currie et al., 2004}; \protect\hyperlink{ref-currie2006}{Currie \& Hyndman, 2006}; \protect\hyperlink{ref-hyndman2005}{Hyndman et al., 2005}; \protect\hyperlink{ref-wada2009}{Wada \& Wang, 2009}). These results imply broadly non-uniform coupling depths among subduction zones.
...
Dempster, A.\DIFdelbegin \DIFdel{P.}\DIFdelend , Laird, N.\DIFdelbegin \DIFdel{M.}\DIFdelend , \& Rubin, D. \DIFdelbegin \DIFdel{B. }\DIFdelend (1977). Maximum likelihood from incomplete data via the EM algorithm. \emph{Journal of the Royal Statistical Society: Series B (Methodological)}, \emph{39}(1), 1--22.
...
\leavevmode\vadjust pre{\DIFaddbegin \hypertarget{ref-fisher2000}{}}%DIF > 
\DIFadd{Fisher, A., \& Becker, K. (2000). Channelized fluid flow in oceanic crust reconciles heat-flow and permeability data. }\emph{\DIFadd{Nature}}\DIFadd{, }\emph{403}(6765)\DIFadd{, 71--74.
}

\leavevmode\vadjust \DIFadd{pre}{\DIFaddend \hypertarget{ref-fourier1827}{}}%
...
\leavevmode\vadjust pre{\DIFaddbegin \hypertarget{ref-hutnak2008}{}}%DIF > 
\DIFadd{Hutnak, M., Fisher, A., Harris, R., Stein, C., Wang, K., Spinelli, G., et al. (2008). Large heat and fluid fluxes driven through mid-plate outcrops on ocean crust. }\emph{\DIFadd{Nature Geoscience}}\DIFadd{, }\emph{1}(9)\DIFadd{, 611--614.
}

\leavevmode\vadjust \DIFadd{pre}{\DIFaddend \hypertarget{ref-hyndman2003}{}}%
...
\leavevmode\vadjust pre{\DIFaddbegin \hypertarget{ref-nyblade1993}{}}%DIF > 
\DIFadd{Nyblade, A., \& Pollack, H. (1993). A global analysis of heat flow from precambrian terrains: Implications for the thermal structure of archean and proterozoic lithosphere. }\emph{\DIFadd{Journal of Geophysical Research: Solid Earth}}\DIFadd{, }\emph{\DIFadd{98}}\DIFadd{(B7), 12207--12218.
}

\leavevmode\vadjust \DIFadd{pre}{\DIFaddend \hypertarget{ref-parsons1977}{}}%
...
\DIFdelbegin \DIFdel{Summary statistics for the regression models presented in Section \ref{cdEstimators} are given in Tables \ref{tab:anova} and \ref{tab:regSummary}.
}\DIFdelend
...
\DIFdelbegin %DIFDELCMD < \hypertarget{interpDiffAppendix}{%
%DIFDELCMD < \section{Interpolation Comparisons}\label{interpDiffAppendix}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \clearpage
\DIFaddend 

\DIFdelbegin \DIFdel{Below are }\DIFdelend \DIFaddbegin \begingroup
...
\DIFadd{Below are summaries and visualizations of }\DIFaddend all Similarity and Kriging \DIFdelbegin \DIFdel{interpolation }\DIFdelend \DIFaddbegin \DIFadd{interpolations }\DIFaddend not presented in the main \DIFdelbegin \DIFdel{text }\DIFdelend \DIFaddbegin \DIFadd{body }\DIFaddend of Chapter \ref{chpt3}.
...
{\centering \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\linewidth,]{assets/figs/chpt3/KyushuRyukyuUpperPlate} 
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1\linewidth,]{assets/figs/chpt3/kamchatkaMarianasUpperPlate} 
\DIFaddendFL 

}

\DIFdelbeginFL %DIFDELCMD < \caption[Along strike variability of upper-plate surface heat flow near Kyushu Ryukyu]{%%%
\DIFdelFL{Along strike variability of upper-plate surface }\DIFdelendFL \DIFaddbeginFL \caption[Surface heat flow profiles for Kamchatka Marianas upper-plate sectors]{\DIFaddFL{Surface }\DIFaddendFL heat flow \DIFdelbeginFL \DIFdelFL{near Kyushu Ryukyu}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{profiles for Kamchatka Marianas upper-plate sectors}\DIFaddendFL . Please refer to the main text for explanation of panels and colors.}\DIFdelbeginFL %DIFDELCMD < \label{fig:kyushuRyukyuUpper}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:kamchatkaMarianasUpper}
\DIFaddendFL \end{figure}
