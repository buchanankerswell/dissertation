...
{
/* Counters */
int n1,n2,n3,n4,n5,yn,ynfile=0;
...
/**/
/**/
/* Set initial marker weights */
/* Clear cell counters */
for (m1=0;m1<nodenum;m1++)
	{
	sol0[m1]=0;
	}
/* Compute number of markers in cells */
for (mm1=0;mm1<marknum;mm1++)
/* Check markers out of grid */
if(markx[mm1]>0 && marky[mm1]>0 && (double)(markx[mm1])<xsize && (double)(marky[mm1])<ysize && markt[mm1]<50)
	{
	/* Cell index */
	m1=(long int)(markx[mm1]/xstpx);
	if(m1<0) m1=0; if(m1>xnumx-2) m1=xnumx-2;
	m2=(long int)(marky[mm1]/ystpy);
	if(m2<0) m2=0; if(m2>ynumy-2) m2=ynumy-2;
	m3=m1*ynumy+m2;
	sol0[m3]+=1.0;
	}
/* Compute weights of markers in cells */
for (mm1=0;mm1<marknum;mm1++)
/* Check markers out of grid */
if(markx[mm1]>0 && marky[mm1]>0 && (double)(markx[mm1])<xsize && (double)(marky[mm1])<ysize && markt[mm1]<50)
	{
	...
	if(m2<0) m2=0; if(m2>ynumy-2) m2=ynumy-2;
	m3=m1*ynumy+m2;
	/* Compute marker weight */
	if(sol0[m3]>(double)(mnumx*mnumy)) markwt[mm1]=1.0-(double)(mnumx*mnumy)/sol0[m3];
/*
printf("%ld %ld %ld %ld   %e %e    %e %e   %e  %e",mm1,m1,m2,m3,markx[mm1],marky[mm1],xstpx,ystpy,sol0[m3],markwt[mm1]); getchar();
*/
	}
}
/**/
...
*/
					...
					if(yn==4)
						...
						tk[m3]=(t[1]+t[7]*(gy[m2]-y1*ysize)+(t[3]-t[1])*x)-(1.0-erf(ival))*((t[1]-t[0])*(1.0-x)+(t[3]-t[2])*x);
/*
...
*/
					...
					if(yn==5)
						...
						ival=t[1]+t[7]*(gy[m2]-y1*ysize)-(1.0-erf(ival))*(t[1]-t[0]);
						tk[m3]=t[2]+y*(t[3]-t[2]);
					...
					if(yn==6)
						...
						ival=t[3]+t[7]*(gy[m2]-y1*ysize)-(1.0-erf(ival))*(t[3]-t[2]);
						tk[m3]=t[0]+y*(t[1]-t[0]);
