...
/*
	dvsum+=dscel*((vx[m3-1]-vx[m3-1-ynumy])/(gx[m1]-gx[m1-1])+(vy[m3-ynumy]-vy[m3-1-ynumy])/(gy[m2]-gy[m2-1]));
	dvsum+=dscel*dv[m3];
*/
	dvsum+=dscel*(dv[m3]-prb[m3]*(pr[m3]-pr0[m3])/timestep);
	dvnum+=dscel;
...
/* Add fluid velocity */
maxvx+=MAXV(ABSV(vyfluid),ABSV(vymelt));
maxvy+=MAXV(ABSV(vyfluid),ABSV(vymelt));
if (maxvx)
	{
