...
ffscanf();sedimcyc=atof(sa)*3.15576e+7;
...
ffscanf();stpmelt=atof(sa);
ffscanf();xtkmelt=atof(sa);
ffscanf();lambfld=atof(sa);
ffscanf();lambmlt=atof(sa);
ffscanf();xvolk=atof(sa);
ffscanf();yintmin=atof(sa);
ffscanf();yintmax=atof(sa);
ffscanf();rointmin=atof(sa);
ffscanf();rointmax=atof(sa);
ffscanf();nuplast=atof(sa);
ffscanf();eiiplast=atof(sa);
ffscanf();maxwater=atof(sa);
ffscanf();teclmin=atof(sa);
ffscanf();teclmax=atof(sa);
ffscanf();strainserp=atof(sa);
ffscanf();depthserp=atof(sa);
/**/
/**/
/* Cooling zone */
ffscanf();coolzone=atof(sa);
ffscanf();coolkoef=atof(sa);
ffscanf();dysediment=atof(sa);
ffscanf();dybasalt=atof(sa);
ffscanf();dygabbro=atof(sa);
/*
printf("%e %e %e %e %e",tkpor,pbpor,vyfluid,vymelt,dmwamin);getchar();
*/
/**/
...
if(fl1itp==0)
	...
	for (m2=0;m2<ynumy;m2++)
		...
		ffscanf();pr0[m3]=atof(sa);
		ffscanf();prb[m3]=atof(sa);
		ffscanf();dv[m3]=atof(sa);
	...
	for (mm1=0;mm1<=marknum;mm1++)
		...
		ffscanf();markwt[mm1]=atof(sa);
		ffscanf();markex[mm1]=atof(sa);
		ffscanf();markn[mm1]=atof(sa);
...
/* Load in Binary Format ---------------------------- */
	...
	for (m1=0;m1<nodenum;m1++)
		...
		fread(&ival0,szfloat,1,fl);pr0[m1]=(double)(ival0);
		fread(&ival0,szfloat,1,fl);prb[m1]=(double)(ival0);
		fread(&ival0,szfloat,1,fl);dv[m1]=(double)(ival0);
	...
	for (mm1=0;mm1<=marknum;mm1++)
		...
		fread(&ival0,szfloat,1,fl);markwt[mm1]=ival0;
		fread(&ival0,szfloat,1,fl);markex[mm1]=ival0;
		fread(&ival0,szfloat,1,fl);markn[mm1]=ival0;
...
if (fl1otp==0)
	...
	for (m1=0;m1<xnumx;m1++)
		...
		for (m2=0;m2<ynumy;m2++)
			...
			/* ro, Nu koef */
			fprintf(fl,"% 9.8e % 9.8e % 9.8e % 9.8e % 9.8e % 9.8e % 9.8e % 9.8e % 9.8e ",ro[m3],nu[m3],nd[m3],mu[m3],ep[m3],et[m3],dv[m3],pr0[m3],prb[m3]);
			/* T, Bond T, Cp, Kt, Ht */
	...
	for (m1=0;m1<=marknum;m1++)
		{
		mm2=markt[m1];
		fprintf(fl,"% 9.8e % 9.8e % 9.8e % 9.8e % 9.8e % 9.8e % 9.8e % 9.8e % 9.8e %d \n",markx[m1],marky[m1],markk[m1],markd[m1],markw[m1],markwt[m1],markex[m1],markn[m1],marke[m1],mm2);
		}
	}
/* Save data in text format ---------------------------- */
/**/
...
/* Save data in binary format ---------------------------- */
	...
	for (m1=0;m1<nodenum;m1++)
		...
		ival0=(float)(pr0[m1]);fwrite(&ival0,szfloat,1,fl);
		ival0=(float)(prb[m1]);fwrite(&ival0,szfloat,1,fl);
		ival0=(float)(dv[m1]);fwrite(&ival0,szfloat,1,fl);
	...
	for (mm1=0;mm1<=marknum;mm1++)
		...
		ival0=markwt[mm1];fwrite(&ival0,szfloat,1,fl);
		ival0=markex[mm1];fwrite(&ival0,szfloat,1,fl);
		ival0=markn[mm1];fwrite(&ival0,szfloat,1,fl);
