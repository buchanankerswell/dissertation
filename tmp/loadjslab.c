...
while(sa[0]!='~')
	...
	if(sa[0] == 'S' || sa[0] == 's') fl0otp[fl0num]=3;
	...
	if(sa[0] == 'P' || sa[0] == 'p') fl0otp[fl0num]=15;
	if(sa[0] == 'W' || sa[0] == 'w') fl0otp[fl0num]=16;
	/**/
...
double xincr,xminx0,yminy0,swt1;
...
*/
	...
	if(fl1otp==5) 
		...
		if ((double)(markx[m3])>xminx && (double)(markx[m3])<xmaxx && (double)(marky[m3])>yminy && (double)(marky[m3])<ymaxy && markt[m3]<50)
			{
/*
printf("%d %d  %ld %d   %e %e %e \n",cmin,cmax,m3,markt[m3],markx[m3],marky[m3],markk[m3]); getchar();
*/
...
*/
				...
				swt1=(1.0-markwt[m3])*(1.0-markex[m3]);
				if(m1>0 && m2>0) val0[m4]+=(float)((1.0-ea)*(1.0-na))*swt1;
				if(m1>0 && m2<yresol) val0[m4+1]+=(float)((1.0-ea)*na)*swt1;
				if(m1<xresol && m2>0) val0[m4+yresol]+=(float)(ea*(1.0-na))*swt1;
				if(m1<xresol && m2<yresol) val0[m4+yresol+1]+=(float)(ea*na)*swt1;
/*
...
*/
	...
	if(fl1otp!=5) 
		{
		/* Markers cycle (not for shear heating, vorticty, pressure) */
		if(fl1otp!=4 && fl1otp!=12 && fl1otp!=15) 
		for (m3=0;m3<=marknum;m3+=markstp) 
		/* Check markers out of visualisation area */
		if ((double)(markx[m3])>xminx && (double)(markx[m3])<xmaxx && (double)(marky[m3])>yminy && (double)(marky[m3])<ymaxy && markt[m3]<50)
			{
...
*/
			...
			if(mnu<=0) 
				...
				mnu=viscalc(mtk,mpb,(double)(markx[m3]),(double)(marky[m3]),m3,mm2);
				markn[m3]=mnu;
				}
			/**/
			...
			if(mm2>1 && mm2!=5 && mm2!=6 && mm2!=25 && mm2!=26)
				...
				if(mm2>20 && markex[m3]>0)
					{
					/* Compute TD variables */
					tdbasecalc1(mtk,mpb,mm2-20,m3);
					mro=mro*(1.0-markex[m3])+markex[m3]*eps[41];
					mwa=mwa*(1.0-markex[m3])+markex[m3]*eps[42];
					mcp=mcp*(1.0-markex[m3])+markex[m3]*eps[43];
					mbb=mbb*(1.0-markex[m3])+markex[m3]*eps[44];
					mvp=mvp*(1.0-markex[m3])+markex[m3]*eps[47];
					mvs=mvs*(1.0-markex[m3])+markex[m3]*eps[48];
					}
/*
...
*/
				...
				break;
				/* 15 - P,p - pressure */
				case 15: ival=eps[10]; break;
				/* 16 - W,w - water    */
				case 16: ival=markw[m3]; break;
				}
			/**/
			/* Define relative position of the marker */
			ea=((double)(markx[m3])-xminx0)/xresx+1.0;
...
*/
					...
					break;
					/* 15 - P,p - pressure */
					case 15: val0[m4]=eps[10]; break;
					/* 16 - W,w - water    */
					case 16: val0[m4]=0; break;
					}
				}
			}
		}
