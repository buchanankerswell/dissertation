...
long int v[9];
/* Buffer */
double ival=0,ival1=0,ival2=0,epsval,dpdx,dpdy,e,n;
/* Distances */
...
switch(ynerr)
	...
	/**/
/*
if(m1==786 && m2==93) {printf("%ld %ld   %ld     %e %e %e\n",m1,m2,v[4],ht[v[4]],cp[v[4]],ro[v[4]]); getchar();}
*/
...
for (m2=0;m2<ynumy;m2++)
	...
	if(!bondm[mcmax1])
		...
		heaterr(m1,m2,1);
/*
if(m1==786 && m2>91 && m2<95) {for (n1=0;n1<=wn[0];n1++) printf("%ld %ld   %ld %ld   %ld %e\n",m1,m2,mcmax,mcmax1,wn[n1],wi[n1]); getchar();}
*/
		/* Add matrix */
		gausmat3(3,mcmax,0);
		}
...
if (markx[m3]>0 && marky[m3]>0 && (double)(markx[m3])<xsize && (double)(marky[m3])<ysize && markt[m3]<50)
	...
	if(markk[m3]<=0) 
		...
		if(markk[m3]<tdeep) markk[m3]=(float)(tdeep);
		}
	/**/
/*
...
if (ynerr)
	...
	/* Add Const */
	leftt=sol0[mcmax]-bondv[bondm[mcmax1]][0];
	/* Add Koef */
	if(bondn[bondm[mcmax1]][0]) leftt-=bondv[bondm[mcmax1]][1]*sol0[bondn[bondm[mcmax1]][0]-1-nodenum3];
	/* Add Koef1 */
	if(bondn[bondm[mcmax1]][1]) leftt-=bondv[bondm[mcmax1]][2]*sol0[bondn[bondm[mcmax1]][1]-1-nodenum3];
	/* Add Koef2 */
	if(bondn[bondm[mcmax1]][2]) leftt-=bondv[bondm[mcmax1]][3]*sol0[bondn[bondm[mcmax1]][2]-1-nodenum3];
	return leftt;
	}
/**/
/**/
