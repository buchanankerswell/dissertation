...
*/
	...
	if(timesum<=1e+11 || markd[mm1]<=0 || markx[mm1]<8e+5 || markx[mm1]>17e+5) 
		{
	...
	wro=1050.0;
	mro=mro/(1.0+markw[mm1]*1e-2*(mro/wro-1.0));
	markd[mm1]=mro;
	/**/
	/* Save properties */
	mbb0=mbb=eps[20];
...
if (mm2>20)
	...
	/* Ro=ro0*(1-bro*(TK-298.15))*(1+aro*(Pkbar-0.001)) */
		...
		if(mm2>=27 && mm2<=28)
			...
			eps[23]=xmelt*markro[mm2]*(1.0-markbb[mm2]*(mtk-298.15))*(1.0+markaa[mm2]*(mpb-1.0)*1e-3)+(1.0-xmelt)*ival*markro[mm2-20]*(1.0-markbb[mm2-20]*(mtk-298.15))*(1.0+markaa[mm2-20]*(mpb-1.0)*1e-3);
        		}
		else
			{
			/* Ol-Sp and Pv transitions in the mantle */
			if(mm2>=29 && mm2<=34) 
				...
				{
				eps[23]=xmelt*markro[mm2]*(1.0-markbb[mm2]*(mtk-298.15))*(1.0+markaa[mm2]*(mpb-1.0)*1e-3)+(1.0-xmelt)*markro[mm2-20]*(1.0-markbb[mm2-20]*(mtk-298.15))*(1.0+markaa[mm2-20]*(mpb-1.0)*1e-3);
				}
			}
		}
	/**/
